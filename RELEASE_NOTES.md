...
- `Keypair` implements Kit's `MessagePartialSigner` and `TransactionPartialSigner` interfaces. It exposes `address`, `signMessages(...)`, and `signTransactions(...)`, and can be passed directly to Kit APIs and generated program clients that accept a `TransactionSigner`.
