...
- Prefer Kit-compatible signer APIs when integrating with Kit, Kit Plugins, Codama-generated clients, browser wallets, ledgers, or custom signing systems. `Keypair` provides `signMessages(...)` and `signTransactions(...)`, and implements Kit's `MessagePartialSigner` and `TransactionPartialSigner` interfaces so it can be passed directly to Kit APIs that accept a `TransactionSigner`.
...
- If code converts a web3.js keypair for Kit APIs, pass the keypair directly where a `TransactionSigner` or `MessagePartialSigner` is accepted — `Keypair` implements both. For an API that specifically needs a `KeyPairSigner` backed by a raw `CryptoKeyPair`, rebuild one with `createKeyPairSignerFromBytes(keypair.secretKey)`.
