...
import { getAddressCodec, assertIsAddress, createAddressWithSeed } from '@solana/addresses';
import { assertDigestCapabilityIsAvailable, assertVerificationCapabilityIsAvailable, assertKeyExporterIsAvailable } from '@solana/assertions';
import { verifySignature, signatureBytes, createKeyPairFromBytes, createKeyPairFromPrivateKeyBytes, signBytes } from '@solana/keys';
import { toArrayBuffer, fixEncoderSize, fixDecoderSize, fixCodecSize, transformCodec, addCodecSizePrefix } from '@solana/codecs-core';
import { sha256 as sha256$1 } from '@noble/hashes/sha256';
import { ed25519 } from '@noble/curves/ed25519';
import { getStructEncoder, getBytesEncoder, getArrayEncoder, getStructDecoder, getArrayDecoder, getBytesDecoder, getStructCodec, getBytesCodec, getArrayCodec, getEnumCodec } from '@solana/codecs-data-structures';
import { getShortU16Encoder, getU8Encoder, getU8Decoder, getShortU16Decoder, getU32Decoder, getU64Decoder, getU64Codec, getU32Codec, getI64Codec, getU64Encoder, getU8Codec, getU16Encoder } from '@solana/codecs-numbers';
import * as BufferLayout from '@solana/buffer-layout';
import { blob } from '@solana/buffer-layout';
import { getBase58Encoder, getBase58Decoder, getBase58Codec, getUtf8Codec, getBase64Codec } from '@solana/codecs-strings';
import require$$0 from 'util';
import require$$0$1 from 'http';
import require$$0$2 from 'https';
import { createSolanaRpcApi } from '@solana/rpc-api';
import { lamports } from '@solana/rpc-types';
import { create, number, nullable, array, coerce, type, boolean, instance, optional, string, union, tuple, literal, any, unknown, define, assert as assert$1 } from 'superstruct';
import { createRpc, DEFAULT_RPC_CONFIG, createJsonRpcApi } from '@solana/rpc';
import { CommonClient, WebSocket } from 'rpc-websockets';
import _classPrivateFieldLooseBase from '@babel/runtime/helpers/classPrivateFieldLooseBase';
import _classPrivateFieldLooseKey from '@babel/runtime/helpers/classPrivateFieldLooseKey';
import { SolanaError, SOLANA_ERROR__CODECS__ENUM_DISCRIMINATOR_OUT_OF_RANGE } from '@solana/errors';
 ...
 */
const sha256Sync = data => sha256$1(data);
  ...
  * @param {Array<Signer>} signers Array of signers that will sign the transaction
  ...
  static populate(message, signatures = []) {
    ...
    signatures.forEach((signature, index) => {
      const sigPubkeyPair = {
        signature: signature == BASE58_CODEC.decode(DEFAULT_SIGNATURE) ? null : Buffer.from(BASE58_CODEC.encode(signature)),
...
const rustString = (property = 'string') => {
  ...
  rslShim.alloc = str => {
    return BufferLayout.u32().span + BufferLayout.u32().span + UTF8_ENCODER.encode(str).length;
  };
  return rslShim;
};
...
class Secp256k1Program {
  ...
  static publicKeyToEthAddress(publicKey) {
    ...
    try {
      return toBuffer(keccak_256(publicKeyBytes).slice(-ETHEREUM_ADDRESS_BYTES));
    } catch (error) {
...
const decodeVoteAccountData = bytes => {
  try {
    ...
    if (error instanceof SolanaError && error.context.__code == SOLANA_ERROR__CODECS__ENUM_DISCRIMINATOR_OUT_OF_RANGE) {
...
export { Address, AddressLookupTableAccount, AddressLookupTableInstruction, AddressLookupTableProgram, Authorized, BLOCKHASH_CACHE_TIMEOUT_MS, BPF_LOADER_DEPRECATED_PROGRAM_ID, BPF_LOADER_PROGRAM_ID, BpfLoader, COMPUTE_BUDGET_INSTRUCTIONS, COMPUTE_BUDGET_INSTRUCTION_LAYOUTS, ComputeBudgetProgram, Connection, Ed25519Program, EpochSchedule, Keypair, LAMPORTS_PER_SOL, LOOKUP_TABLE_INSTRUCTIONS, LOOKUP_TABLE_INSTRUCTION_LAYOUTS, Loader, Lockup, MAX_SEED_LENGTH, Message, MessageAccountKeys, MessageV0, NONCE_ACCOUNT_LENGTH, NonceAccount, PACKET_DATA_SIZE, PUBLIC_KEY_LENGTH, PublicKey, SIGNATURE_LENGTH_IN_BYTES, STAKE_CONFIG_ID, STAKE_INSTRUCTIONS, STAKE_INSTRUCTION_LAYOUTS, SYSTEM_INSTRUCTIONS, SYSTEM_INSTRUCTION_LAYOUTS, SYSVAR_CLOCK_PUBKEY, SYSVAR_EPOCH_SCHEDULE_PUBKEY, SYSVAR_INSTRUCTIONS_PUBKEY, SYSVAR_RECENT_BLOCKHASHES_PUBKEY, SYSVAR_RENT_PUBKEY, SYSVAR_REWARDS_PUBKEY, SYSVAR_SLOT_HASHES_PUBKEY, SYSVAR_SLOT_HISTORY_PUBKEY, SYSVAR_STAKE_HISTORY_PUBKEY, Secp256k1Program, SendTransactionError, SolanaJSONRPCError, SolanaJSONRPCErrorCode, StakeAuthorizationLayout, StakeInstruction, StakeProgram, SystemInstruction, SystemProgram, Transaction, TransactionExpiredBlockheightExceededError, TransactionExpiredNonceInvalidError, TransactionExpiredTimeoutError, TransactionInstruction, TransactionMessage, TransactionStatus, VALIDATOR_INFO_KEY, VERSION_PREFIX_MASK, VOTE_INSTRUCTIONS, VOTE_PROGRAM_ID, ValidatorInfo, VersionedMessage, VersionedTransaction, VoteAccount, VoteAuthorizationLayout, VoteInit, VoteInstruction, VoteProgram, VoteStateVersion, __TEST_ONLY__VOTE_ACCOUNT_V1_14_11_CODEC, clusterApiUrl, sendAndConfirmRawTransaction, sendAndConfirmTransaction };
