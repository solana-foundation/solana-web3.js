...
export function SignTransaction() {
    ...
    const onClick = async () => {
        try {
            ...
            if (!(await transaction.verifySignatures())) throw new Error(`Transaction signature invalid! ${signature}`);
            notify('success', `Transaction signature valid! ${signature}`);
        } catch (error) {
            notify('error', `Transaction signing failed! ${(error as Error).message}`);
        }
    };

    return (
        <ActionButton onClick={onClick} disabled={!publicKey} unsupported={!!publicKey && !signTransaction}>
            Sign Transaction
        </ActionButton>
    );
}
