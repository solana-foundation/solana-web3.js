...
export function SignOffchainMessage() {
    ...
    const onClick = async () => {
        try {
            ...
            if (!signOffchainMessage) throw new Error('Wallet does not support offchain message signing!');
            ...
            notify('success', `Offchain message signature: ${getBase58Decoder().decode(signature)}`);
        } catch (error) {
            notify('error', `Sign Offchain Message failed: ${(error as Error).message}`);
        }
    };

    return (
        <ActionButton onClick={onClick} disabled={!publicKey} unsupported={!!publicKey && !signOffchainMessage}>
