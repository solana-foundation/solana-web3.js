...
describe('PublicKey', function () {
    ...
    it('equals', () => {
        ...
        const base58Key = new PublicKey('CiDwVBFgWV9E5MvXWoLgnEgn2hK7rJikbvfWavzAQz3');
    ...
    it('toJSON', () => {
        ...
        expect(JSON.stringify({ key })).to.eq('{"key":"CiDwVBFgWV9E5MvXWoLgnEgn2hK7rJikbvfWavzAQz3"}');
    });
    ...
    it('createWithSeed', async () => {
        ...
        const derivedKey = await PublicKey.createWithSeed(defaultPublicKey, 'limber chicken: 4/45', defaultPublicKey);

        expect(derivedKey.equals(new PublicKey('9h1HyLCW5dZnBVap8C5egQ9Z6pHyjsh5MNy83iPqqRuq'))).to.be.true;
    });

    it('createProgramAddress', async () => {
        const programId = new PublicKey('BPFLoader1111111111111111111111111111111111');
        const publicKey = new PublicKey('SeedPubey1111111111111111111111111111111111');

        let programAddress = await PublicKey.createProgramAddress(
            [Buffer.from('', 'utf8'), Buffer.from([1])],
            programId,
        );
        expect(programAddress.equals(new PublicKey('3gF2KMe9KiC6FNVBmfg9i267aMPvK37FewCip4eGBFcT'))).to.be.true;

        programAddress = await PublicKey.createProgramAddress([Buffer.from('☉', 'utf8')], programId);
        expect(programAddress.equals(new PublicKey('7ytmC1nT1xY4RfxCV2ZgyA7UakC93do5ZdyhdF3EtPj7'))).to.be.true;

        programAddress = await PublicKey.createProgramAddress(
            [Buffer.from('Talking', 'utf8'), Buffer.from('Squirrels', 'utf8')],
            programId,
        );
        expect(programAddress.equals(new PublicKey('HwRVBufQ4haG5XSgpspwKtNd3PC9GM9m1196uJW36vds'))).to.be.true;

        programAddress = await PublicKey.createProgramAddress([publicKey.toBytes()], programId);
        expect(programAddress.equals(new PublicKey('GUs5qLUfsEHkcMB9T38vjr18ypEhRuNWiePW2LoK4E3K'))).to.be.true;

        const programAddress2 = await PublicKey.createProgramAddress([Buffer.from('Talking', 'utf8')], programId);
        expect(programAddress.equals(programAddress2)).to.eq(false);

        await expect(PublicKey.createProgramAddress([Buffer.alloc(MAX_SEED_LENGTH + 1)], programId)).to.be.rejectedWith(
            'The seed at index 0 with length 33 exceeds',
        );

        await expect(PublicKey.createProgramAddress(Array(17).fill(Buffer.alloc(0)), programId)).to.be.rejectedWith(
        ...
        // https://github.com/solana-labs/solana/issues/11950
        {
            ...
            const seeds = [new PublicKey('H4snTKK9adiU15gP22ErfZYtro3aqR9BTMXiH3AwiUTQ').toBytes(), nonceSeed];
            ...
            expect(programAddress.equals(new PublicKey('12rqwuEgBYiGhBrDJStCiqEtzQpTTiZbh7teNVLuYcFA'))).to.be.true;
        }

        // Should work in promise mode, for backwards compatibility
        await PublicKey.createProgramAddress([Buffer.from('', 'utf8'), Buffer.from([1])], programId);
    });

    it('findProgramAddress', async () => {
        const programId = new PublicKey('BPFLoader1111111111111111111111111111111111');
        const [programAddress, nonce] = await PublicKey.findProgramAddress([Buffer.from('', 'utf8')], programId);
        expect(
            programAddress.equals(
                await PublicKey.createProgramAddress([Buffer.from('', 'utf8'), Buffer.from([nonce])], programId),
            ),
        ).to.be.true;

        await expect(PublicKey.findProgramAddress(Array(16).fill(Buffer.alloc(0)), programId)).to.be.rejectedWith(
            'A maximum of 16 seeds, including the bump seed, may be supplied when creating an address.',
        );
    ...
    it('accepts Uint8Array seeds for program address derivation APIs', async () => {
        ...
        const asyncAddressFromBuffers = await PublicKey.createProgramAddress(bufferSeeds, programId);
        const asyncAddressFromUint8 = await PublicKey.createProgramAddress(uint8Seeds, programId);

        expect(asyncAddressFromUint8.equals(asyncAddressFromBuffers)).to.be.true;

        const [addressFromBuffers, nonceFromBuffers] = await PublicKey.findProgramAddress(
            [Buffer.from('', 'utf8')],
            programId,
        );
        const [addressFromUint8, nonceFromUint8] = await PublicKey.findProgramAddress(
    ...
    it('accepts sliced Uint8Array seeds for program address derivation APIs', async () => {
        ...
        const bufferSeeds = [Buffer.from('Talking', 'utf8'), Buffer.from([1])];
        const slicedSeeds = [
            Uint8Array.from([99, ...bufferSeeds[0], 77]).subarray(1, bufferSeeds[0].length + 1),
            Uint8Array.from([88, ...bufferSeeds[1], 66]).subarray(1, 2),
        ];

        const asyncAddressFromBuffers = await PublicKey.createProgramAddress(bufferSeeds, programId);
        const asyncAddressFromSlicedUint8 = await PublicKey.createProgramAddress(slicedSeeds, programId);

        expect(asyncAddressFromSlicedUint8.equals(asyncAddressFromBuffers)).to.be.true;
    });
    ...
    it('isOnCurve', async () => {
        ...
        const offCurve = new PublicKey('12rqwuEgBYiGhBrDJStCiqEtzQpTTiZbh7teNVLuYcFA');
    ...
    it('decode accepts sliced Uint8Array input', () => {
        const publicKey = new PublicKey(1);
        const encoded = publicKey.encode();
        const sliced = Uint8Array.from([99, ...encoded, 77]).subarray(1, encoded.length + 1);
    ...
    it('decodeUnchecked accepts sliced Uint8Array input', () => {
        const publicKey = new PublicKey(1);
        const encoded = publicKey.encode();
        const extended = Uint8Array.from([99, ...encoded, 9, 8, 7, 77]).subarray(1, encoded.length + 4);
    ...
    it('verifies signatures in async and sync modes', async () => {
        ...
        expect(await signer.publicKey.verifySignature(signature, wrongMessage)).to.be.false;
    });
    ...
    it('verifies the bytes supplied at invocation time even if the caller mutates them', async () => {
        ...
        const messageMutation = signer.publicKey.verifySignature(signature, mutableMessage);
        mutableMessage.set(signedMessage);
        const signatureMutation = signer.publicKey.verifySignature(mutableSignature, signedMessage);
