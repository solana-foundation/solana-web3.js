...
import { Transaction, TransactionInstruction, TransactionMessage, VersionedTransaction } from '../src/transaction';
...
const expectPromiseToReject = async (promise: Promise<unknown>, expectedMessage?: string) => {
    try {
        await promise;
...
describe('Transaction', () => {
    describe('compileMessage', () => {
        it('accountKeys are ordered', async function () {
            ...
            const payer = new PublicKey('3qMLYYyNvaxNZP7nW8u5abHMoJthYqQehRLbFVPNNcvQ');
            const accountWritableSigner2 = new PublicKey('3XLtLo5Z4DG8b6PteJidF6kFPNDfxWjxv4vTLrjaHTvd');
            const accountWritableSigner3 = new PublicKey('4rvqGPb4sXgyUKQcvmPxnWEZTTiTqNUZ2jjnw7atKVxa');
            const accountSigner4 = new PublicKey('5oGjWjyoKDoXGpboGBfqm9a5ZscyAjRi3xuGYYu1ayQg');
            const accountSigner5 = new PublicKey('65Rkc3VmDEV6zTRGtgdwkTcQUxDJnJszj2s4WoXazYpC');
            const accountWritable6 = new PublicKey('72BxBZ9eD9Ue6zoJ9bzfit7MuaDAnq1qhirgAoFUXz9q');
            const accountWritable7 = new PublicKey('BtYrPUeVphVgRHJkf2bKz8DLRxJdQmZyANrTM12xFqZL');
            const accountRegular8 = new PublicKey('Di1MbqFwpodKzNrkjGaUHhXC4TJ1SHUAxo9agPZphNH1');
            const accountRegular9 = new PublicKey('DYzzsfHTgaNhCgn7wMaciAYuwYsGqtVNg9PeFZhH93Pc');
            const programId = new PublicKey('Fx9svCTdxnACvmEmx672v2kP1or4G1zC73tH7XsXbKkP');
        ...
        it('accountKeys collapses signedness and writability of duplicate accounts', async function () {
            // These pubkeys are chosen specially to be in sort order.
            const payer = new PublicKey('2eBgaMN8dCnCjx8B8Wrwk974v5WHwA6Vvj4N2mW9KDyt');
            const account2 = new PublicKey('DL8FErokCN7rerLdmJ7tQvsL1FsqDu1sTKLLooWmChiW');
            const account3 = new PublicKey('EdPiTYbXFxNrn1vqD7ZdDyauRKG4hMR6wY54RU1YFP2e');
            const account4 = new PublicKey('FThXbyKK4kYJBngSSuvo9e6kc7mwPHEgw4V8qdmz1h3k');
            const programId = new PublicKey('Gcatgv533efD1z2knsH9UKtkrjRWCZGi12f8MjNaDzmN');
        ...
        it('prepends the nonce advance instruction when compiling nonce-based transactions', () => {
            ...
            expect(message.instructions[0]).to.deep.equal(expectedNonceAdvanceCompiledInstruction);
        });
    ...
    it('partialSign', async function () {
        ...
        await expectPromiseToReject(partialTransaction.serialize({ requireAllSignatures: false }));
        await partialTransaction.serialize({
    ...
    it('defaults to the maximum lastValidBlockHeight when the transaction has none', async function () {
        const keyPairSigner = await generateKeyPairSigner();
        const signerPublicKey = new PublicKey(keyPairSigner.address);
        const recipient = await generateKeypair();
        const recentBlockhash = blockhash(keyPairSigner.address);
    ...
    it('applies co-signer signatures returned by a Kit transaction partial signer', async function () {
        ...
        const transactionOnlySigner = {
            ...
            signTransactions: async transactions => {
                const [payerSignatures] = await payerSigner.signTransactions(transactions);
                const [coSignerSignatures] = await coSigner.signTransactions(transactions);
        ...
        expect(transaction.signatures.find(pair => pair.publicKey.equals(payerPublicKey))?.signature).not.to.be.null;
        expect(transaction.signatures.find(pair => pair.publicKey.equals(coSignerPublicKey))?.signature).not.to.be.null;
        expect(await transaction.verifySignatures()).to.be.true;
    });
    ...
    it('rejects message-only Kit signers', async function () {
        ...
        try {
            // @ts-expect-error message-only signers cannot sign transactions
            await transaction.sign(messageOnlySigner);
            expect.fail('expected sign to reject');
        } catch (e) {
            expect(isSolanaError(e, SOLANA_ERROR__SIGNER__EXPECTED_TRANSACTION_PARTIAL_SIGNER)).to.be.true;
        }
    });
    ...
    it('allows noop Kit partial signers to leave signatures empty', async function () {
        ...
        expect(transaction.signatures[0].publicKey.equals(signer.publicKey)).to.be.true;
    ...
    describe('dedupe', () => {
        ...
        it('setSigners', () => {
            ...
            transaction.setSigners(payer.publicKey, duplicate1.publicKey, duplicate2.publicKey);
    ...
    it('parse wire format and serialize', async () => {
        ...
        const recipient = new PublicKey('J3dxNj7nDRRqRRXuEMynDG57DkZK4jYRuv3Garmb1i99'); // Arbitrary known public key
        const transfer = SystemProgram.transfer({
        ...
        expect(await deserializedTransaction.serialize()).to.eql(serializedTransaction);
    });
    ...
    it('copies the populated message so later mutation of the source does not change the compiled message', () => {
        const recentBlockhash = blockhash(new PublicKey(1).toString());
        const message = new Message({
            accountKeys: [new PublicKey(1).toString(), new PublicKey(2).toString(), new PublicKey(3).toString()],
            header: {
                numReadonlySignedAccounts: 0,
        ...
        expect(transaction.compileMessage().accountKeys[1]).to.eql(new PublicKey(2));
    ...
    it('constructs a transaction with last valid block height', () => {
        const parsedBlockhash = blockhash('EETubP5AKHgjPAhzPAFcb8BAY1hMH639CWCFTqi3hq1k');
    ...
    it('constructs a transaction with nonce information', () => {
        ...
        const nonceValue = blockhash('EETubP5AKHgjPAhzPAFcb8BAY1hMH639CWCFTqi3hq1k');
        const nonceInfo = {
            nonce: nonceValue,
            nonceInstruction: SystemProgram.nonceAdvance({
    ...
    it('constructs a transaction with only a recent blockhash', () => {
        const recentBlockhash = blockhash('EETubP5AKHgjPAhzPAFcb8BAY1hMH639CWCFTqi3hq1k');
        const transaction = new Transaction({
            recentBlockhash,
        });
        expect(transaction.recentBlockhash).to.eq(recentBlockhash);
    ...
    it('serialize unsigned transaction', async () => {
        ...
        await expectPromiseToReject(expectedTransaction.serialize(), 'Transaction fee payer required');
        ...
        expect(Buffer.from(serializedTransaction)).to.eql(expectedSerializationWithNoSignatures);
        ...
        expect(Buffer.from(await expectedTransaction.serialize())).to.eql(expectedSerialization);
        expect(expectedTransaction.signatures).to.have.length(1);
    });
    ...
    describe('partially signed transaction signature verification tests', () => {
        ...
        it('throws when partially signing with a non signer key', async () => {
            await expectPromiseToReject(
                expectedTransaction.partialSign(fakeKey),
                'unknown signer: ' + fakeKey.publicKey.toBase58(),
            );
        });
    });
    ...
    it('preserves Uint8Array instruction data added from plain objects', async () => {
        ...
        expect(transaction.compileMessage().instructions[0].data).to.eql(BASE58_DECODER.decode(data));
    });
    ...
    it('normalizes externally added Uint8Array signatures to Uint8Array storage', async () => {
        ...
        const signature = new Uint8Array(await sign(tx.serializeMessage(), authority.secretKey));
    ...
    it('normalizes Uint8Array signatures provided via constructor fields', async () => {
        ...
        expect(transaction.signatures[0].signature?.constructor).to.equal(Uint8Array);
    ...
    it('can serialize, deserialize, and reserialize with a partial signer', async function () {
        ...
        const t1 = Transaction.from(await t0.serialize({ requireAllSignatures: false }));
    ...
    it('deserializes from Buffer, sliced Uint8Array, and Array<number> inputs', async function () {
        ...
        const recentBlockhash = blockhash('EETubP5AKHgjPAhzPAFcb8BAY1hMH639CWCFTqi3hq1k');
        const recipient = new PublicKey('J3dxNj7nDRRqRRXuEMynDG57DkZK4jYRuv3Garmb1i99');
        const transaction = new Transaction({
            blockhash: recentBlockhash,
            feePayer: sender.publicKey,
            lastValidBlockHeight: 9999,
        ...
        expect(Buffer.from(deserialized.instructions[0].data)).to.eql(Buffer.from(transaction.instructions[0].data));
        ...
        expect(await Transaction.from(Array.from(serialized)).serialize()).to.eql(serialized);
    });
});
...
describe('VersionedTransaction', () => {
    ...
    describe('sign with kit transaction partial signers', () => {
        it('passes blockhash lifetime to Kit transaction partial signers', async function () {
            ...
            expect(
                await new PublicKey(keyPairSigner.address).verifySignature(
        ...
        it('defaults to the maximum lastValidBlockHeight when none is provided', async function () {
            const keyPairSigner = await generateKeyPairSigner();
            const recentBlockhash = await generateBlockhash();
            let lifetimeConstraint: unknown;
            const transactionOnlySigner = {
        ...
        it('passes blockhash lifetime for a v1 message', async function () {
            const keyPairSigner = await generateKeyPairSigner();
            const recentBlockhash = await generateBlockhash();
            let lifetimeConstraint: unknown;
            const transactionOnlySigner = {
        ...
        it('passes previously added signatures through to the signer', async function () {
            const payer = await generateKeyPairSigner();
            ...
            expect(Buffer.from(transaction.signatures[0])).to.not.eql(Buffer.alloc(64));
        ...
        it('dedupes distinct signer objects that share an address', async function () {
            ...
            expect(await payer.publicKey.verifySignature(transaction.signatures[0], message.serialize())).to.be.true;
        });
        ...
        it('rejects a durable nonce whose nonce account is loaded from a lookup table', async function () {
            const payer = await generateKeyPairSigner();
            const lookupTableKey = (await generateKeypair()).publicKey;
            const nonce = await generateBlockhash();
            ...
            try {
                ...
                expect(isSolanaError(e, SOLANA_ERROR__TRANSACTION__NONCE_ACCOUNT_CANNOT_BE_IN_LOOKUP_TABLE)).to.be.true;
            }
        });
    });
    ...
    describe('addSignature', () => {
        ...
        it('appends externally generated signatures at correct indexes', async () => {
            const signature1 = await sign(transaction.message.serialize(), signer1.secretKey);
            const signature2 = await sign(transaction.message.serialize(), signer2.secretKey);
    ...
    describe('v1 transactions', () => {
        it('serializes with the message-first envelope and round trips', async () => {
            ...
            expect(serialized.subarray(0, serializedMessage.length)).to.eql(serializedMessage);
        ...
        it('serialized bytes decode with the kit transaction decoder', async () => {
            ...
            const kitTransaction = getTransactionDecoder().decode(transaction.serialize());
            expect(new Uint8Array(kitTransaction.messageBytes)).to.eql(message.serialize());
            expect(new Uint8Array(kitTransaction.signatures[payer.publicKey.toBase58()]!)).to.eql(
                transaction.signatures[0],
            );
        });
    ...
    describe('add(InstructionPlan)', () => {
        const makeKitIx = (programAddress: PublicKey, payload: number): KitInstruction => ({
            accounts: [],
        ...
        it('flattens a SingleInstructionPlan into one instruction', () => {
            ...
            const tx = new Transaction().add(singleInstructionPlan(makeKitIx(programId, 7)));

            expect(tx.instructions).to.have.length(1);
            expect(tx.instructions[0].programId.toBase58()).to.eq(programId.toBase58());
        ...
        it('flattens a SequentialInstructionPlan preserving order', () => {
            ...
            const tx = new Transaction().add(
                sequentialInstructionPlan([makeKitIx(programA, 1), makeKitIx(programB, 2)]),
            );

            expect(tx.instructions).to.have.length(2);
            expect(tx.instructions[0].programId.toBase58()).to.eq(programA.toBase58());
            expect(tx.instructions[0].data).to.eql(new Uint8Array([1]));
            expect(tx.instructions[1].programId.toBase58()).to.eq(programB.toBase58());
            expect(tx.instructions[1].data).to.eql(new Uint8Array([2]));
        });

        it('flattens nested sequential plans', () => {
            const [a, b, c] = [getUniqueAddress(), getUniqueAddress(), getUniqueAddress()];
            const tx = new Transaction().add(
                sequentialInstructionPlan([
                    makeKitIx(a, 1),
        ...
        it('flattens a real codama-client plan (getCreateMintInstructionPlan)', async () => {
            const { TOKEN_PROGRAM_ADDRESS, getCreateMintInstructionPlan } = await import('@solana-program/token');
            ...
            expect(createAccount.programId.toBase58()).to.eq(SystemProgram.programId.toBase58());
            ...
            expect(createAccount.keys.every(k => k.isSigner && k.isWritable)).to.be.true;

            expect(initMint.programId.toBase58()).to.eq(TOKEN_PROGRAM_ADDRESS);
            expect(initMint.keys).to.have.length(1);
            expect(initMint.keys[0].pubkey.toBase58()).to.eq(newMint.address.toString());
        ...
        it('throws "No instructions" when every input is an empty plan', () => {
            expect(() => new Transaction().add(sequentialInstructionPlan([]))).to.throw('No instructions');
        });
        ...
        it('throws when the plan resolves to a MessagePackerInstructionPlan leaf', () => {
            ...
            expect(() => new Transaction().add(packer)).to.throw(/Unsupported InstructionPlan leaf kind/);
        });
    });
});
