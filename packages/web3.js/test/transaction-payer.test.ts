...
import { Keypair, Connection, Transaction, SystemProgram, LAMPORTS_PER_SOL } from '../src';
...
describe('Transaction Payer', function () {
    ...
    it('transaction-payer', async () => {
        ...
        const balance = await connection.getBalance(accountPayer.publicKey, 'confirmed');
        ...
        expect(await connection.getBalance(accountFrom.publicKey, 'confirmed')).to.eq(minimumAmount + 2n);
    }).timeout(30 * 1000);
});
