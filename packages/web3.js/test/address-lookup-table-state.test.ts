...
const loadLookupTableFixture = (
    ...
    const fixture = JSON.parse(readFileSync(fixtureUrl, 'utf8')) as LookupTableFixture;
...
const expectFixtureBytes = (fixture: LookupTableFixture, bytes: Buffer): void => {
...
const getAnnotation = (fixture: LookupTableFixture, label: string): LookupTableFixture['annotations'][number] => {
    const annotation = fixture.annotations.find(entry => entry.label === label);
    expect(annotation, `Missing fixture annotation: ${label}`).to.not.eq(undefined);
    return annotation as LookupTableFixture['annotations'][number];
};

const readU64LE = (bytes: Buffer, fixture: LookupTableFixture, label: string): bigint =>
    bytes.readBigUInt64LE(getAnnotation(fixture, label).offset);

const readU8 = (bytes: Buffer, fixture: LookupTableFixture, label: string): number =>
    bytes.readUInt8(getAnnotation(fixture, label).offset);

const readBytes = (bytes: Buffer, fixture: LookupTableFixture, label: string): Uint8Array => {
    const annotation = getAnnotation(fixture, label);
    return bytes.subarray(annotation.offset, annotation.offset + annotation.length);
};

const getAddressAnnotations = (fixture: LookupTableFixture): LookupTableFixture['annotations'] =>
    fixture.annotations.filter(annotation => annotation.label.startsWith('addresses['));

describe('AddressLookupTableAccount', () => {
    it('deserializes lookup table state with authority and addresses', () => {
        const { fixture, bytes } = loadLookupTableFixture('address-lookup-table-state-with-authority.json');
        ...
        const lastExtendedStartIndex = readU8(bytes, fixture, 'lastExtendedStartIndex');
        ...
        if (authorityOption === 0) {
            ...
            expect(state.authority?.equals(new PublicKey(authorityBytes))).to.eq(true);
        }
        expect(state.addresses).to.have.length(addressAnnotations.length);
        addressAnnotations.forEach((annotation, index) => {
            const addressBytes = bytes.subarray(annotation.offset, annotation.offset + annotation.length);
            expect(state.addresses[index].equals(new PublicKey(addressBytes))).to.eq(true);
        });
    });

    it('deserializes lookup table state without authority', () => {
        const { fixture, bytes } = loadLookupTableFixture('address-lookup-table-state-without-authority.json');
        expectFixtureBytes(fixture, bytes);
    ...
    it('rejects account data whose discriminator is not the lookup table variant', () => {
        ...
        expect(() => AddressLookupTableAccount.deserialize(zeroed)).to.throw(/discriminator/);
    });
});
