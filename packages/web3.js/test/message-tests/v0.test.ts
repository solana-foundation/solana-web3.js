...
import { MessageAccountKeys, MessageAddressTableLookup, MessageV0 } from '../../src/message';
...
function createTestLookupTable(addresses: Array<PublicKey>): AddressLookupTableAccount {
...
describe('MessageV0', () => {
    ...
    it('getAccountKeys', () => {
        ...
        ).to.throw('Failed to get account keys because of a mismatch in the number of account keys from lookups');

        const accountKeysFromLookups = message.resolveAddressTableLookups([lookupTable]);
        const expectedAccountKeys = new MessageAccountKeys(staticAccountKeys, accountKeysFromLookups);
    ...
    it('resolveAddressTableLookups', () => {
        ...
        const createTestMessage = (addressTableLookups: MessageAddressTableLookup[]): MessageV0 => {
        ...
        expect(createTestMessage([]).resolveAddressTableLookups([lookupTable])).to.eql({
            writable: [],
            readonly: [],
        });
    ...
    it('serialize and deserialize', () => {
        ...
        expect(JSON.stringify(messageV0)).to.eql(JSON.stringify(deserializedMessage));
    });
    ...
    it('deserialize failures', () => {
        ...
        }).to.throw('Expected versioned message with version 0 but found version 1');
    });
