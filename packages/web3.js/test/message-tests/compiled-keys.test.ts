...
describe('CompiledKeys', () => {
    ...
    it('compile with dup key', () => {
        ...
        const compiledKeys = CompiledKeys.compile(
            [
                new TransactionInstruction({
                    programId: programId,
                    keys: [createAccountMeta(key, false, false), createAccountMeta(key, true, true)],
                }),
            ],
            payer,
        );
    ...
    it('getMessageComponents with overflow', () => {
        ...
        expect(() => compiledKeys.getMessageComponents()).to.throw('Max static account keys length exceeded');
    });
    ...
    it('extractTableLookup with invalid lookup table', () => {
        ...
        const lookupTable = createTestLookupTable(keys);
        const compiledKeys = new CompiledKeys(keys[0], map);
        expect(() => compiledKeys.extractTableLookup(lookupTable)).to.throw('Max lookup table index exceeded');
    });
});
...
function createAccountMeta(pubkey: PublicKey, isSigner: boolean, isWritable: boolean): AccountMeta {
    return {
        pubkey,
        isSigner,
        isWritable,
