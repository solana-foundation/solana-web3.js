...
import { Message, MessageAccountKeys, MessageV0, MessageV1 } from '../../src/message';
...
const TEST_RECENT_BLOCKHASH = blockhash('Bjj4AWTNrjQVHqgWbP2XaxXz4DYH1WZMyERHxsad7b2w');

function createTestKeys(count: number): Array<PublicKey> {
    return Array.from({ length: count }).map(() => getUniqueAddress());
}
...
describe('MessageV1', () => {
    it('getAccountKeys', () => {
        ...
        expect(message.getAccountKeys()).to.eql(new MessageAccountKeys(staticAccountKeys));
    });
    ...
    it('compiles with Kit instructions', () => {
        ...
        expect(messageFromKitInstruction.serialize()).to.deep.equal(messageFromLegacyInstruction.serialize());
    });
    ...
    it('serialize and deserialize', () => {
        const messageV1 = new MessageV1({
            ...
            staticAccountKeys: [new PublicKey(1), new PublicKey(2), new PublicKey(3), new PublicKey(4)],
            compiledInstructions: [
                {
                    programIdIndex: 1,
                    accountKeyIndexes: [2, 3],
        ...
        expect(deserializedMessage.staticAccountKeys).to.eql(messageV1.staticAccountKeys);
        expect(deserializedMessage.recentBlockhash).to.eq(messageV1.recentBlockhash);
        expect(deserializedMessage.compiledInstructions).to.eql(messageV1.compiledInstructions);
    ...
    it('compiles and round trips the maximum of 64 instructions', () => {
        ...
        const instructions = Array.from({ length: 64 }).map(
            (_, index) =>
                new TransactionInstruction({
                    programId,
                    keys: [
        ...
        expect(deserialized.compiledInstructions).to.eql(message.compiledInstructions);
    ...
    it('rejects compiling more than 64 instructions', () => {
        const payerKey = getUniqueAddress();
        const programId = getUniqueAddress();
        const instructions = Array.from({ length: 65 }).map(
            () =>
                new TransactionInstruction({
                    programId,
        ...
        ).to.throw('Version 1 messages support at most 64 instructions but found 65');
    });
    ...
    it('serialized bytes decode with the kit message decoder', () => {
        ...
        const decoded = getCompiledTransactionMessageDecoder().decode(message.serialize());
    ...
    it('deserialize failures', () => {
        ...
        }).to.throw('Expected versioned message with version 1 but found version 0');
    });
    ...
    it('rejects a config mask with unsupported bits set', () => {
        ...
        expect(deserialize).to.throw('Unexpected bits set in the transaction config mask');
    });

    it('isAccountWritable', () => {
        const staticAccountKeys = [getUniqueAddress(), getUniqueAddress(), getUniqueAddress(), getUniqueAddress()];
