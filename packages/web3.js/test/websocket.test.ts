...
async function waitForSocketToOpen(connection: Connection): Promise<SubscriptionChannel> {
    while (true) {
...
async function waitForSocketClose(connection: Connection, socket: SubscriptionChannel): Promise<void> {
    while (getConnectionSocket(connection) === socket) {
        await sleep(25);
    }
}

function getConnectionSocket(connection: Connection): SubscriptionChannel | null {
    return (connection as unknown as ConnectionWithSubscriptionChannelState)._subscriptionChannel;
}
