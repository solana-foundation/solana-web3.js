...
describe('send and confirm helpers', function () {
  ...
  describe('sendAndConfirmTransaction', function () {
    ...
    beforeEach(() => {
      sendTransactionStub = stub(connection, 'sendTransaction').resolves(
        'mock-signature',
      );
    });
    ...
    it('confirms at the connection commitment when no commitment is supplied', async () => {
      ...
      expect(confirmTransactionStub).to.have.been.calledWithExactly(
        'mock-signature',
        'confirmed',
      );
    });
    ...
    it('confirms at `finalized` when neither the options nor the connection specify a commitment', async () => {
      ...
      expect(bareConfirmStub).to.have.been.calledWithExactly(
        'mock-signature',
        'finalized',
      );
    });
  ...
  describe('sendAndConfirmRawTransaction', function () {
    ...
    beforeEach(() => {
      sendRawTransactionStub = stub(connection, 'sendRawTransaction').resolves(
        'mock-signature',
      );
    });
