...
describe('Keypair', function () {
    ...
    it('satisfies the Kit partial signer shapes', async () => {
        ...
        const [signatureDictionary] = await keypair.signMessages([createSignableMessage(message)]);
        const signature = signatureDictionary[keypair.address];

        expect(signature).not.to.be.undefined;
        expect(await keypair.publicKey.verifySignature(signature!, message)).to.be.true;
    });
    ...
    it('two generated keypairs differ', async () => {
        ...
        expect(keypairA.publicKey.toBase58()).to.not.equal(keypairB.publicKey.toBase58());
    });
    ...
    it('create keypair from secret key', async () => {
        ...
        expect(keypair.publicKey.toBase58()).to.eq('2q7pyhPwAwZ3QMfZrnAbDhnh9mDUqycszcpf86VgQxhF');
    ...
    it('generate keypair from random seed', async () => {
        ...
        expect(keypair.publicKey.toBase58()).to.eq('2KW2XRd9kwqet15Aha2oK3tYvd3nWbTFH1MBiRAv1BE1');
    });
    ...
    it('fails verification for the wrong message', async () => {
        ...
        expect(await keypair.verifySignature(signature, Buffer.from('other message'))).to.be.false;
    });
    ...
    it('fails verification for a different public key', async () => {
        ...
        expect(await otherKeypair.publicKey.verifySignature(signature, message)).to.be.false;
