...
describe('AddressLookupTableProgram', function () {
    ...
    it('rejects a non-extend instruction passed to decodeExtendLookupTable', async () => {
        const authorityPubkey = (await Keypair.generate()).publicKey;
        const payerPubkey = (await Keypair.generate()).publicKey;
        const [createInstruction] = await AddressLookupTableProgram.createLookupTable({
            authority: authorityPubkey,
            payer: payerPubkey,
            recentSlot: 0,
        });

        const decode = () => AddressLookupTableInstruction.decodeExtendLookupTable(createInstruction);
