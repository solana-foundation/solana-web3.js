...
describe('SystemProgram', function () {
    it('createAccount', async () => {
        ...
        const transaction = new Transaction().add(SystemProgram.createAccount(params));
        ...
        expect(decodedParams).to.eql(SystemInstruction.decodeCreateAccount(systemInstruction));
    });
    ...
    it('transfer', async () => {
        ...
        expect(decodedParams).to.eql(SystemInstruction.decodeTransfer(systemInstruction));
    });
    ...
    it('transferWithSeed', async () => {
        ...
        expect(decodedParams).to.eql(SystemInstruction.decodeTransferWithSeed(systemInstruction));
    });
    ...
    it('allocate', async () => {
        ...
        expect(decodedParams).to.eql(SystemInstruction.decodeAllocate(systemInstruction));
    });
    ...
    it('allocateWithSeed', async () => {
        ...
        expect(decodedParams).to.eql(SystemInstruction.decodeAllocateWithSeed(systemInstruction));
    });
    ...
    it('assignWithSeed', async () => {
        ...
        expect(transaction.instructions).to.have.length(1);
        const [systemInstruction] = transaction.instructions;
        expect(params).to.eql(SystemInstruction.decodeAssignWithSeed(systemInstruction));
    });
    ...
    it('createAccountWithSeed', async () => {
        ...
        expect(decodedParams).to.eql(SystemInstruction.decodeCreateWithSeed(systemInstruction));
    });
    ...
    it('createNonceAccount', async () => {
        ...
        const transaction = new Transaction().add(SystemProgram.createNonceAccount(params));
        ...
        expect(createParams).to.eql(SystemInstruction.decodeCreateAccount(createInstruction));

        const initParams = {
            noncePubkey: params.noncePubkey,
            authorizedPubkey: fromPubkey,
        };
        expect(initParams).to.eql(SystemInstruction.decodeNonceInitialize(initInstruction));
    });
    ...
    it('createNonceAccount with seed', async () => {
        ...
        expect(createParams).to.eql(SystemInstruction.decodeCreateWithSeed(createInstruction));
    ...
    it('nonceWithdraw', async () => {
        ...
        const transaction = new Transaction().add(SystemProgram.nonceWithdraw(params));
        expect(transaction.instructions).to.have.length(1);
        ...
        expect(decodedParams).to.eql(SystemInstruction.decodeNonceWithdraw(instruction));
    });
    ...
    it('nonceAuthorize', async () => {
        ...
        const transaction = new Transaction().add(SystemProgram.nonceAuthorize(params));
        expect(transaction.instructions).to.have.length(1);
        const [instruction] = transaction.instructions;
    ...
    it('non-SystemInstruction error', async () => {
        ...
        expect(() => {
            SystemInstruction.decodeInstructionType(new TransactionInstruction(badProgramId));
        }).to.throw();
    ...
    if (process.env.TEST_LIVE) {
        it('live Nonce actions', async () => {
            ...
            await sendAndConfirmTransaction(connection, createNonceAccount, [from, nonceAccount], {
            ...
            const withdrawBalance = await connection.getBalance(withdrawAccount.publicKey);
        ...
        it('live withSeed actions', async () => {
            ...
            const createAccountWithSeedAddress = await PublicKey.createWithSeed(basePubkey, seed, programId);
            const space = 0;

            const minimumAmount = await connection.getMinimumBalanceForRentExemption(space);
            ...
            await sendAndConfirmTransaction(connection, createAccountWithSeedTransaction, [baseAccount], {
                preflightCommitment: 'confirmed',
            });
            const createAccountWithSeedBalance = await connection.getBalance(createAccountWithSeedAddress);
            ...
            const createAccountWithSeedBalance2 = await connection.getBalance(createAccountWithSeedAddress2);
            ...
            const transferWithSeedAddress = await PublicKey.createWithSeed(basePubkey, seed, programId2);
            await sendAndConfirmTransaction(
                connection,
                new Transaction().add(
                    SystemProgram.transfer({
            ...
            let transferWithSeedAddressBalance = await connection.getBalance(transferWithSeedAddress);
            expect(transferWithSeedAddressBalance).to.eq(3n * minimumAmount);

            // Test TransferWithSeed
            const programId3 = await Keypair.generate();
            const toPubkey = await PublicKey.createWithSeed(basePubkey, seed, programId3.publicKey);
            ...
            const transferWithSeedTransaction = new Transaction().add(SystemProgram.transfer(transferWithSeedParams));
            await sendAndConfirmTransaction(connection, transferWithSeedTransaction, [baseAccount], {
                preflightCommitment: 'confirmed',
            });
            const toBalance = await connection.getBalance(toPubkey);
            expect(toBalance).to.eq(2n * minimumAmount);
            transferWithSeedAddressBalance = await connection.getBalance(createAccountWithSeedAddress);
            ...
            const allocateWithSeedTransaction = new Transaction().add(SystemProgram.allocate(allocateWithSeedParams));
            await sendAndConfirmTransaction(connection, allocateWithSeedTransaction, [baseAccount], {
                preflightCommitment: 'confirmed',
            });
            ...
            await sendAndConfirmTransaction(connection, assignWithSeedTransaction, [baseAccount], {
                preflightCommitment: 'confirmed',
            });
