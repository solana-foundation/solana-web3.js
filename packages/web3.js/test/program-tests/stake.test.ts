...
describe('StakeProgram', function () {
    ...
    it('split and withdraw preserve lamports above Number.MAX_SAFE_INTEGER', async () => {
        ...
        expect(StakeProgram.split(decodedSplit, 1).instructions[1].data).to.deep.equal(splitInstruction.data);

        const withdrawParams = { stakePubkey, authorizedPubkey, toPubkey, lamports };
        const [withdrawInstruction] = StakeProgram.withdraw(withdrawParams).instructions;
        const decodedWithdraw = StakeInstruction.decodeWithdraw(withdrawInstruction);
        expect(decodedWithdraw).to.eql(withdrawParams);
        expect(StakeProgram.withdraw(decodedWithdraw).instructions[0].data).to.deep.equal(withdrawInstruction.data);
    });
