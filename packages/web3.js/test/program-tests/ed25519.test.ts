...
const readEd25519InstructionHeader = (data: Buffer) => {
    const view = new DataView(data.buffer, data.byteOffset, ED25519_INSTRUCTION_HEADER_SIZE);
...
describe('ed25519 header encoding', () => {
    it('encodes default instruction indexes and offsets', async () => {
        ...
        expect(data.subarray(publicKeyOffset, publicKeyOffset + PUBLIC_KEY_BYTES)).to.eql(Buffer.from(publicKey));
        expect(data.subarray(signatureOffset, signatureOffset + SIGNATURE_BYTES)).to.eql(Buffer.from(signature));
...
describe('ed25519 instruction validation', () => {
    ...
    it('matches instruction data built from private key', async () => {
        ...
        expect(Buffer.from(withPrivateKey.data)).to.eql(Buffer.from(withPublicKey.data));
    });
