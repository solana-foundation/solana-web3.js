...
describe('ComputeBudgetProgram', function () {
    it('requestUnits', () => {
        ...
        expect(ComputeBudgetInstruction.decodeInstructionType(ix)).to.eq('RequestUnits');
    });
    ...
    it('requestHeapFrame', () => {
        ...
        expect(ComputeBudgetInstruction.decodeInstructionType(ix)).to.eq('RequestHeapFrame');
    });
    ...
    it('setComputeUnitLimit', () => {
        ...
        const decodedParams = ComputeBudgetInstruction.decodeSetComputeUnitLimit(ix);
        expect(decodedParams).to.eql(params);
        expect(ComputeBudgetInstruction.decodeInstructionType(ix)).to.eq('SetComputeUnitLimit');
    });
    ...
    it('setComputeUnitPrice', () => {
        ...
        expect(ComputeBudgetInstruction.decodeInstructionType(ix)).to.eq('SetComputeUnitPrice');
    });
    ...
    it('setLoadedAccountsDataSizeLimit', () => {
        ...
        const decodedParams = ComputeBudgetInstruction.decodeSetLoadedAccountsDataSizeLimit(ix);
        expect(decodedParams).to.eql(params);
        expect(ComputeBudgetInstruction.decodeInstructionType(ix)).to.eq('SetLoadedAccountsDataSizeLimit');
    });
    ...
    if (process.env.TEST_LIVE) {
        it('send live request heap ix', async () => {
            ...
            async function expectRequestHeapFailure(bytes: number) {
                ...
                await expect(
                    sendAndConfirmTransaction(connection, requestHeapFrameTransaction, [baseAccount], {
            ...
            await sendAndConfirmTransaction(connection, requestHeapFrameTransaction, [baseAccount], {
                preflightCommitment: 'confirmed',
            });
        });
        ...
        it('send live compute unit ixs', async () => {
            ...
            await expect(
                sendAndConfirmTransaction(connection, prioritizationFeeTooHighTransaction, [baseAccount], {
                    preflightCommitment: 'confirmed',
                }),
            ).to.be.rejected;
            ...
            await sendAndConfirmTransaction(connection, validPrioritizationFeeTransaction, [baseAccount], {
                preflightCommitment: 'confirmed',
            });
            expect((await connection.getBalance(baseAccount.publicKey)) <= BigInt(STARTING_AMOUNT - FEE_AMOUNT)).to.eq(
                true,
            );
        });
    }
