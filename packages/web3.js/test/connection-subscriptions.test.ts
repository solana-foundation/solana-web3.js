...
const ALTERNATE_TRANSACTION_SIGNATURE = BASE58_DECODER.decode(new Uint8Array(64).fill(1));
...
describe('Subscriptions', () => {
    ...
    const subscriptionMethodsConfig = {
        accountSubscribe: {
            ...
            setupListenerWithDefaultsOmitted(callback: AccountChangeCallback): number {
                return connection.onAccountChange(PublicKey.default, callback);
            },
            setupListenerWithDefaultableParamsSetToTheirDefaults(callback: AccountChangeCallback): number {
                return connection.onAccountChange(PublicKey.default, callback, connection.commitment || 'confirmed');
            },
            publishNotificationForServerSubscriptionId(harness: SubscriptionHarness, serverSubscriptionId: number) {
            ...
            teardownListener(...args: Parameters<typeof connection.removeAccountChangeListener>) {
                return connection.removeAccountChangeListener(...args);
            },
        },
        blockSubscribe: {
            getExpectedCommitment: () =>
                connection.commitment === 'confirmed' || connection.commitment === 'finalized'
                    ? connection.commitment
                    : 'confirmed',
            getExpectedAlternateParams: () => [
                {
                    mentionsAccountOrProgram: 'C2jDL4pcwpE2pP5EryTGn842JJUJTcurPGZUquQjySxK',
                },
                {
                    commitment:
                        connection.commitment === 'confirmed' || connection.commitment === 'finalized'
            ...
            setupAlternateListener(callback: BlockSubscriptionCallback): number {
                return connection.onBlock(new PublicKey('C2jDL4pcwpE2pP5EryTGn842JJUJTcurPGZUquQjySxK'), callback);
            },
            ...
            teardownListener(...args: Parameters<typeof connection.removeBlockListener>) {
        ...
        logsSubscribe: {
            ...
            setupAlternateListener(callback: LogsCallback): number {
                return connection.onLogs(new PublicKey('C2jDL4pcwpE2pP5EryTGn842JJUJTcurPGZUquQjySxK'), callback);
            },
            ...
            setupListenerWithDefaultableParamsSetToTheirDefaults(callback: LogsCallback): number {
                return connection.onLogs(PublicKey.default, callback, connection.commitment || 'confirmed');
            },
            publishNotificationForServerSubscriptionId(harness: SubscriptionHarness, serverSubscriptionId: number) {
                emitHarnessEvent(harness, 'logsNotification', {
                    ...
                    result: {
                        ...
                        value: {
                            err: null,
                            logs: ['SBF program 83astBRguLMdt2h5U1Tpdq5tjFoJ6noeGwaY3mDLVcri success'],
                            signature:
            ...
            teardownListener(...args: Parameters<typeof connection.removeOnLogsListener>) {
        ...
        programSubscribe: {
            ...
            setupListenerWithDefaultsOmitted(callback: ProgramAccountChangeCallback): number {
                return connection.onProgramAccountChange(PublicKey.default, callback);
            },
            setupListenerWithDefaultableParamsSetToTheirDefaults(callback: ProgramAccountChangeCallback): number {
                return connection.onProgramAccountChange(
                    PublicKey.default,
                    callback,
            ...
            teardownListener(...args: Parameters<typeof connection.removeProgramAccountChangeListener>) {
        ...
        rootSubscribe: {
            ...
            teardownListener(...args: Parameters<typeof connection.removeRootChangeListener>) {
        ...
        signatureSubscribe: {
            ...
            getExpectedParams: () => [TEST_TRANSACTION_SIGNATURE, { commitment: connection.commitment || 'confirmed' }],
            ...
            setupListenerWithDefaultsOmitted(callback: SignatureResultCallback): number {
                return connection.onSignature(TEST_TRANSACTION_SIGNATURE, callback);
            },
            setupListenerWithDefaultableParamsSetToTheirDefaults(callback: SignatureResultCallback): number {
            ...
            teardownListener(...args: Parameters<typeof connection.removeSignatureListener>) {
        ...
        slotSubscribe: {
            ...
            teardownListener(...args: Parameters<typeof connection.removeSlotChangeListener>) {
        ...
        slotsUpdatesSubscribe: {
            ...
            teardownListener(...args: Parameters<typeof connection.removeSlotUpdateListener>) {
        ...
        voteSubscribe: {
            ...
            teardownListener(...args: Parameters<typeof connection.removeVoteListener>) {
    ...
    it('rejects if asked to await an inactive subscription', async () => {
        try {
            ...
            expect((error as Error).message).to.equal('Subscription with id `123456` is no longer active.');
        }
    });
    ...
    Object.entries(subscriptionMethodsConfig).forEach(
        ([
            ...
            const getExpectedSpec = () => createSubscriptionSpec(subscriptionMethod, getExpectedParams());
            const getExpectedAlternateSpec = () =>
                createSubscriptionSpec(subscriptionMethod, getExpectedAlternateParams());
            ...
            describe(`The \`${subscriptionMethod}\` RPC method`, () => {
                describe('attaching the first notification listener', () => {
                    ...
                    let fatalSubscription = (): void => {
                        expect.fail('Expected a function to have been assigned to `fatalSubscription` in the test.');
                    };
                    const serverSubscriptionId = 0;
                    beforeEach(() => {
                        stubbedHarness.requestSubscription.withArgs(getExpectedSpec()).callsFake(
                            () =>
                    ...
                    describe('when awaiting subscription readiness', () => {
                        it('resolves once the subscription is acknowledged by the server', async () => {
                            const readinessPromise = connection.awaitSubscriptionReady(clientSubscriptionId);
                        ...
                        it('rejects if the wait is aborted', async () => {
                            ...
                            try {
                                ...
                                expect((error as Error).message).to.equal('subscription wait aborted');
                            }
                        });
                        ...
                        it('rejects if that listener is removed before acknowledgement even when another listener keeps the shared subscription active', async () => {
                            ...
                            const firstReadinessPromise = connection.awaitSubscriptionReady(clientSubscriptionId);
                            const secondReadinessPromise =
                                connection.awaitSubscriptionReady(secondClientSubscriptionId);
                            ...
                            try {
                                ...
                                expect.fail('Expected the first subscription readiness to reject.');
                            } catch (error) {
                                expect((error as Error).message).to.equal(
                                    `Subscription with id \`${clientSubscriptionId}\` is no longer active.`,
                                );
                    ...
                    describe('once the subscription has been acknowledged by the server', () => {
                        ...
                        describe('then unsubscribing that listener', () => {
                            ...
                            beforeEach(() => {
                                stubbedHarness.unsubscribe.resetHistory();
                                stubbedHarness.unsubscribe.withArgs(serverSubscriptionId).callsFake(
                                    () =>
                                        // Defer the acknowledgement.
                                ...
                                void teardownListener(clientSubscriptionId);
                            });
                            ...
                            describe('then having the socket connection error', () => {
                                beforeEach(() => {
                                    stubbedHarness.emit('error', new Error('A bad thing happened to the socket'));
                                });
                                describe('making another subscription while disconnected', () => {
                                    ...
                                    it('does not issue an RPC call', () => {
                                        expect(stubbedHarness.requestSubscription).not.to.have.been.called;
                                    });
                                });
                            });
                        ...
                        if (setupAlternateListener) {
                            describe('attaching a second notification listener with different params', () => {
                                ...
                                it('results in a second subscription request being made to the RPC', () => {
                                    expect(stubbedHarness.requestSubscription).to.have.been.calledWithExactly(
    ...
    describe('block notification payload mapping', () => {
        ...
        it('maps jsonParsed full block notifications into parsed transaction payloads', async () => {
            let acknowledgeSubscription = (_serverSubscriptionId: number): void => {
                expect.fail('Expected a function to have been assigned to `acknowledgeSubscription` in the test.');
            };
            const callback = spy();
            ...
            const expectedSpec = createSubscriptionSpec('blockSubscribe', expectedParams);
            ...
            expect(result.block.transactions[0].transaction.message.accountKeys[0].pubkey).to.eql(
                '7v5fMKBqC9PuwjSdS9k9JU7efEXmq3bHTMF5fuSHnqrm',
            );
            expect(result.block.transactions[0].transaction.message.instructions[0].programId).to.eql(
                'Vote111111111111111111111111111111111111111',
            );
            expect(result.block.transactions[0].meta).to.not.have.property('loadedAddresses');
        });
        ...
        it('maps base64 full block notifications into wire transaction payloads', async () => {
            ...
            expect(result.block.transactions[0].transaction).to.eql(['AQID', 'base64']);
    ...
    describe('account notification payload mapping', () => {
        ...
        it('maps jsonParsed account notifications into parsed account data payloads', async () => {
            ...
            const expectedSpec = createSubscriptionSpec('accountSubscribe', expectedParams);
        ...
        it('maps jsonParsed program notifications into parsed account data payloads', async () => {
            ...
            const expectedSpec = createSubscriptionSpec('programSubscribe', expectedParams);
    ...
    describe('subscription request shaping', () => {
        it('passes base64+zstd account subscription config through the websocket RPC', () => {
            ...
            expect(stubbedHarness.requestSubscription).to.have.been.calledOnceWithExactly(expectedSpec);
        });
        ...
        it('passes deprecated program subscription filters through the websocket RPC', () => {
            ...
            const expectedParams = [
                PublicKey.default.toBase58(),
                {
                    commitment: 'confirmed',
                    encoding: 'base64',
                    filters: [{ dataSize: 123 }, { memcmp: { bytes: 'AAA', encoding: 'base58', offset: 1 } }],
                },
            ];
            const expectedSpec = createSubscriptionSpec('programSubscribe', expectedParams);
            ...
            connection.onProgramAccountChange(PublicKey.default, callback, undefined, [
                { dataSize: 123 },
    ...
    describe('auto-disposing subscriptions', () => {
        ...
        const getExpectedParams = () => [testSignature, { commitment: connection.commitment || 'confirmed' }];
        ...
        const expectedSpec = () => createSubscriptionSpec('signatureSubscribe', getExpectedParams());
        beforeEach(() => {
            stubbedHarness.requestSubscription.withArgs(expectedSpec()).resolves(serverSubscriptionId);
        ...
        describe('before an auto-disposing subscription has published any notification', () => {
            describe('then unsubscribing the listener', () => {
                ...
                it('results in an unsubscribe request being made to the RPC', () => {
                    expect(stubbedHarness.unsubscribe).to.have.been.calledWith(serverSubscriptionId);
                });
            });
        });
        describe('after an auto-disposing subscription has published a non-final notification', () => {
            ...
            it('should not result in an unsubscribe request being made to the RPC', () => {
                expect(stubbedHarness.unsubscribe).not.to.have.been.calledWith(serverSubscriptionId);
            });
            describe('then unsubscribing the listener', () => {
                beforeEach(async () => {
                    stubbedHarness.unsubscribe.resetHistory();
        ...
        describe('after an auto-disposing subscription has published its final notification', () => {
            ...
            describe('then unsubscribing the listener', () => {
                ...
                it('releases the local server subscription handle', () => {
                    expect(
                        getSubscriptionRegistry(connection).hasServerSubscription(FIRST_LOCAL_SERVER_SUBSCRIPTION_ID),
                    ).to.be.false;
                });
            });
        });
    ...
    describe('signature subscriptions whose terminal callback throws', () => {
        ...
        [
            {
                name: 'onSignature',
                register: (callback: () => void) => connection.onSignature(testSignature, callback),
                expectedParams: () => [testSignature, { commitment: connection.commitment || 'confirmed' }],
            },
            {
            ...
            describe(`registered via \`${name}\``, () => {
                ...
                beforeEach(async () => {
                    stubbedHarness.requestSubscription
                        .withArgs(createSubscriptionSpec('signatureSubscribe', expectedParams()))
    ...
    describe('signature subscriptions with received notifications enabled', () => {
        ...
        describe('when a received notification is published', () => {
            ...
            it('fires the listener callback with the received notification', () => {
                expect(listenerCallback).to.have.been.calledOnceWithExactly({ type: 'received' }, { slot: 11n });
            });
        ...
        describe(`given a Connection with ${
            ...
            Object.entries(subscriptionMethodsConfig).forEach(
                ([
                    ...
                    beforeEach(() => {
                        const stubbedConnection = stubSubscriptionHarness(url, maybeOverrideCommitment);
                        connection = stubbedConnection.connection;
                        stubbedHarness = stubbedConnection.harness;
                    });
                    afterEach(async () => {
                    ...
                    if (setupListenerWithDefaultsOmitted && setupListenerWithDefaultableParamsSetToTheirDefaults) {
    ...
    describe('during state machine updates', () => {
        ...
        it('the processor always operates over the most up-to-date state of a given subscription', () => {
            ...
            connection.onAccountChange(new PublicKey('27Y78XJXG9A13pnPajrB1VYU6EF8uNSoojPZBmhKsi8C'), () => {});
            // Then remove the first one before the connection opens.
            void connection.removeAccountChangeListener(clientSubscriptionIdA);
...
if (process.env.TEST_LIVE === undefined) {
    describe('Subscription default commitment', () => {
        ...
        const ADDRESS = new PublicKey('7A6PCsp5EQFHsUpnvLmLvNjWvYSNJGCLYtpvzL1shV4o');
        ...
        afterEach(async () => {
            await connection.removeAccountChangeListener(subscriptionId).catch(() => {});
            await server.stop();
            await new Promise<void>(resolve => setImmediate(resolve));
            consoleErrorStub.restore();
        });
