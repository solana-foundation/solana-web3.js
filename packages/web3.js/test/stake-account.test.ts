import { getStakeStateAccountCodec, stakeStateV2, type StakeStateAccountArgs } from '@solana-program/stake';
import { expect } from 'chai';

import { PublicKey, StakeAccount } from '../src';

type InitializedStakeStateArgs = Extract<StakeStateAccountArgs['state'], { __kind: 'Initialized' }>;

type DelegatedStakeStateArgs = Extract<StakeStateAccountArgs['state'], { __kind: 'Stake' }>;
...
describe('StakeAccount', () => {
    it('decodes initialized stake account state', () => {
        ...
        expect(account.state.meta.authorized.withdrawer.equals(withdrawer)).to.eq(true);
    ...
    it('decodes delegated stake account state with bigint fields and flags', () => {
        ...
        const account = StakeAccount.fromAccountData(paddedBytes.subarray(1, paddedBytes.length - 1));
        ...
        expect(account.state.stake.delegation.voterPubkey.equals(voterPubkey)).to.eq(true);
        ...
        expect(account.state.stake.delegation.reserved).to.deep.eq([1, 2, 3, 4, 5, 6, 7, 8]);
