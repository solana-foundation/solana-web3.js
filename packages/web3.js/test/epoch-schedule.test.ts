...
describe('EpochSchedule', () => {
    it('slot methods work', () => {
        ...
        expect(epochSchedule.getEpoch(firstNormalSlot + 3n * slotsPerEpoch + 12345n)).to.be.equal(17n);
        expect(epochSchedule.getEpochAndSlotIndex(firstNormalSlot + 3n * slotsPerEpoch + 12345n)).to.be.eql([
            17n,
            12_345n,
        ]);
        ...
        expect(epochSchedule.getFirstSlotInEpoch(16n)).to.be.equal(firstNormalSlot + 2n * slotsPerEpoch);
        expect(epochSchedule.getLastSlotInEpoch(16n)).to.be.equal(firstNormalSlot + 3n * slotsPerEpoch - 1n);
    });
});
