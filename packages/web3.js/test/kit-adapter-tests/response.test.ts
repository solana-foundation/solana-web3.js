...
describe('Kit response adapters', () => {
    ...
    it('should reject unsupported transaction versions instead of mapping them to legacy messages', () => {
        ...
        // Act & Assert
        expect(() => mapTypedTransactionResponse(response)).to.throw('Unsupported transaction version: 2');
        expect(() =>
            mapTypedParsedTransactionResponse(
                response as unknown as Parameters<typeof mapTypedParsedTransactionResponse>[0],
            ),
    ...
    it('should preserve stack heights in parsed transaction paths', () => {
        ...
        // Assert
        expect(transaction.transaction.message.instructions.map(ix => ix.stackHeight)).to.deep.equal([2, 3]);
        expect(transaction.meta?.innerInstructions?.[0].instructions.map(ix => ix.stackHeight)).to.deep.equal([2, 3]);
        expect(simulation.innerInstructions?.[0].instructions.map(ix => ix.stackHeight)).to.deep.equal([2, 3]);
    });
    it('should map simulation fee, balances, token balances, and loaded addresses', () => {
        ...
        expect(simulation.loadedAddresses?.writable.map(address => address.toBase58())).to.deep.equal([ADDRESS]);
    });
    ...
    it('should omit metadata fields that the RPC response omits', () => {
        // Arrange
        const sparseMeta: Partial<ReturnType<typeof createMeta>> = createMeta();
        delete sparseMeta.postTokenBalances;
        delete sparseMeta.preTokenBalances;
        delete sparseMeta.returnData;
        delete sparseMeta.rewards;
        const instruction = {
            accounts: [ADDRESS],
            data: '',
            programId: ADDRESS,
        ...
        // Assert
        expect(result.meta).to.not.have.any.keys('postTokenBalances', 'preTokenBalances', 'returnData', 'rewards');
        expect(result.transaction.message.instructions[0]).to.not.have.property('stackHeight');
    });
});
