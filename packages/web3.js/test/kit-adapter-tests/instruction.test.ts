...
import { fromKitInstruction, toKitInstruction } from '../../src/kit-adapters/instruction';
...
function toWeb3JsByteArrayAppropriateForPlatform(data: Uint8Array) {
    return typeof Buffer !== 'undefined' ? Buffer.from(data) : (new Uint8Array(data) as Buffer);
}
...
describe('toKitInstruction', () => {
    ...
    keyConversionCases.forEach(({ isSigner, isWritable, expected }) => {
        it(`converts keys with isSigner: ${isSigner}, isWritable: ${isWritable} to ${expected}`, () => {
            ...
            expect(converted.accounts.some(account => account.role === expected)).to.be.true;
        });
    });
});
...
describe('fromKitInstruction', () => {
    it('converts a Kit instruction to a TransactionInstruction', () => {
        ...
        expect(converted.programId.toBase58()).to.eq('11111111111111111111111111111111');
        ...
        expect(converted.keys[0].pubkey.toBase58()).to.eq('7EqQdEULxWcraVx3mXKFjc84LhCkMGZCkRuDpvcMwJeK');
    ...
    it('roundtrips with toKitInstruction', () => {
        ...
        for (let index = 0; index < original.keys.length; index += 1) {
            expect(roundtripped.keys[index].pubkey.equals(original.keys[index].pubkey)).to.be.true;
            expect(roundtripped.keys[index].isSigner).to.eq(original.keys[index].isSigner);
            expect(roundtripped.keys[index].isWritable).to.eq(original.keys[index].isWritable);
        }
    ...
    it('preserves signer and writable roles from Codama client with noopSigner', async () => {
        ...
        expect(instruction.programId.toBase58()).to.equal('11111111111111111111111111111111');
...
describe('isKitInstruction', () => {
    [AccountRole.READONLY, AccountRole.WRITABLE, AccountRole.READONLY_SIGNER, AccountRole.WRITABLE_SIGNER].forEach(
        role => {
...
describe('Transaction.add() with Kit instructions', () => {
    it('accepts a raw Kit instruction via add()', () => {
        ...
        expect(transaction.instructions[0]).to.be.instanceOf(TransactionInstruction);
        expect(transaction.instructions[0].programId.toBase58()).to.eq('11111111111111111111111111111111');
        ...
        expect(transaction.instructions[0].data).to.deep.equal(new Uint8Array([1, 2, 3]));
    });
    ...
    it('normalizes a raw Kit instruction the same way as fromKitInstruction()', () => {
        ...
        const viaTransactionAdd = new Transaction().add(kitInstruction).instructions[0];

        expect(viaTransactionAdd.programId.equals(directConversion.programId)).to.be.true;
    ...
    it('mixes converted Kit and Web3.js instructions in a single add()', () => {
        ...
        expect(transaction.instructions[0].data).to.deep.equal(new Uint8Array([10]));
        expect(transaction.instructions[1].data).to.deep.equal(new Uint8Array([20]));
    });
});
...
describe('ambiguous dual-shaped instructions', () => {
    ...
    it('Transaction.add() rejects an object carrying both Kit and legacy fields', () => {
        expect(() => new Transaction().add(dualInstruction() as unknown as TransactionInstruction)).to.throw(
    ...
    it('still accepts a legacy instruction and a Kit instruction for the same transfer', () => {
        ...
        const transaction = new Transaction().add(new TransactionInstruction({ programId, keys, data }), {
            programAddress,
            accounts,
            data,
        });
        ...
        expect(transaction.instructions).to.have.length(2);
        expect(SystemInstruction.decodeTransfer(transaction.instructions[0]).toPubkey).to.deep.equal(safeDestination);
        expect(SystemInstruction.decodeTransfer(transaction.instructions[1]).toPubkey).to.deep.equal(
            attackerDestination,
        );
    });
});
