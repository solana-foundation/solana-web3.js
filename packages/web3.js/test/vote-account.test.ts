...
const loadVoteAccountFixture = (): {
    ...
    const fixtureUrl = new URL('./fixtures/vote-account-v1_14_11.json', import.meta.url);
    const fixture = JSON.parse(readFileSync(fixtureUrl, 'utf8')) as VoteAccountFixture;
...
const prependVoteStateVersion = (encoded: Uint8Array): Uint8Array => {
    ...
    new DataView(bytes.buffer, bytes.byteOffset, 4).setUint32(0, VoteStateVersion.V1_14_11, true);
    bytes.set(encoded, 4);
    return bytes;
};
...
const prependVoteStateVersionFor = (version: VoteStateVersion, encoded: Uint8Array): Uint8Array => {
    const bytes = new Uint8Array(4 + encoded.length);
...
const buildVoteAccountDataWithPriorVoterIndex = (idx: bigint, isEmpty = 0): VoteAccountData => {
...
describe('VoteAccount', () => {
    it('decodes vote account data from a byte fixture (V1_14_11)', () => {
        ...
        for (const annotation of fixture.annotations) {
            const slice = bytes.subarray(annotation.offset, annotation.offset + annotation.length);
            expect(toHex(slice)).to.eq(annotation.hex);
        }

        const account = VoteAccount.fromAccountData(bytes);

        expect(account.nodePubkey.equals(new PublicKey(buildKeyBytes(1)))).to.eq(true);
        expect(account.authorizedWithdrawer.equals(new PublicKey(buildKeyBytes(101)))).to.eq(true);
        ...
        expect(account.authorizedVoters[0].authorizedVoter.equals(new PublicKey(buildKeyBytes(201)))).to.eq(true);
        expect(account.authorizedVoters[1].epoch).to.eq(9n);
        expect(account.authorizedVoters[1].authorizedVoter.equals(new PublicKey(buildKeyBytes(202)))).to.eq(true);
        ...
        expect(firstPriorVoter.authorizedPubkey.equals(new PublicKey(buildKeyBytes(56)))).to.eq(true);
        expect(lastPriorVoter.epochOfLastAuthorizedSwitch).to.eq(5n);
        expect(lastPriorVoter.targetEpoch).to.eq(1005n);
        expect(lastPriorVoter.authorizedPubkey.equals(new PublicKey(buildKeyBytes(55)))).to.eq(true);
    ...
    it('decodes vote account data with variable-length fields', () => {
        ...
        expect(account.nodePubkey.equals(new PublicKey(data.nodePubkey))).to.eq(true);
        expect(account.authorizedWithdrawer.equals(new PublicKey(data.authorizedWithdrawer))).to.eq(true);
        ...
        expect(
            account.authorizedVoters[0].authorizedVoter.equals(new PublicKey(data.authorizedVoters[0].authorizedVoter)),
        ).to.eq(true);
        expect(account.priorVoters).to.have.length(32);
        expect(account.priorVoters[0].epochOfLastAuthorizedSwitch).to.eq(2n);
        expect(account.priorVoters[account.priorVoters.length - 1].epochOfLastAuthorizedSwitch).to.eq(1n);
        expect(account.epochCredits).to.deep.eq([{ epoch: 1n, credits: 2n, prevCredits: 0n }]);
    ...
    it('decodes from a sliced Uint8Array view', () => {
        ...
        const account = VoteAccount.fromAccountData(padded.subarray(1, buffer.length + 1));
    ...
    it('normalizes prior voters by wrapping entries after idx to the front', () => {
        const account = VoteAccount.fromAccountData(buildVoteAccountBytes(buildVoteAccountDataWithPriorVoterIndex(5n)));
    ...
    it('normalizes prior voters correctly when idx is 0', () => {
        const account = VoteAccount.fromAccountData(buildVoteAccountBytes(buildVoteAccountDataWithPriorVoterIndex(0n)));
    ...
    it('rejects prior voters idx values outside the circular buffer range', () => {
        expect(() =>
            VoteAccount.fromAccountData(buildVoteAccountBytes(buildVoteAccountDataWithPriorVoterIndex(32n))),
    ...
    it('decodes V3 vote account data', () => {
        ...
        expect(account.nodePubkey.equals(new PublicKey(buildKeyBytes(20)))).to.eq(true);
        expect(account.authorizedWithdrawer.equals(new PublicKey(buildKeyBytes(21)))).to.eq(true);
        ...
        expect(account.priorVoters[account.priorVoters.length - 1].epochOfLastAuthorizedSwitch).to.eq(3n);
        expect(account.epochCredits).to.deep.eq([{ epoch: 2n, credits: 9n, prevCredits: 4n }]);
    ...
    it('decodes V4 vote account data', () => {
        ...
        expect(account.nodePubkey.equals(new PublicKey(buildKeyBytes(30)))).to.eq(true);
        expect(account.authorizedWithdrawer.equals(new PublicKey(buildKeyBytes(31)))).to.eq(true);
        expect(account.inflationRewardsCollector?.equals(new PublicKey(buildKeyBytes(32)))).to.eq(true);
        expect(account.blockRevenueCollector?.equals(new PublicKey(buildKeyBytes(33)))).to.eq(true);
        expect(account.commission).to.eq(12);
        ...
        expect(account.blsPubkeyCompressed).to.deep.eq(Uint8Array.from({ length: 48 }, (_, i) => 100 + i));
        expect(account.votes).to.deep.eq([
        ...
        expect(account.priorVoters).to.deep.eq([]);
        expect(account.epochCredits).to.deep.eq([{ epoch: 3n, credits: 11n, prevCredits: 6n }]);
