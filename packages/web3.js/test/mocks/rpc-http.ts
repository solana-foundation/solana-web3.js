...
import { Connection, PublicKey, Transaction, Signer, VersionedMessage } from '../../src';
import type { Commitment, HttpHeaders, RpcParams, SignatureResult } from '../../src/connection';
import invariant from '../../src/utils/assert';
import { createSignatureStatusRpcResult, mockRpcMessage } from './rpc-subscriptions';
...
function toJsonRpcWireValue(value: unknown): unknown {
    ...
    if (value && typeof value === 'object') {
        return Object.fromEntries(Object.entries(value).map(([key, entry]) => [key, toJsonRpcWireValue(entry)]));
    }
...
const latestBlockhash = async ({ connection, commitment }: { connection: Connection; commitment?: Commitment }) => {
    ...
    const params: Array<object> = commitment === 'finalized' ? [] : [{ commitment: commitment ?? 'confirmed' }];
...
const processTransaction = async ({
    ...
    await mockRpcMessage({
        ...
        result: createSignatureStatusRpcResult((err ?? null) as SignatureResult['err']),
    });

    return await connection.confirmTransaction({ blockhash, lastValidBlockHeight, signature }, commitment);
};
