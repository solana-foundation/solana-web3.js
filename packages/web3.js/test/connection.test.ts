...
import { blockhash, createJsonRpcApi, createRpc, getBase58Codec, type Blockhash } from '@solana/kit';
import { generateKeyPairSigner, type TransactionPartialSigner } from '@solana/signers';
...
import { helpers, mockErrorResponse, mockRpcResponse, mockServer } from './mocks/rpc-http';
import {
...
const SAMPLE_BLOCKHASH = blockhash('EkSnNWidA2rMT4wAhyLQ6UxJ2yR6b6bJ7hVn6XK7rxJQ');
...
const LEGACY_TOKEN_TEST_MINT_PUBKEY = new PublicKey('7MbpdfJa5xqwexkp6WUvkYHTPo4VgxYACDBNFWYLwCdo');
...
const LEGACY_TOKEN_TEST_OWNER_SECRET_KEY =
    ...
    new Uint8Array([
        153, 120, 247, 45, 160, 119, 144, 219, 220, 209, 73, 91, 210, 102, 31, 136, 155, 12, 68, 27, 226, 215, 61, 214,
        10, 245, 247, 180, 236, 63, 100, 202, 140, 247, 112, 54, 120, 32, 168, 118, 72, 115, 190, 34, 171, 126, 15, 119,
        252, 173, 50, 173, 8, 10, 96, 239, 21, 32, 94, 67, 37, 43, 145, 249,
    ]);

// See scripts/fixtures/legacy-token-test-token-account.json
const LEGACY_TOKEN_TEST_ACCOUNT_PUBKEY = new PublicKey('EryTMgfSEabo5Fc7dN5z3nBQKzfHUJRpHAMnXdCrTq4S');
...
async function waitForSlot(this: Mocha.Context, connection: Connection, minSlot: number = 0): Promise<void> {
...
const verifySignatureStatus = (status: SignatureStatus | null, err?: unknown): SignatureStatus => {
    if (status === null) {
...
describe('Connection', function () {
    ...
    if (mockServer) {
        ...
        it('exposes framework-agnostic RPC configuration', async () => {
            ...
            const rpc = createRpc({
                ...
                transport: async ({ payload, signal }) => {
                    ...
                    for (const [header, value] of Object.entries(connection.rpcHttpHeaders ?? {})) {
            ...
            if (healthResponse && typeof healthResponse === 'object' && 'result' in healthResponse) {
        ...
        it('should attribute middleware fatals to the middleware', async () => {
            ...
            expect(error).to.be.an.instanceOf(Error).and.to.have.property('stack').that.include('fetchMiddleware');
        });
    ...
    it('get account info - not found', async () => {
        ...
        const parsedAccountInfo = await connection.getParsedAccountInfo(account.publicKey);
    ...
    if (!process.env.TEST_LIVE) {
        it('get address lookup table - rejects accounts not owned by the lookup table program', async () => {
            ...
            await expect(connection.getAddressLookupTable(account.publicKey)).to.be.rejectedWith(
                /not owned by the Address Lookup Table program/,
            );
        });
        ...
        it('get address lookup table - rejects lookup-table-owned accounts with an invalid discriminator', async () => {
            ...
            await expect(connection.getAddressLookupTable(account.publicKey)).to.be.rejectedWith(/discriminator/);
        });
    }
    ...
    it('get account info with config object', async () => {
        ...
        if (process.env.TEST_LIVE) {
            ...
            const lamports = await connection.getMinimumBalanceForRentExemption(NONCE_ACCOUNT_LENGTH, 'confirmed');
            ...
            const accountInfoAndContext = await connection.getAccountInfoAndContext(account.publicKey, {
                commitment: 'confirmed',
                dataSlice,
                minContextSlot: Number(minContextSlot),
            });
    ...
    it('get multiple accounts info', async () => {
        ...
        await mockRpcResponse({
            ...
            params: [[account1.publicKey.toBase58(), account2.publicKey.toBase58()], { encoding: 'base64' }],
            preserveBigIntJsonValues: true,
            value: value,
            withContext: true,
        });

        const responseWithContext = await connection.getMultipleAccountsInfoAndContext(
        ...
        const res = await connection.getMultipleAccountsInfo([account1.publicKey, account2.publicKey], 'confirmed');
        ...
        if (process.env.TEST_LIVE) {
            ...
            for (const accountInfo of res) {
                ...
                expect(accountInfo.owner).to.eql(new PublicKey('11111111111111111111111111111111'));
                expect(accountInfo.lamports).to.eq(BigInt(LAMPORTS_PER_SOL));
    ...
    it('get program accounts', async () => {
        ...
        const fee = Number((await helpers.getFeeForMessage({ connection, message })).value ?? 0n);
...

        {
            await mockRpcResponse({
                ...
                params: [programId.publicKey.toBase58(), { commitment: 'confirmed', encoding: 'base64' }],
                value: [
                    {
                        account: {
                            data: ['', 'base64'],
            ...
            const programAccounts = await connection.getProgramAccounts(programId.publicKey, {
                commitment: 'confirmed',
            });
            ...
            programAccounts.forEach(function (keyedAccount) {
                ...
                if (keyedAccount.pubkey.equals(account0.publicKey)) {
                    expect(keyedAccount.account.lamports).to.eq(BigInt(LAMPORTS_PER_SOL - fee));
                } else {
                    expect(keyedAccount.pubkey).to.eql(account1.publicKey);
                    expect(keyedAccount.account.lamports).to.eq(BigInt(LAMPORTS_PER_SOL / 2 - fee));
                }
            });
        }

        {
            ...
            const programAccounts = await connection.getProgramAccounts(programId.publicKey, 'confirmed');
            expect(programAccounts).to.have.length(2);
            programAccounts.forEach(function (keyedAccount) {
                if (keyedAccount.pubkey.equals(account0.publicKey)) {
                    expect(keyedAccount.account.lamports).to.eq(BigInt(LAMPORTS_PER_SOL - fee));
...

        {
            ...
            const programAccountsDoMatchFilter = await connection.getProgramAccounts(programId.publicKey, {
                commitment: 'confirmed',
                encoding: 'base64',
                filters: [{ dataSize: 0 }],
            });
...

        {
            ...
            const programAccountsDontMatchFilter = await connection.getProgramAccounts(programId.publicKey, {
                commitment: 'confirmed',
                filters: [
                    {
                        memcmp: {
...

        {
            await mockRpcResponse({
                method: 'getProgramAccounts',
                params: [programId.publicKey.toBase58(), { commitment: 'confirmed', encoding: 'jsonParsed' }],
                value: [
                    {
                        account: {
                            data: ['', 'base64'],
            ...
            const programAccounts = await connection.getParsedProgramAccounts(programId.publicKey, {
                commitment: 'confirmed',
            });
            expect(programAccounts).to.have.length(2);
            ...
            programAccounts.forEach(function (element) {
                if (element.pubkey.equals(account0.publicKey)) {
                    expect(element.account.lamports).to.eq(BigInt(LAMPORTS_PER_SOL - fee));
                } else {
                    expect(element.pubkey).to.eql(account1.publicKey);
                    expect(element.account.lamports).to.eq(BigInt(LAMPORTS_PER_SOL / 2 - fee));
                }
            });
        }

        {
            ...
            const programAccountsDontMatchFilter = await connection.getParsedProgramAccounts(programId.publicKey, {
                commitment: 'confirmed',
                filters: [{ dataSize: 2 }],
            });
            expect(programAccountsDontMatchFilter).to.have.length(0);
...

        {
            ...
            const programAccountsDoMatchFilter = await connection.getParsedProgramAccounts(programId.publicKey, {
                commitment: 'confirmed',
                filters: [
                    {
                        memcmp: {
...

        {
            ...
            const programAccountsWithContext = await connection.getProgramAccounts(programId.publicKey, {
                commitment: 'confirmed',
                withContext: true,
            });
    ...
    it('get token largest accounts', async () => {
        if (mockServer) {
            ...
            expect(largestAccounts.value[0].address.toBase58()).to.eq(tokenAccount.toBase58());
        } else {
    ...
    it('get token largest accounts with config object', async () => {
        if (mockServer) {
            ...
            const largestAccountsWithDefaultCommitment = await connectionWithCommitment.getTokenLargestAccounts(mint);

            expect(largestAccountsWithDefaultCommitment.value).to.have.length(1);
            expect(largestAccountsWithDefaultCommitment.value[0].address.toBase58()).to.eq(tokenAccount.toBase58());
        } else {
            const largestAccounts = await connection.getTokenLargestAccounts(
                new PublicKey('7MbpdfJa5xqwexkp6WUvkYHTPo4VgxYACDBNFWYLwCdo'),
                {
    ...
    it('get inflation reward', async () => {
        if (mockServer) {
            await mockRpcResponse({
                ...
                params: [
                    ['7GHnTRB8Rz14qZQhDXf8ox1Kfu7mPcPLpKaBJJirmYj2', 'CrinLuHjVGDDcQfrEoCmM4k31Ni9sMoTCEEvNSUSh7Jg'],
                    {
                        epoch: 0,
                    },
                ],
    ...
    it('get leader schedule keyed by vote account', async () => {
        if (mockServer) {
            ...
            await mockRpcResponse({
                method: 'getLeaderSchedule',
                params: [123, { commitment: 'confirmed', identity, keyByVoteAccount: true }],
                value: {
                    [voteAccount]: [0, 1, 2, 3],
                },
            });
    ...
    it('request airdrop without config object', async () => {
        ...
        if (mockServer) {
            ...
            const result = await connection.requestAirdrop(recipient, LAMPORTS_PER_SOL);
    ...
    it('request airdrop with config object', async () => {
        ...
        if (mockServer) {
            ...
            await mockRpcResponse({
                method: 'requestAirdrop',
                params: [recipient.toBase58(), LAMPORTS_PER_SOL, { commitment: 'confirmed' }],
                value: signature,
            });

            const result = await connection.requestAirdrop(recipient, LAMPORTS_PER_SOL, { commitment: 'confirmed' });
    ...
    if (process.env.TEST_LIVE) {
        it('get vote accounts', async () => {
            const voteAccounts = await connection.getVoteAccounts();
            expect(voteAccounts.current.concat(voteAccounts.delinquent).length).to.be.greaterThan(0);
        });
    }

    if (process.env.TEST_LIVE) {
        describe('transaction sending error logs', () => {
            async function expectLiveSendFailureLogsOrStatus(error: unknown, connection: Connection): Promise<void> {
                ...
                try {
                    ...
                    expect(logs.some(log => log.includes('Transfer: insufficient lamports'))).to.eq(true);
                    expect(
                        logs.some(log =>
            ...
            it('sendAndConfirmTransaction skipPreflight: true', async () => {
                ...
                try {
                    await sendAndConfirmTransaction(connection, transferSolTransaction, [keypair], confirmOptions);
            ...
            it('Simulate transaction contains logs', async () => {
                ...
                const simulationResultPromise = connection.simulateTransaction(transferSolTransaction, [keypair]);
    ...
    it('sendAndConfirmRawTransaction accepts Uint8Array inputs', async () => {
        ...
        const sendRawTransactionStub = stub(connection, 'sendRawTransaction').resolves(signature);
        ...
        try {
            ...
            expect(confirmTransactionStub).to.have.been.calledOnceWithExactly(confirmationStrategy, 'confirmed');
        } finally {
    ...
    it('sendAndConfirmRawTransaction accepts Array<number> inputs', async () => {
        ...
        try {
            const result = await sendAndConfirmRawTransaction(connection, rawTransaction, options);
            ...
            expect(confirmTransactionStub).to.have.been.calledOnceWithExactly(signature, 'finalized');
        } finally {
            sendRawTransactionStub.restore();
            confirmTransactionStub.restore();
        }
    ...
    it('sendAndConfirmRawTransaction forwards maxRetries', async () => {
        ...
        try {
            await sendAndConfirmRawTransaction(connection, rawTransaction, confirmationStrategy, options);
    ...
    it('sendAndConfirmRawTransaction confirms with a blockheight strategy whose properties are inherited', async () => {
        ...
        try {
            await sendAndConfirmRawTransaction(connection, rawTransaction, confirmationStrategy);
    ...
    it('sendAndConfirmRawTransaction reports a confirmation failure as a send error', async () => {
        ...
        try {
            await expect(sendAndConfirmRawTransaction(connection, rawTransaction, options)).to.be.rejectedWith(
                SendTransactionError,
    ...
    it('sendRawTransaction rejects malformed runtime input', async () => {
        ...
        await expect(connection.sendRawTransaction(malformedRawTransaction)).to.be.rejectedWith(
    ...
    it('simulateTransaction surfaces preflight failure logs', async () => {
        ...
        expect(await simulationError.getLogs(failingConnection)).to.deep.equal(simulationLogs);
    });
        ...
        describe('transaction confirmation (live)', () => {
            ...
            describe('nonce-based transaction confirmation', () => {
                ...
                beforeEach(async function () {
                    ...
                    const [_, blockhash, minimumNonceAccountRentLamports] = await Promise.all([
                        connection.confirmTransaction(
                            await connection.requestAirdrop(keypair.publicKey, LAMPORTS_PER_SOL),
                        ),
                        helpers.latestBlockhash({ connection }),
                    ...
                    const createNonceAccountTransaction = SystemProgram.createNonceAccount({
                    ...
                    const createNonceAccountTransactionSignature = await connection.sendTransaction(
                    ...
                    const nonceAccount = await connection.getNonce(nonceKeypair.publicKey, { minContextSlot });
                    nonceInfo = {
                    ...
                    invariant(nonceAccount, 'Expected a nonce account to have been created in the test setup');
                    const ix = new TransactionInstruction({
                        keys: [
                            {
                                pubkey: keypair.publicKey,
                ...
                it('throws when confirming using a nonce that is no longer valid', async () => {
                    ...
                    await sendAndConfirmTransaction(
                        connection,
                        new Transaction({ feePayer: keypair.publicKey, ...blockhash }).add(nonceInfo.nonceInstruction),
                        [keypair],
                    );
        ...
        describe('transaction confirmation (mock)', () => {
            ...
            describe('block height strategy', () => {
                ...
                it('rejects upon receiving an abort signal', async () => {
                    ...
                    const fetch = stub().callsFake(async (_url, requestInfo) => {
                        ...
                        return await new Promise<Response>((_, reject) => {
                            ...
                            requestInfo.signal?.addEventListener('abort', () => reject(requestInfo.signal?.reason), {
                                once: true,
                            });
                        });
                    });
                ...
                it('throws a `TransactionExpiredBlockheightExceededError` when the block height advances past the last valid one for this transaction without a signature confirmation', async () => {
                    ...
                    const fetch = stub().callsFake((_url, requestInfo) => {
                        ...
                        if (method === 'getBlockHeight') {
                            ...
                            return new Response(JSON.stringify({ jsonrpc: '2.0', id: '', result: blockHeight }), {
                                headers: { 'content-type': 'application/json' },
                                status: 200,
                            });
                        }
                    ...
                    await clock.tickAsync(1000);
                    await expect(confirmationPromise).to.be.rejectedWith(TransactionExpiredBlockheightExceededError);
                ...
                it('confirms the transaction when the block height is exceeded but a final status check shows it landed', async () => {
                    ...
                    await teardownSubscriptions(connection);
                    const fetchBlockHeights = [lastValidBlockHeight, lastValidBlockHeight + 1];
                ...
                it('does not produce an unhandled rejection when the fallback signature status check fails, and still confirms the transaction', async () => {
                    ...
                    const fetch = stub().callsFake((_url, requestInfo) => {
                        ...
                        if (getSignatureStatusesCallCount === 1) {
                            return Promise.reject(new Error('transient status transport failure'));
                        }
                        return new Response(
                            JSON.stringify({
                                jsonrpc: '2.0',
                ...
                it('rejects with the abort reason when the caller aborts during the final status check after the block height is exceeded', async () => {
                    ...
                    await expect(confirmationPromise).to.be.rejectedWith('aborted by caller');
                });
                ...
                it('when the `getBlockHeight` method throws an error it does not timeout but rather keeps waiting for a confirmation', async () => {
                    ...
                    let resolveResultPromise = function (result: RpcWebSocketSignatureNotificationResult): any {
                        return result;
                    };
                    ...
                    await mockRpcMessage({
                        method: 'signatureSubscribe',
                        params: [mockSignature, { commitment: 'confirmed' }],
                        result: new Promise<RpcWebSocketSignatureNotificationResult>(resolve => {
            ...
            describe('nonce strategy', () => {
                ...
                it('confirms the transaction if the signature confirmation is received before the nonce is advanced', async () => {
                    ...
                    await mockNonceAccountResponse(nonceAccountPubkey.toBase58(), nonceValue, authority.toBase58());
                ...
                it('rejects with the abort reason when the signal is aborted while double-checking the signature after the nonce advanced', async () => {
                    ...
                    await expect(confirmationPromise).to.eventually.be.rejectedWith('cancelled by caller');
                });
                ...
                it('confirms a `nonceInfo`-only transaction sent with `sendAndConfirmTransaction` using the nonce strategy', async () => {
                    ...
                    // Act
                    const sendPromise = sendAndConfirmTransaction(connection, transaction, [payer], {
                        commitment: 'confirmed',
                    });
                    await clock.runToLastAsync();
                ...
                it('rejects malformed nonce information before sending', async () => {
                    ...
                    await expect(sendAndConfirmTransaction(connection, transaction, [payer])).to.be.rejectedWith(
            ...
            it('confirm transaction - does not check the signature status before the signature subscription comes alive', async () => {
                ...
                const getSignatureStatusesExpectation = mock(connection).expects('getSignatureStatuses').never();
                void connection.confirmTransaction(mockSignature);
            ...
            it('confirm transaction - checks the signature status once the signature subscription comes alive', async () => {
                ...
                const getSignatureStatusesExpectation = mock(connection).expects('getSignatureStatuses').once();

                const confirmationPromise = connection.confirmTransaction(mockSignature);
                void clock.runAllAsync();
            ...
            it('confirm transaction - checks the signature status if the signature subscription fails before coming alive', async () => {
                ...
                let rejectSubscriptionSetupFailure!: (_reason: { code: number; message: string }) => void;
                ...
                await expect(confirmationPromise).to.eventually.have.nested.property('value.err', null);
            ...
            it('confirm transaction - does not confirm the transaction when signature status check yields confirmation for a lower commitment before signature subscription confirms the transaction', async () => {
                ...
                await expect(confirmationPromise).to.be.rejectedWith(TransactionExpiredTimeoutError);
            });
        });
    }
    ...
    describe('transaction confirmation', () => {
        it('confirm transaction - error', async () => {
            ...
            await expect(connection.getSignatureStatus(badTransactionSignature)).to.be.rejectedWith(
    ...
    if (mockServer) {
        it('get minimum balance for rent exemption rejects on RPC error', async () => {
            ...
            await expect(connection.getMinimumBalanceForRentExemption(0)).to.be.rejectedWith('rent unavailable');
        });
    }
    ...
    it('get signatures for address', async function () {
        ...
        while (!address || !expectedSignature) {
            ...
            if (block.transactions.length > 0) {
                const { signature, publicKey } = block.transactions[0].transaction.signatures[0];
                if (signature) {
                    address = publicKey;
    ...
    it('get parsed confirmed transactions', async function () {
        ...
        await mockRpcResponse({
            ...
            params: [confirmedTransaction, { commitment: 'confirmed', encoding: 'jsonParsed' }],
            value: parsedConfirmedTransactionResponse,
        });

        let result = await connection.getParsedConfirmedTransactions([confirmedTransaction, confirmedTransaction]);
    ...
    it('get block production', async () => {
        ...
        for (const [key, validatorProduction] of Object.entries(resultIdentityDictionary)) {
        ...
        const resultCommitmentOnly = await connection.getBlockProduction(commitment);
        ...
        for (const [property, validatorProduction] of Object.entries(resultCOIdentityDictionary)) {
        ...
        if (mockServer) {
            ...
            if (resultCOSlotRange.lastSlot !== undefined) {
                ...
                expect(resultCOSlotRange.lastSlot >= resultCOSlotRange.firstSlot).to.eq(true);
            }
        }
    });
    ...
    if (!process.env.TEST_LIVE) {
        describe('v1 transactions', () => {
            const signature = 'w2Zeq8YkpyB463DttvfzARD7k9ZxGEwbsEw4boEK7jDp3pfoxZbTdLFSsEPhzXhpCcjGi2kHtHFobgX49MMhbWt';
            const v1JsonMessage = {
                accountKeys: [
                    'va12u4o9DipLEB2z4fuoHszroq1U9NcAB9aooFDPJSf',
                    '57zQNBZBEiHsCZFqsaY6h176ioXy5MsSLmcvHkEyaLGy',
            ...
            it('getTransaction returns a MessageV1 with its transaction config', async () => {
                await mockRpcResponse({
                    method: 'getTransaction',
                    params: [signature, { commitment: 'confirmed', maxSupportedTransactionVersion: 1 }],
                    value: {
                        blockTime: 1614281964,
                ...
                expect(message.staticAccountKeys.map(key => key.toBase58())).to.eql(v1JsonMessage.accountKeys);
    ...
    if (mockServer) {
        ...
        it('defaults searchTransactionHistory to false in getSignatureStatuses when only minContextSlot is set', async () => {
            ...
            await mockRpcResponse({
                method: 'getSignatureStatuses',
                params: [[mockSignature], { minContextSlot: 123, searchTransactionHistory: false }],
                value: [null],
                withContext: true,
            });
    ...
    it('get transaction', async function () {
        ...
        if (mockServer) {
            ...
            expect(result.meta?.preBalances).to.eql([499260357380n, 15298080n, 1n, 1n, 1n]);
            expect(result.meta?.postBalances).to.eql([499260347380n, 15298080n, 1n, 1n, 1n]);
        } else {
            ...
            expect(result.meta?.preBalances).to.satisfy(
                (balances: bigint[] | undefined) =>
                    balances != null && balances.length > 0 && balances.every(balance => typeof balance === 'bigint'),
            );
    ...
    it('get confirmed transaction', async function () {
        ...
        const nullResponse = await connection.getConfirmedTransaction(recentSignature, 'finalized');
        expect(nullResponse).to.be.null;
    });
    ...
    if (mockServer) {
        it('get parsed confirmed transaction coerces public keys of inner instructions', async () => {
            ...
            const result = await connection.getParsedConfirmedTransaction(confirmedTransaction);
            ...
            const result2 = await connection.getParsedConfirmedTransaction(confirmedTransaction);
                    ...
                    transactions: [
                        {
                            ...
                            transaction: {
                                message: {
                                    ...
                                    instructions: [
                                        {
                                            parsed: {
                                                info: {
                                                    ...
                                                    voteAccount: 'AhcvnNdppGEcgdpK5gfcaZnAWz4ct8V4n7De5QiLiuzG',
                                                    voteAuthority: '7v5fMKBqC9PuwjSdS9k9JU7efEXmq3bHTMF5fuSHnqrm',
                                                },
                                                type: 'vote',
                                            },
                                            program: 'vote',
                                    ...
                                    recentBlockhash: 'GLqYrN6AQxCGtFTQywkPj2WN5tafC3KerBhW4QkmAyD4',
                                },
                                signatures: [
        ...
        it('can deserialize a response when `transactionDetails` is `accounts`', async () => {
            ...
            if (mockServer) {
                ...
                expect(accountsModeBlock.blockHeight).to.satisfy(
                    (blockHeight: bigint | null) => blockHeight === null || typeof blockHeight === 'bigint',
                );
                expect(accountsModeBlock.blockTime).to.satisfy(
                    (blockTime: bigint | null) => blockTime === null || typeof blockTime === 'bigint',
                );
            }
        });
        ...
        it('can deserialize a response when `transactionDetails` is `signatures`', async () => {
            ...
            if (mockServer) {
                ...
                expect(signaturesModeBlock.blockHeight).to.satisfy(
                    (blockHeight: bigint | null) => blockHeight === null || blockHeight >= 0n,
                );
                expect(signaturesModeBlock.signatures).to.satisfy((signatures: Array<unknown>) =>
    ...
    describe('get block', function () {
        ...
        it('gets a block having a parent', async function () {
            ...
            expect(result.blockWithTransaction.previousBlockhash).to.eq(result.parentBlock.blockhash);
            expect(result.blockWithTransaction.blockhash).not.to.be.null;
            expect(result.blockWithTransaction.transactions[0].transaction).not.to.be.null;
            ...
            await expect(connection.getBlock(Number.MAX_SAFE_INTEGER)).to.be.rejectedWith(
    ...
    describe('get confirmed block', function () {
        ...
        it('gets a block having a parent', async function () {
            ...
            let result:
                ...
                | undefined;
            while (!result) {
                const candidateBlock = await connection.getConfirmedBlock(candidateSlot);
                if (candidateBlock && candidateBlock.transactions.length) {
                    const parentBlock = await connection.getConfirmedBlock(candidateSlot - 1);
                    if (parentBlock) {
                        result = { blockWithTransaction: candidateBlock, parentBlock };
                        break;
                    }
            ...
            await expect(connection.getConfirmedBlock(Number.MAX_SAFE_INTEGER)).to.be.rejectedWith(
                `Block not available for slot ${Number.MAX_SAFE_INTEGER}`,
            );
        });
    });
    ...
    it('get blocks forwards the config when the end slot is undefined', async () => {
        ...
        const observingConnection = new Connection(url, {
            ...
            fetch: async (fetchUrl, options) => {
                ...
                if (body.method === 'getBlocks') {
                    observedParams.push(body.params ?? []);
                }
                return await globalThis.fetch(fetchUrl, options);
            },
        });
        await mockRpcResponse({
            method: 'getBlocks',
        ...
        expect(observedParams).to.deep.equal([[5, null, { commitment: 'confirmed' }]]);
    });
    ...
    describe('get block signatures', function () {
        ...
        it('gets a block having a parent', async function () {
            ...
            let result:
                ...
                | undefined;
            while (!result) {
                const candidateBlock = await connection.getBlockSignatures(candidateSlot);
                if (candidateBlock && candidateBlock.signatures.length) {
                    const parentBlock = await connection.getBlockSignatures(candidateSlot - 1);
                    if (parentBlock) {
                        result = { blockWithTransaction: candidateBlock, parentBlock };
                        break;
                    }
            ...
            await expect(connection.getBlockSignatures(Number.MAX_SAFE_INTEGER)).to.be.rejectedWith(
                `Block not available for slot ${Number.MAX_SAFE_INTEGER}`,
            );
        });
    });
    ...
    if (mockServer) {
        it('get latest blockhash and context preserves the Kit bigint shape', async () => {
            ...
            expect(typeof latestBlockhashResponse.value.lastValidBlockHeight).to.eq('bigint');
        });
    }

    it('is blockhash valid', async () => {
        const validatedBlockhash = blockhash('FDeS2dHPUQgAsLZpExG7WUFiMHRcVGgUAeiJr8rfXR1K');
        if (mockServer) {
            for (const isBlockhashValid of [true, false]) {
                ...
                const isBlockhashValidRpcResult = await connection.isBlockhashValid(validatedBlockhash, {
                    commitment: 'confirmed',
                });
    ...
    it('get fee for message (legacy)', async () => {
        ...
        await mockRpcResponse({
            ...
            params: [Buffer.from(message.serialize()).toString('base64'), { commitment: 'confirmed' }],
            value: 5000,
            withContext: true,
        });
    ...
    it('get fee for message (v0)', async () => {
        ...
        const recentBlockhash = (await helpers.latestBlockhash({ connection })).blockhash;
        const instructions = [
            SystemProgram.transfer({
                fromPubkey: accountFrom.publicKey,
                toPubkey: accountTo.publicKey,
        ...
        await mockRpcResponse({
            method: 'getFeeForMessage',
            params: [Buffer.from(messageV0.serialize()).toString('base64'), { commitment: 'confirmed' }],
            value: 5000,
            withContext: true,
        });
    ...
    it('get supply', async () => {
        ...
        expect(supply.nonCirculatingAccounts[0].toBase58()).to.eq(nonCirculatingAccount);
    });
    ...
    [undefined, 'confirmed' as Commitment].forEach(function (commitment) {
        describe("when the connection's default commitment is `" + commitment + '`', () => {
            ...
            it('get performance samples', async () => {
                ...
                const perfSamples = await connectionWithCommitment.getRecentPerformanceSamples();
                ...
                if (perfSamples.length > 0) {
                    if (mockServer) {
                        ...
                        expect(typeof perfSamples[0].numNonVoteTransactions).to.eq('bigint');
    ...
    if (process.env.TEST_LIVE) {
        describe('token methods', () => {
            ...
            before(async function () {
                ...
                testOwnerKeypair = await Keypair.fromSecretKey(LEGACY_TOKEN_TEST_OWNER_SECRET_KEY);
                ...
                selfTransferSignature = await sendAndConfirmTransaction(connection, selfTransferTransaction, [
                    testOwnerKeypair,
                ]);
            });
            ...
            it('get token largest accounts', async () => {
                const largestAccounts = (await connection.getTokenLargestAccounts(testTokenMintPubkey)).value;

                expect(largestAccounts).to.have.length(1);
                const largestAccount = largestAccounts[0];
                expect(largestAccount.address.equals(testTokenAccountPubkey)).to.be.true;
                expect(largestAccount.amount).to.eq('11110');
                expect(largestAccount.decimals).to.eq(2);
                expect(largestAccount.uiAmount).to.eq(111.1);

                await expect(connection.getTokenLargestAccounts(newAccount)).to.be.rejected;
            });
            ...
            it('get confirmed token transaction', async () => {
                ...
                const parsedTx = await connection.getParsedConfirmedTransaction(selfTransferSignature, 'confirmed');
                if (parsedTx === null) {
                ...
                const nullResponse = await connection.getParsedConfirmedTransaction(missingSignature);

                expect(nullResponse).to.be.null;
            });

            it('get token account balance', async () => {
                const response = await connection.getTokenAccountBalance(testTokenAccountPubkey);
                expect(typeof response.context.slot).to.eq('bigint');
                ...
                await expect(connection.getTokenAccountBalance(newAccount)).to.be.rejected;
            });

            it('get parsed token account info', async () => {
                const response = await connection.getParsedAccountInfo(testTokenAccountPubkey);
                expect(typeof response.context.slot).to.eq('bigint');
            ...
            it('get parsed token accounts by owner', async () => {
                const response = await connection.getParsedTokenAccountsByOwner(testOwnerKeypair.publicKey, {
    ...
    ([undefined, 'processed', 'confirmed', 'finalized'] as (Commitment | undefined)[]).forEach(
                ...
                const sendTransactionMethod = stub().returns({
                    send: () =>
                        Promise.resolve({
                            id: '1',
                            jsonrpc: '2.0',
                            result: '5ZDp1HfNZhNRHc75ncsiZ4sCq1fGJHMGf9u36M3foD5PMH4Xu5S4X2x7aryn4JinUdG11oSYCk7zxbNmLJzzqUft',
                        }),
                });
                connection._typedRpc = { sendTransaction: sendTransactionMethod } as any;

                await connection.sendEncodedTransaction('AQ==', {
                    ...(explicitPreflightCommitment ? { preflightCommitment: explicitPreflightCommitment } : null),
                    skipPreflight: true,
                });
    ...
    it('getIdentity', async () => {
        if (mockServer) {
            ...
            expect(mockedIdentity.identity.toBase58()).to.eq('11111111111111111111111111111111');
            return;
        }
    ...
    it('getHighestSnapshotSlot', async () => {
        ...
        try {
            ...
            if (!message.includes('No snapshot') && !message.includes('Method not found')) {
                throw error;
            }
        }
    });
    ...
    it('request airdrop', async () => {
        ...
        await mockRpcResponse({
            method: 'getAccountInfo',
            params: [account.publicKey.toBase58(), { commitment: 'confirmed', encoding: 'base64' }],
            value: {
                owner: '11111111111111111111111111111111',
                lamports: LAMPORTS_PER_SOL,
                data: ['', 'base64'],
        ...
        const accountInfo = await connection.getAccountInfo(account.publicKey, 'confirmed');
        if (accountInfo === null) {
            expect(accountInfo).not.to.be.null;
            return;
        }
        ...
        await mockRpcResponse({
            method: 'getAccountInfo',
            params: [account.publicKey.toBase58(), { commitment: 'confirmed', encoding: 'jsonParsed' }],
            value: {
                owner: '11111111111111111111111111111111',
                lamports: LAMPORTS_PER_SOL,
                data: ['', 'base64'],
        ...
        const parsedAccountInfo = (await connection.getParsedAccountInfo(account.publicKey, 'confirmed')).value;
        ...
        it('simulateTransaction with config object preserves bigint fields', async () => {
            ...
            const versionedTx = new VersionedTransaction(
                new Message({
                    ...
                    recentBlockhash: blockhash('CSymwgTNX1j3E4qhKfJAUE41nBWEwXufoYryPbkde5RR'),
                    instructions: [],
            ...
            expect(response.value.innerInstructions?.[0].instructions).to.have.length(2);

            const parsedInstruction = response.value.innerInstructions?.[0].instructions[0];
            ...
            const rawInstruction = response.value.innerInstructions?.[0].instructions[1];
    ...
    if (process.env.TEST_LIVE) {
        ...
        it('sendTransaction', async () => {
            ...
            const recentBlockhash = await (await helpers.latestBlockhash({ connection })).blockhash;
        ...
        it('sendTransaction with a v1 transaction signed by a Kit transaction partial signer', async () => {
            ...
            expect(await connection.getBalance(recipient.publicKey)).to.eq(BigInt(transferLamports));
        });
        ...
        it('simulate transaction with message', async () => {
            ...
            const results1 = await connection.simulateTransaction(message, [account1], true);
            ...
            const results2 = await connection.simulateTransaction(
                message,
                [account1],
                [account1.publicKey, new PublicKey('Missing111111111111111111111111111111111111')],
            );
        ...
        it('transaction', async () => {
            ...
            const signature = await sendAndConfirmTransaction(connection, transaction, [accountFrom], {
                preflightCommitment: 'confirmed',
            });
            ...
            const signature2 = await sendAndConfirmTransaction(connection, transaction2, [accountFrom], {
                preflightCommitment: 'confirmed',
            });

            expect(signature).not.to.eq(signature2);
            expect(transaction.recentBlockhash).not.to.eq(transaction2.recentBlockhash);
            ...
            await sleep(Math.max(0, 1000 + BLOCKHASH_CACHE_TIMEOUT_MS - (Date.now() - lastFetch)));
            ...
            expect(transaction4.recentBlockhash).not.to.eq(transaction3.recentBlockhash);
            ...
            expect(await connection.getBalance(accountTo.publicKey)).to.eq(minimumAmount + 42n);
        ...
        it('multi-instruction transaction', async () => {
            ...
            let signature = await connection.requestAirdrop(accountFrom.publicKey, LAMPORTS_PER_SOL);
            await connection.confirmTransaction(signature);
            expect(await connection.getBalance(accountFrom.publicKey)).to.eq(BigInt(LAMPORTS_PER_SOL));

            const minimumAmount = await connection.getMinimumBalanceForRentExemption(0);

            signature = await connection.requestAirdrop(accountTo.publicKey, Number(minimumAmount + 21n));
            await connection.confirmTransaction(signature);
            expect(await connection.getBalance(accountTo.publicKey)).to.eq(minimumAmount + 21n);
            ...
            signature = await connection.sendTransaction(transaction, [accountFrom, accountTo], {
                skipPreflight: true,
            });
            ...
            expect((await connection.getBalance(accountFrom.publicKey)) > 0n).to.eq(true);
            expect((await connection.getBalance(accountFrom.publicKey)) <= BigInt(LAMPORTS_PER_SOL)).to.eq(true);
        ...
        describe('given an open websocket connection', () => {
            it('account change notification', async () => {
                ...
                try {
                    const accountInfoPromise = new Promise<AccountInfo<Uint8Array>>(resolve => {
                        subscriptionId = connection.onAccountChange(owner.publicKey, resolve, {
                            commitment: 'confirmed',
                        });
                    });
            ...
            it('program account change notification', async () => {
                ...
                let subscriptionId: number | undefined;
                try {
                    const keyedAccountInfoPromise = new Promise<KeyedAccountInfo>(resolve => {
                    ...
                    expect(keyedAccountInfo.accountInfo.lamports).to.eq(BigInt(balanceNeeded));
                    expect(keyedAccountInfo.accountInfo.data).to.be.instanceOf(Uint8Array);
                    expect(Buffer.isBuffer(keyedAccountInfo.accountInfo.data)).to.be.false;
                    expect(keyedAccountInfo.accountInfo.owner.equals(SystemProgram.programId)).to.be.true;
                } finally {
                    if (subscriptionId != null) {
            ...
            it('logs notification', async () => {
                ...
                try {
                    ...
                    expect(logsRes.logs[0]).to.eq('Program 11111111111111111111111111111111 invoke [1]');
                    expect(logsRes.logs[1]).to.eq('Program 11111111111111111111111111111111 success');
                } finally {
                    if (subscriptionId != null) {
        ...
        describe('address lookup table program', () => {
            ...
            before(async () => {
                ...
                lookupTableAddresses = await Promise.all(
                    Array.from({ length: 10 }).map(async () => (await Keypair.generate()).publicKey),
                );
            ...
            it('createLookupTable', async () => {
                ...
                [createIx, lookupTableKey] = await AddressLookupTableProgram.createLookupTable({
                    recentSlot,
            ...
            it('getAddressLookupTable', async () => {
                const lookupTableResponse = await connection.getAddressLookupTable(lookupTableKey, {
                    commitment: 'processed',
                });
        ...
        describe('v0 transaction', () => {
            ...
            it('send and confirm', async () => {
                ...
                const transferToAccount = await connection.getAccountInfo(transferToKey, {
                    commitment: 'confirmed',
            ...
            it('getTransaction', async () => {
                ...
                expect(fetchedTransaction.meta?.computeUnitsConsumed).to.not.be.undefined;
            ...
            it('getParsedTransaction', async () => {
                ...
                expect(parsedTransaction.meta?.computeUnitsConsumed).to.not.be.undefined;
                expect(parsedTransaction.transaction.message.addressTableLookups).to.eql(addressTableLookups);
