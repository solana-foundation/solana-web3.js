...
import { blockhash, createJsonRpcApi, createRpc, getBase58Codec } from '@solana/kit';
...
describe('Connection', function () {
    ...
    if (mockServer) {
        it('get minimum balance for rent exemption warns and returns zero on RPC error', async () => {
            ...
            try {
                await expect(connection.getMinimumBalanceForRentExemption(0)).to.eventually.eq(0n);
        ...
        describe('address lookup table program', () => {
            ...
            before(async () => {
                ...
                lookupTableAddresses = await Promise.all(
                    new Array(10).fill(0).map(async () => (await Keypair.generate()).publicKey),
                );
