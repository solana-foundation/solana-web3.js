    ...
    if (mockServer) {
        it('getNonce rejects an uninitialized System-owned account', async () => {
            ...
            await mockRpcResponse({
                ...
                params: [nonceAccount.toBase58(), { encoding: 'base64', commitment: 'confirmed' }],
                value: {
                    owner: '11111111111111111111111111111111',
                    lamports: 50,
                    data: [Buffer.from(uninitializedAccountData).toString('base64'), 'base64'],
                    executable: false,
                    rentEpoch: 20,
                },
                withContext: true,
            ...
            // Assert
            await expect(noncePromise).to.eventually.be.rejectedWith('nonce account is not initialized');
        });
    }
