...
- The legacy `Web3Signer` shape (`{publicKey, secretKey}`) and the `MessageSigner` type alias were removed. The exported `Signer` type is now `MessagePartialSigner & TransactionPartialSigner` from `@solana/kit`, and transaction signing APIs accept any Kit `TransactionPartialSigner`, which `Signer` satisfies. Pass `Keypair` instances or other Kit transaction signers; migrate any remaining `{publicKey, secretKey}` literals via `await Keypair.fromSecretKey(legacySigner.secretKey)`.
...
- `Connection.getMinimumBalanceForRentExemption(...)` now propagates RPC and transport failures instead of logging a warning and resolving `0n`. Callers that relied on the zero fallback must handle the rejection, because a failed rent query should not be mistaken for a rent-free account.
...
- The exported `Signer` type is now `MessagePartialSigner & TransactionPartialSigner` from `@solana/kit`. The legacy `{publicKey, secretKey}` shape is no longer accepted by `Transaction.sign(...)`, `Transaction.partialSign(...)`, `VersionedTransaction.sign(...)`, `Connection.sendTransaction(...)`, `Connection.simulateTransaction(...)`, or `sendAndConfirmTransaction(...)`. All of these accept any Kit `TransactionPartialSigner` (`Signer`, what `Keypair` provides, satisfies it) and assert transaction-signing capability at runtime; `Transaction.sign(...)`, `Transaction.partialSign(...)`, and `VersionedTransaction.sign(...)` sign through Kit's `partiallySignTransactionWithSigners`, deriving the blockhash or durable nonce lifetime from the compiled message; `VersionedTransaction.sign(signers, {lastValidBlockHeight})` forwards the block height to signers that need it. Message-only signers (`MessagePartialSigner` without `signTransactions`) are rejected. Pass `Keypair` instances or any other Kit `TransactionPartialSigner`.
...
- `sendAndConfirmTransaction(...)` now confirms with the durable-nonce strategy whenever `nonceInfo` is set, even if the transaction still carries `recentBlockhash` and `lastValidBlockHeight`. Previously, stale blockheight fields left on a reused `Transaction` could produce a false `TransactionExpiredBlockheightExceededError` after the nonce-backed transaction had already landed.
