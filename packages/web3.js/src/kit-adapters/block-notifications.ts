...
import { mapBlockBase, mapLoadedAddresses, normalizeTransactionVersion } from './response';
import type { BlockNotificationBlock, BlockSubscriptionConfig } from './subscription-types';
...
type BlockNotificationTransactionKind = 'accounts' | 'base58' | 'base64' | 'parsed' | 'json';
...
type BlockNotificationAccountsTransaction = (TransactionForAccounts<void> | TransactionForAccounts<0 | 1>) &
    Readonly<{ version?: TransactionVersion | bigint }>;

type BlockNotificationBase58Transaction = (TransactionForFullBase58<void> | TransactionForFullBase58<0 | 1>) &
    Readonly<{ version?: TransactionVersion | bigint }>;

type BlockNotificationBase64Transaction = (TransactionForFullBase64<void> | TransactionForFullBase64<0 | 1>) &
    Readonly<{ version?: TransactionVersion | bigint }>;

type BlockNotificationJsonTransaction = (TransactionForFullJson<void> | TransactionForFullJson<0 | 1>) &
    Readonly<{ version?: TransactionVersion | bigint }>;
...
type BlockNotificationFullTransactionMetaSource = BlockNotificationBase58Transaction['meta'];
...
function inferBlockNotificationTransactionKind(value: unknown): BlockNotificationTransactionKind | undefined {
    if (!isRecord(value)) {
        return undefined;
    }
    ...
    if (
        ...
        (isRecord(accountKeys[0]) && typeof (accountKeys[0] as { pubkey?: unknown }).pubkey === 'string')
    ) {
        return 'parsed';
    }
...
function hasTransactionsArray(block: unknown): block is BlockNotificationTransactionsSource {
    return isRecord(block) && Array.isArray((block as { transactions?: unknown }).transactions);
}

function hasSignaturesArray(block: unknown): block is RawBlockLike & Readonly<{ signatures: readonly string[] }> {
    return isRecord(block) && Array.isArray((block as { signatures?: unknown }).signatures);
}

function normalizeBlockNotificationMeta<TMeta extends BlockNotificationRawMeta>(meta: TMeta, expectation: string) {
    return {
        ...meta,
        ...
        ...(meta.costUnits != null
            ? {
                  costUnits: coerceNumericToBigInt(meta.costUnits, `${expectation}.costUnits`),
              }
            : null),
    };
}
...
function assertBlockNotificationTransactionsSourceHasKind<TKind extends BlockNotificationTransactionKind>(
    blockSource: BlockNotificationTransactionsSource,
    kind: TKind,
    expectation: string,
): asserts blockSource is BlockNotificationTransactionsSource<BlockNotificationTransactionByKind[TKind]> {
    assert(
        blockSource.transactions.every(transaction => inferBlockNotificationTransactionKind(transaction) === kind),
        expectation,
    );
}
...
>(
    ...
    mapMeta: (meta: TTransactionResponse['meta'], expectation: string) => TMappedMeta,
): Array<
...
export function mapBlockNotificationBlock(
    ...
    if (dispatchConfig == null || dispatchConfig === 'default') {
        if (hasTransactionsArray(notificationBlock)) {
            const transactionKind = inferBlockNotificationTransactionsSourceKind(notificationBlock);
            assert(transactionKind != null, 'Expected block subscription transactions');
            return mapBlockNotificationWithTransactionKind(notificationBlock, transactionKind);
        }
    ...
    switch (dispatchConfig.transactionDetails ?? 'full') {
        case 'accounts':
            assert(hasTransactionsArray(notificationBlock), 'Expected block subscription accounts transactions');
            return mapBlockNotificationWithTransactionKind(notificationBlock, 'accounts');
        case 'none':
            return mapBlockBase(notificationBlock as RawBlockLike);
        case 'signatures':
            ...
            return mapBlockNotificationSignaturesBlock(notificationBlock);
        case 'full':
            assert(hasTransactionsArray(notificationBlock), 'Expected block subscription transactions');
            return mapBlockNotificationWithTransactionKind(
                notificationBlock,
                dispatchConfig.encoding === 'jsonParsed' ? 'parsed' : (dispatchConfig.encoding ?? 'json'),
            );
    }
}
