...
export type TypedBlocksRequestConfig = NonNullable<Parameters<GetBlocksApi['getBlocks']>[2]>;

export type TypedRpcRequestMethod<TArgs extends unknown[], TResult = unknown> = (...args: TArgs) => {
...
export type TypedInflationRewardRequestConfig = Parameters<GetInflationRewardApi['getInflationReward']>[1];
...
export type TypedBlockWithoutTransactionsConfig<TTransactionDetails extends TypedBlockWithoutTransactionsMode> =
    Readonly<{
        commitment?: Finality;
...
export function getTypedBlockConfigBase(
    ...
    config: Pick<GetVersionedBlockConfig, 'maxSupportedTransactionVersion' | 'rewards'> | undefined,
...
export function getTypedBlockWithoutTransactionsConfig<TTransactionDetails extends TypedBlockWithoutTransactionsMode>(
    transactionDetails: TTransactionDetails,
    finality: Finality | undefined,
    config: Pick<GetVersionedBlockConfig, 'maxSupportedTransactionVersion' | 'rewards'> | undefined,
...
export function buildTypedFullBlockConfig(
    finality: Finality | undefined,
    config:
        | Pick<GetVersionedBlockConfig, 'maxSupportedTransactionVersion' | 'rewards' | 'transactionDetails'>
        | undefined,
...
export function buildTypedParsedFullBlockConfig(
    ...
    return {
        encoding: 'jsonParsed',
        ...getTypedBlockConfigBase(finality, config),
        ...(config?.transactionDetails === 'full' ? { transactionDetails: 'full' as const } : null),
...
export function buildTypedSignatureStatusesConfig(
    ...
    if (Object.keys(typedConfig).length === 0 && config?.searchTransactionHistory == null) {
        return undefined;
    }
...
export function getTokenAccountsRpcFilter(filter: TokenAccountsFilter): { mint: Address } | { programId: Address } {
    ...
    if (hasMint === hasProgramId) {
        throw new Error('Ambiguous token accounts filter. Supply exactly one of `mint` or ' + '`programId`.');
    }
...
export function getProgramAccountsRpcFilters(
...
): Array<GetProgramAccountsDatasizeFilter | GetProgramAccountsMemcmpFilter> | undefined {
    return filters?.map(filter => {
        ...
        if (hasMemcmp === hasDataSize) {
            throw new Error(
                'Ambiguous program accounts filter. Supply exactly one of `memcmp` ' + 'or `dataSize` per filter.',
            );
        }
