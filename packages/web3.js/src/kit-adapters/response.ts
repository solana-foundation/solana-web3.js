...
import { Message, MessageV0, MessageV1, type V1TransactionConfig, type VersionedMessage } from '../message';
...
type TypedLegacyMessageSource = TransactionForFullJson<void>['transaction']['message'];

type TypedVersion0MessageSource = TransactionForFullJson<0>['transaction']['message'];
 ...
 */
type TypedVersion1MessageSource = Omit<TransactionForFullJson<1>['transaction']['message'], 'addressTableLookups'>;

type TypedMessageSource = TypedLegacyMessageSource | TypedVersion0MessageSource | TypedVersion1MessageSource;
...
type TypedAccountsModeBlockTransaction = TransactionForAccounts<void> | TransactionForAccounts<0 | 1>;
...
type RawParsedMessageInstruction = TypedParsedTransactionSource['transaction']['message']['instructions'][number];

type RawParsedInnerInstructions = NonNullable<NonNullable<TypedParsedTransactionSource['meta']>['innerInstructions']>;

type RawParsedInnerInstruction = RawParsedInnerInstructions[number]['instructions'][number];
...
type RawBase64AccountInfo = RawAccountInfo<AccountInfoWithBase64EncodedData['data']>;
...
type RawJsonParsedAccountInfo = RawAccountInfo<RawBase64AccountInfo['data'] | RawParsedAccountData>;
...
type RawSimulatedAccountInfo = Readonly<AccountInfoBase & AccountInfoWithBase64EncodedData & { rentEpoch?: unknown }>;
...
function assertHasRawAccountRentEpoch<TAccount extends object>(
    ...
    assert('rentEpoch' in account && typeof account.rentEpoch === 'bigint', expectation);
}
...
export function mapJsonParsedAccountInfo(account: null, expectation?: string): null;
...
export function mapKeyedBase64AccountInfos(
    ...
    return mapKeyedAccounts(value, account => mapBase64AccountInfo(account, expectation));
}
...
export function mapKeyedJsonParsedAccountInfos(
    ...
    return mapKeyedAccounts(value, account => mapJsonParsedAccountInfo(account, expectation));
}
...
export function mapKeyedParsedAccountInfos(
    ...
    return mapKeyedAccounts(value, account => mapParsedAccountInfo(account, expectation));
}

function mapSimulatedAccountInfo(account: RawSimulatedAccountInfo | null): SimulatedTransactionAccountInfo | null {
    if (account == null) {
        return null;
    }
    ...
    if (account.rentEpoch != null) {
        assert(typeof account.rentEpoch === 'bigint', 'Expected simulated account rentEpoch to be bigint');
        rentEpoch = account.rentEpoch;
    }
...
function isRawSimulatedReplacementBlockhash(value: unknown): value is RawSimulatedReplacementBlockhash {
    return isRecord(value) && typeof value.blockhash === 'string' && typeof value.lastValidBlockHeight === 'bigint';
}
...
function mapSimulatedReturnData(returnData: RawSimulatedReturnData | null): TransactionReturnData | null {
    if (returnData == null) {
        return null;
    }
...
function isRpcParsedInnerInstructions(innerInstructions: unknown): innerInstructions is RawParsedInnerInstructions {
    return (
        Array.isArray(innerInstructions) &&
        innerInstructions.every(({ instructions }) =>
            instructions.every((instruction: unknown) => isRecord(instruction) && 'programId' in instruction),
        )
    );
}
...
function mapBlockRewards(rewards: readonly RawBlockReward[] | undefined) {
    return rewards?.map(reward => ({
        ...
        postBalance: reward.postBalance == null ? null : coerceNumericToBigInt(reward.postBalance, 'postBalance'),
...
export function mapBlockBase<TBlock extends RawBlockLike>(block: TBlock) {
    return {
        ...
        blockHeight: block.blockHeight == null ? null : coerceNumericToBigInt(block.blockHeight, 'blockHeight'),
        blockTime: block.blockTime == null ? null : coerceNumericToBigInt(block.blockTime, 'blockTime'),
...
export function mapSimulatedTransactionResponseValue(
    ...
    if ('fee' in value) {
        mappedValue.fee = value.fee == null ? null : coerceNumericToBigInt(value.fee, 'fee');
    }
    if ('preBalances' in value) {
        mappedValue.preBalances =
            value.preBalances == null
                ? null
                : value.preBalances.map(balance => coerceNumericToBigInt(balance, 'preBalance'));
    }
    if ('postBalances' in value) {
        mappedValue.postBalances =
            value.postBalances == null
                ? null
                : value.postBalances.map(balance => coerceNumericToBigInt(balance, 'postBalance'));
    }
    if ('preTokenBalances' in value) {
        mappedValue.preTokenBalances = cloneOptionalArray(value.preTokenBalances) ?? null;
    }
    if ('postTokenBalances' in value) {
        mappedValue.postTokenBalances = cloneOptionalArray(value.postTokenBalances) ?? null;
    }
    if ('loadedAddresses' in value) {
        mappedValue.loadedAddresses = value.loadedAddresses == null ? null : mapLoadedAddresses(value.loadedAddresses);
    }
    if (value.loadedAccountsDataSize !== undefined) {
        mappedValue.loadedAccountsDataSize = value.loadedAccountsDataSize;
    }
    if ('replacementBlockhash' in value && isRawSimulatedReplacementBlockhash(value.replacementBlockhash)) {
        mappedValue.replacementBlockhash = mapSimulatedReplacementBlockhash(value.replacementBlockhash);
    }
    ...
    if ('innerInstructions' in value) {
        ...
        mappedValue.innerInstructions = mapRpcParsedInnerInstructions(innerInstructions ?? null);
    }

    return mappedValue;
}

function legacyMessageFromResponse(message: TypedLegacyMessageSource, recentBlockhash: Blockhash): Message {
    return new Message({
...
function version0MessageFromResponse(message: TypedVersion0MessageSource, recentBlockhash: Blockhash): MessageV0 {
    return new MessageV0({
        header: message.header,
        staticAccountKeys: message.accountKeys.map(accountKey => new PublicKey(accountKey)),
        recentBlockhash,
        ...
        addressTableLookups: message.addressTableLookups?.map(mapParsedAddressTableLookup) ?? [],
    });
}

export function mapTransactionConfigResponse(config: RpcTransactionConfig): V1TransactionConfig | undefined {
    ...
    if (config.loadedAccountsDataSizeLimit != null) {
        mapped.loadedAccountsDataSizeLimit = Number(config.loadedAccountsDataSizeLimit);
    }
    if (config.priorityFee != null) {
        mapped.priorityFeeLamports = coerceNumericToBigInt(config.priorityFee, 'priorityFee');
    }
    return Object.keys(mapped).length > 0 ? mapped : undefined;
}

function version1MessageFromResponse(message: TypedVersion1MessageSource, recentBlockhash: Blockhash): MessageV1 {
    return new MessageV1({
        ...
        transactionConfig:
            message.transactionConfig != null ? mapTransactionConfigResponse(message.transactionConfig) : undefined,
    });
}
...
function mapParsedMessageAccount(account: RawParsedMessageAccount | ParsedMessageAccount): ParsedMessageAccount {
    return {
        ...account,
...
function mapParsedAddressTableLookup(
    ...
    return {
        ...lookup,
        accountKey: lookup.accountKey instanceof PublicKey ? lookup.accountKey : new PublicKey(lookup.accountKey),
...
function mapRpcParsedInstruction(
    ...
    if ('parsed' in instruction) {
        return {
            ...(instruction.stackHeight != null ? { stackHeight: instruction.stackHeight } : null),
    ...
    return {
        ...
        programId:
            instruction.programId instanceof PublicKey ? instruction.programId : new PublicKey(instruction.programId),
    };
}

function cloneOptionalArray<T>(value: readonly T[] | null | undefined): T[] | null | undefined {
...
): BlockResponse['transactions'][number]['meta'] | VersionedBlockResponse['transactions'][number]['meta'] {
    if (meta == null) {
        return null;
    }
    ...
    return transactions.map(transactionResponse => {
        const meta =
            transactionResponse.meta != null && hasCostUnits(transactionResponse.meta)
                ? {
                      ...
                      costUnits: coerceNumericToBigInt(transactionResponse.meta.costUnits, 'costUnits'),
                  }
                : transactionResponse.meta;
        ...
        return {
            ...
            transaction: {
                ...
                accountKeys: transactionResponse.transaction.accountKeys.map(mapParsedMessageAccount),
...
export function mapTypedFullBlockTransaction(
    ...
    return {
        ...
        transaction: {
            ...transactionResponse.transaction,
            signatures: [...transactionResponse.transaction.signatures],
            message: versionedMessageFromResponse(version, transactionResponse.transaction.message),
        },
    };
}

export function mapParsedTransaction(transaction: TypedParsedTransactionSource['transaction']): ParsedTransaction {
    const addressTableLookups =
        'addressTableLookups' in transaction.message && Array.isArray(transaction.message.addressTableLookups)
            ? transaction.message.addressTableLookups.map(mapParsedAddressTableLookup)
            : null;

    const { transactionConfig: rawTransactionConfig, ...rawMessage } = transaction.message;
    const transactionConfig =
        rawTransactionConfig != null ? mapTransactionConfigResponse(rawTransactionConfig) : undefined;
...
function mapTransactionMetaCompat(meta: TypedTransactionSource['meta']): ConfirmedTransactionMeta | null {
    if (meta == null) {
        return null;
    }
    ...
    return {
        ...(meta.computeUnitsConsumed != null
            ? {
                  computeUnitsConsumed: coerceNumericToBigInt(meta.computeUnitsConsumed, 'computeUnitsConsumed'),
              }
            : null),
        err: meta.err,
        ...
        postBalances: meta.postBalances.map(balance => coerceNumericToBigInt(balance, 'postBalance')),
        ...('postTokenBalances' in meta ? { postTokenBalances: cloneOptionalArray(meta.postTokenBalances) } : null),
        preBalances: meta.preBalances.map(balance => coerceNumericToBigInt(balance, 'preBalance')),
        ...('preTokenBalances' in meta ? { preTokenBalances: cloneOptionalArray(meta.preTokenBalances) } : null),
        ...('returnData' in meta ? { returnData: mapSimulatedReturnData(meta.returnData ?? null) } : null),
        ...('rewards' in meta
            ? {
                  rewards: meta.rewards == null ? meta.rewards : mapBlockRewards(meta.rewards),
              }
            : null),
        ...(hasCostUnits(meta) ? { costUnits: coerceNumericToBigInt(meta.costUnits, 'costUnits') } : null),
    };
}

function mapParsedTransactionMetaCompat(meta: TypedParsedTransactionSource['meta']): ParsedTransactionMeta | null {
    if (meta == null) {
        return null;
    }
...
export function normalizeTransactionVersion(
    ...
    assert(numericVersion === 0 || numericVersion === 1, `Unsupported transaction version: ${String(version)}`);
    return numericVersion;
}

export function mapTypedTransactionResponse(response: TypedTransactionSource): VersionedTransactionResponse {
    const version = normalizeTransactionVersion('version' in response ? response.version : undefined);

    return {
        blockTime: response.blockTime == null ? null : coerceNumericToBigInt(response.blockTime, 'blockTime'),
        meta: mapTransactionMetaCompat(response.meta),
        slot: coerceNumericToBigInt(response.slot, 'slot'),
        transaction: {
            ...response.transaction,
            message: versionedMessageFromResponse(version, response.transaction.message),
...
export function mapTypedParsedTransactionResponse(response: TypedParsedTransactionSource): ParsedTransactionWithMeta {
    const version = normalizeTransactionVersion('version' in response ? response.version : undefined);
