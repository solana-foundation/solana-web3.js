...
export class StakeAccount {
    ...
    static fromAccountData(bufferLike: Uint8Array | Array<number>): StakeAccount {
        const decoded = getStakeStateAccountDecoder().decode(toUint8ArrayView(bufferLike));
...
type ValueOf<TRecord> = TRecord extends Record<PropertyKey, infer TValue> ? TValue : never;
...
const GENERATED_TO_LEGACY_INSTRUCTION_TYPE = {
    ...
    [GeneratedStakeInstruction.AuthorizeCheckedWithSeed]: 'AuthorizeCheckedWithSeed',
...
type ParsedInstructionOfType<TInstructionType extends GeneratedStakeInstruction> = Extract<
    ParsedAnyStakeInstruction,
    { instructionType: TInstructionType }
>;

function toGeneratedAuthorized(authorized: Authorized): GeneratedAuthorizedArgs {
    return {
...
function fromGeneratedAuthorized(authorized: GeneratedAuthorized): Authorized {
    return new Authorized(new PublicKey(authorized.staker), new PublicKey(authorized.withdrawer));
}

function parseStakeAccountAuthorized(authorized: GeneratedAuthorized): StakeAccountAuthorized {
    return {
...
function fromGeneratedLockup(lockup: GeneratedLockup): Lockup {
    return new Lockup(Number(lockup.unixTimestamp), Number(lockup.epoch), new PublicKey(lockup.custodian));
}
...
function parseStakeAccountDelegation(delegation: GeneratedDelegation): StakeAccountDelegation {
    return {
...
function parseStakeAccountFlags(stakeFlags: GeneratedStakeFlags): StakeAccountFlags {
...
function toGeneratedStakeAuthorize(stakeAuthorizationType: StakeAuthorizationType): GeneratedStakeAuthorize {
    return stakeAuthorizationType.index as GeneratedStakeAuthorize;
}

function fromGeneratedStakeAuthorize(stakeAuthorize: GeneratedStakeAuthorize): StakeAuthorizationType {
    return { index: stakeAuthorize };
}

type GeneratedOption<T> = Readonly<{ __option: 'None' }> | Readonly<{ __option: 'Some'; value: T }>;
...
function getInstructionType(instruction: TransactionInstruction): StakeInstructionType {
...
function parseStakeInstructionOfType<TInstructionType extends GeneratedStakeInstruction>(
...
export class StakeInstruction {
    ...
    static decodeInitialize(instruction: TransactionInstruction): InitializeStakeParams {
        const parsedInstruction = parseStakeInstructionOfType(instruction, GeneratedStakeInstruction.Initialize);
     ...
     */
    static decodeInitializeChecked(instruction: TransactionInstruction): InitializeCheckedStakeParams {
        const parsedInstruction = parseStakeInstructionOfType(instruction, GeneratedStakeInstruction.InitializeChecked);
    ...
    static decodeDelegate(instruction: TransactionInstruction): DelegateStakeParams {
        const parsedInstruction = parseStakeInstructionOfType(instruction, GeneratedStakeInstruction.DelegateStake);

        return {
            stakePubkey: new PublicKey(parsedInstruction.accounts.stake.address),
            votePubkey: new PublicKey(parsedInstruction.accounts.vote.address),
            authorizedPubkey: new PublicKey(parsedInstruction.accounts.stakeAuthority.address),
        };
    }
    ...
    static decodeAuthorize(instruction: TransactionInstruction): AuthorizeStakeParams {
        const parsedInstruction = parseStakeInstructionOfType(instruction, GeneratedStakeInstruction.Authorize);

        const o: AuthorizeStakeParams = {
            stakePubkey: new PublicKey(parsedInstruction.accounts.stake.address),
            authorizedPubkey: new PublicKey(parsedInstruction.accounts.authority.address),
            newAuthorizedPubkey: new PublicKey(parsedInstruction.data.arg0),
            stakeAuthorizationType: fromGeneratedStakeAuthorize(parsedInstruction.data.arg1),
        };
        if (parsedInstruction.accounts.lockupAuthority) {
            o.custodianPubkey = new PublicKey(parsedInstruction.accounts.lockupAuthority.address);
        }
        return o;
    }
     ...
     */
    static decodeAuthorizeChecked(instruction: TransactionInstruction): AuthorizeCheckedStakeParams {
        const parsedInstruction = parseStakeInstructionOfType(instruction, GeneratedStakeInstruction.AuthorizeChecked);

        const o: AuthorizeCheckedStakeParams = {
            stakePubkey: new PublicKey(parsedInstruction.accounts.stake.address),
            authorizedPubkey: new PublicKey(parsedInstruction.accounts.authority.address),
            newAuthorizedPubkey: new PublicKey(parsedInstruction.accounts.newAuthority.address),
            stakeAuthorizationType: fromGeneratedStakeAuthorize(parsedInstruction.data.stakeAuthorize),
        };
        if (parsedInstruction.accounts.lockupAuthority) {
            o.custodianPubkey = new PublicKey(parsedInstruction.accounts.lockupAuthority.address);
        }
    ...
    static decodeAuthorizeWithSeed(instruction: TransactionInstruction): AuthorizeWithSeedStakeParams {
        const parsedInstruction = parseStakeInstructionOfType(instruction, GeneratedStakeInstruction.AuthorizeWithSeed);
        ...
        const o: AuthorizeWithSeedStakeParams = {
            ...
            newAuthorizedPubkey: new PublicKey(parsedInstruction.data.newAuthorizedPubkey),
            stakeAuthorizationType: fromGeneratedStakeAuthorize(parsedInstruction.data.stakeAuthorize),
        };
        if (parsedInstruction.accounts.lockupAuthority) {
            o.custodianPubkey = new PublicKey(parsedInstruction.accounts.lockupAuthority.address);
     ...
     */
    static decodeAuthorizeCheckedWithSeed(instruction: TransactionInstruction): AuthorizeCheckedWithSeedStakeParams {
    ...
    static decodeSetLockup(instruction: TransactionInstruction): SetLockupStakeParams {
        const parsedInstruction = parseStakeInstructionOfType(instruction, GeneratedStakeInstruction.SetLockup);

        const o: SetLockupStakeParams = {
            stakePubkey: new PublicKey(parsedInstruction.accounts.stake.address),
            authorizedPubkey: new PublicKey(parsedInstruction.accounts.authority.address),
        };
        const unixTimestamp = unwrapGeneratedOption(parsedInstruction.data.unixTimestamp);
     ...
     */
    static decodeSetLockupChecked(instruction: TransactionInstruction): SetLockupCheckedStakeParams {
        const parsedInstruction = parseStakeInstructionOfType(instruction, GeneratedStakeInstruction.SetLockupChecked);
        ...
        if (parsedInstruction.accounts.newAuthority) {
            o.newAuthorizedPubkey = new PublicKey(parsedInstruction.accounts.newAuthority.address);
        }
        const unixTimestamp = unwrapGeneratedOption(parsedInstruction.data.unixTimestamp);
        const epoch = unwrapGeneratedOption(parsedInstruction.data.epoch);
        if (unixTimestamp !== undefined) {
     ...
     */
    static decodeSplit(instruction: TransactionInstruction): DecodedSplitStakeInstruction {
        const parsedInstruction = parseStakeInstructionOfType(instruction, GeneratedStakeInstruction.Split);

        return {
            stakePubkey: new PublicKey(parsedInstruction.accounts.stake.address),
            splitStakePubkey: new PublicKey(parsedInstruction.accounts.splitStake.address),
            authorizedPubkey: new PublicKey(parsedInstruction.accounts.stakeAuthority.address),
     ...
     */
    static decodeGetMinimumDelegation(instruction: TransactionInstruction): GetMinimumDelegationStakeParams {
        parseStakeInstructionOfType(instruction, GeneratedStakeInstruction.GetMinimumDelegation);
    ...
    static decodeMerge(instruction: TransactionInstruction): MergeStakeParams {
        const parsedInstruction = parseStakeInstructionOfType(instruction, GeneratedStakeInstruction.Merge);

        return {
            stakePubkey: new PublicKey(parsedInstruction.accounts.destinationStake.address),
            sourceStakePubKey: new PublicKey(parsedInstruction.accounts.sourceStake.address),
            authorizedPubkey: new PublicKey(parsedInstruction.accounts.stakeAuthority.address),
        };
    }
     ...
     */
    static decodeWithdraw(instruction: TransactionInstruction): DecodedWithdrawStakeInstruction {
        const parsedInstruction = parseStakeInstructionOfType(instruction, GeneratedStakeInstruction.Withdraw);

        const o: DecodedWithdrawStakeInstruction = {
            stakePubkey: new PublicKey(parsedInstruction.accounts.stake.address),
            toPubkey: new PublicKey(parsedInstruction.accounts.recipient.address),
            authorizedPubkey: new PublicKey(parsedInstruction.accounts.withdrawAuthority.address),
            lamports: parsedInstruction.data.args,
        };
        if (parsedInstruction.accounts.lockupAuthority) {
            o.custodianPubkey = new PublicKey(parsedInstruction.accounts.lockupAuthority.address);
    ...
    static decodeDeactivate(instruction: TransactionInstruction): DeactivateStakeParams {
        const parsedInstruction = parseStakeInstructionOfType(instruction, GeneratedStakeInstruction.Deactivate);
     ...
     */
    static decodeDeactivateDelinquent(instruction: TransactionInstruction): DeactivateDelinquentStakeParams {
        const parsedInstruction = parseStakeInstructionOfType(
            instruction,
        ...
        return {
            stakePubkey: new PublicKey(parsedInstruction.accounts.stake.address),
            delinquentVotePubkey: new PublicKey(parsedInstruction.accounts.delinquentVote.address),
            referenceVotePubkey: new PublicKey(parsedInstruction.accounts.referenceVote.address),
        };
    }
    ...
    static decodeMoveStake(instruction: TransactionInstruction): MoveStakeParams {
        const parsedInstruction = parseStakeInstructionOfType(instruction, GeneratedStakeInstruction.MoveStake);

        return {
            sourceStakePubkey: new PublicKey(parsedInstruction.accounts.sourceStake.address),
            destinationStakePubkey: new PublicKey(parsedInstruction.accounts.destinationStake.address),
            authorizedPubkey: new PublicKey(parsedInstruction.accounts.stakeAuthority.address),
            lamports: parsedInstruction.data.args,
        };
    }
     ...
     */
    static decodeMoveLamports(instruction: TransactionInstruction): MoveLamportsParams {
        const parsedInstruction = parseStakeInstructionOfType(instruction, GeneratedStakeInstruction.MoveLamports);
 ...
 */
export type StakeInstructionType = ValueOf<typeof GENERATED_TO_LEGACY_INSTRUCTION_TYPE>;
...
export class StakeProgram {
     ...
     */
    static initializeChecked(params: InitializeCheckedStakeParams): TransactionInstruction {
        ...
        return new Transaction().add(
            fromKitInstruction(
                getAuthorizeInstruction({
                    ...
                    ...(custodianPubkey ? { lockupAuthority: createNoopSigner(custodianPubkey.toBase58()) } : {}),
     ...
     */
    static authorizeCheckedWithSeed(params: AuthorizeCheckedWithSeedStakeParams): Transaction {
        const {
            stakePubkey,
            authorityBase,
            authoritySeed,
    ...
    static setLockup(params: SetLockupStakeParams): Transaction {
        const { stakePubkey, authorizedPubkey, unixTimestamp, epoch, custodian } = params;
    ...
    static setLockupChecked(params: SetLockupCheckedStakeParams): Transaction {
        const { stakePubkey, authorizedPubkey, newAuthorizedPubkey, unixTimestamp, epoch } = params;
     ...
     */
    static deactivateDelinquent(params: DeactivateDelinquentStakeParams): Transaction {
    ...
    static moveStake(params: MoveStakeParams): Transaction {
        const { sourceStakePubkey, destinationStakePubkey, authorizedPubkey, lamports } = params;
