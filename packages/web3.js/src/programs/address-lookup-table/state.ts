import { ADDRESS_LOOKUP_TABLE_DISCRIMINATOR, getAddressLookupTableDecoder } from '@solana-program/address-lookup-table';
...
export class AddressLookupTableAccount {
    ...
    static deserialize(accountData: Uint8Array): AddressLookupTableState {
        ...
        if (state.discriminator !== ADDRESS_LOOKUP_TABLE_DISCRIMINATOR) {
            throw new Error(`Invalid address lookup table discriminator: ${state.discriminator}`);
        }
