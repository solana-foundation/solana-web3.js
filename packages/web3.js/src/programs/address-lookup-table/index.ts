...
const ADDRESS_LOOKUP_TABLE_PROGRAM_ID = new PublicKey(ADDRESS_LOOKUP_TABLE_PROGRAM_ADDRESS);
...
export class AddressLookupTableInstruction {
    ...
    static decodeInstructionType(instruction: TransactionInstruction): LookupTableInstructionType {
        ...
        return GENERATED_TO_LEGACY_INSTRUCTION_TYPE[identifyAddressLookupTableInstruction(instruction.data)];
    }
    ...
    static decodeExtendLookupTable(instruction: TransactionInstruction): ExtendLookupTableParams {
        ...
        const { addresses } = getExtendLookupTableInstructionDataDecoder().decode(instruction.data);
        return {
...
const GENERATED_TO_LEGACY_INSTRUCTION_TYPE = {
    [GeneratedAddressLookupTableInstruction.CreateLookupTable]: 'CreateLookupTable',
    [GeneratedAddressLookupTableInstruction.FreezeLookupTable]: 'FreezeLookupTable',
    [GeneratedAddressLookupTableInstruction.ExtendLookupTable]: 'ExtendLookupTable',
    [GeneratedAddressLookupTableInstruction.DeactivateLookupTable]: 'DeactivateLookupTable',
...
type ParsedAnyAddressLookupTableInstruction = ParsedAddressLookupTableInstruction<string>;

type ParsedInstructionOfType<TInstructionType extends GeneratedAddressLookupTableInstruction> = Extract<
    ParsedAnyAddressLookupTableInstruction,
    { instructionType: TInstructionType }
>;

function parseAddressLookupTableInstructionOfType<TInstructionType extends GeneratedAddressLookupTableInstruction>(
    instruction: TransactionInstruction,
    expectedInstructionType: TInstructionType,
): ParsedInstructionOfType<TInstructionType> {
    const parsedInstruction = parseAddressLookupTableInstruction(toKitInstruction(instruction));
...
function buildExtendLookupTableInstructionWithoutPayer(params: ExtendLookupTableParams): TransactionInstruction {
...
export class AddressLookupTableProgram {
    ...
    static async createLookupTable(params: CreateLookupTableParams) {
        const [lookupTableAddress, bumpSeed] = await PublicKey.findProgramAddress(
            [params.authority.toBytes(), getU64Encoder().encode(params.recentSlot) as Uint8Array],
            this.programId,
        );

        const instruction = fromKitInstruction(
            getCreateLookupTableInstruction({
                address: [lookupTableAddress.toAddress(), bumpSeed] as unknown as Parameters<
        ...
        return [instruction, lookupTableAddress] as [TransactionInstruction, PublicKey];
    }
