...
export type DecodedWithdrawFromVoteAccountParams = Omit<WithdrawFromVoteAccountParams, 'lamports'> & {
    lamports: bigint;
};
...
type InstructionCodecInput<TCodec> = TCodec extends Codec<infer TFrom, any> ? TFrom : never;

type InstructionCodecOutput<TCodec> = TCodec extends Codec<any, infer TTo> ? TTo : never;

type StripInstruction<T> = T extends { instruction: unknown } ? Omit<T, 'instruction'> : T;

type VoteInstructionParams<TCodec> = StripInstruction<InstructionCodecInput<TCodec>>;
type VoteInstructionDecoded<TCodec> = StripInstruction<InstructionCodecOutput<TCodec>>;
...
const UPDATE_VALIDATOR_IDENTITY_CODEC = getStructCodec([['instruction', U32_CODEC]]);
...
function decodeVoteInstructionData<TCodec extends Codec<any, any>>(
    ...
    try {
        decoded = codec.decode(instruction.data) as InstructionCodecOutput<TCodec> & VoteInstructionData;
    } catch (err) {
        if (err instanceof Error && err.message.startsWith('invalid instruction;')) {
            throw err;
        }
        throw new Error('invalid instruction; ' + String(err));
    }

    const { instruction: decodedInstruction, ...rest } = decoded;
    if (decodedInstruction !== expectedInstruction) {
        throw new Error(
            `invalid instruction; instruction index mismatch ${decodedInstruction} != ${expectedInstruction}`,
        );
    }
...
function getVoteInstructionType(instruction: TransactionInstruction): VoteInstructionType {
    ...
    switch (instructionIndex) {
        ...
        case TOWER_SYNC_SWITCH_INSTRUCTION_INDEX:
            throw new Error(`invalid instruction; unsupported vote-program instruction index ${instructionIndex}`);
        default:
            throw new Error(`invalid instruction; unknown instruction index ${instructionIndex}`);
    }
}
...
export class VoteInstruction {
     ...
     */
    static decodeAuthorizeChecked(instruction: TransactionInstruction): AuthorizeCheckedVoteParams {
        this.checkProgramId(instruction.programId);
        this.checkKeyLength(instruction.keys, 4);
    ...
    static decodeAuthorizeWithSeed(instruction: TransactionInstruction): AuthorizeVoteWithSeedParams {
        ...
        } = decodeVoteInstructionData(instruction, AUTHORIZE_WITH_SEED_INSTRUCTION_INDEX, AUTHORIZE_WITH_SEED_CODEC);
     ...
     */
    static decodeAuthorizeCheckedWithSeed(instruction: TransactionInstruction): AuthorizeCheckedVoteWithSeedParams {
        this.checkProgramId(instruction.programId);
        this.checkKeyLength(instruction.keys, 4);

        const { currentAuthorityDerivedKeyOwnerPubkey, currentAuthorityDerivedKeySeed, voteAuthorizationType } =
     ...
     */
    static decodeWithdraw(instruction: TransactionInstruction): DecodedWithdrawFromVoteAccountParams {
        this.checkProgramId(instruction.programId);
        this.checkKeyLength(instruction.keys, 3);

        const { lamports } = decodeVoteInstructionData(instruction, WITHDRAW_INSTRUCTION_INDEX, WITHDRAW_CODEC);
     ...
     */
    static decodeUpdateValidatorIdentity(instruction: TransactionInstruction): UpdateValidatorIdentityParams {
        this.checkProgramId(instruction.programId);
        this.checkKeyLength(instruction.keys, 3);
     ...
     */
    static decodeUpdateCommission(instruction: TransactionInstruction): UpdateCommissionVoteParams {
        this.checkProgramId(instruction.programId);
...
export class VoteProgram {
    ...
    static initializeAccount(params: InitializeAccountParams): TransactionInstruction {
        ...
        return new TransactionInstruction({
            ...
            data: encodeVoteInstructionData(INITIALIZE_ACCOUNT_INSTRUCTION_INDEX, INITIALIZE_ACCOUNT_CODEC, {
                voteInit: {
    ...
    static authorize(params: AuthorizeVoteParams): Transaction {
        ...
        return new Transaction().add(
            new TransactionInstruction({
                keys,
                programId: this.programId,
                data: encodeVoteInstructionData(AUTHORIZE_INSTRUCTION_INDEX, AUTHORIZE_CODEC, {
    ...
    static authorizeChecked(params: AuthorizeCheckedVoteParams): Transaction {
        ...
        return new Transaction().add(
            new TransactionInstruction({
                ...
                programId: this.programId,
                data: encodeVoteInstructionData(AUTHORIZE_CHECKED_INSTRUCTION_INDEX, AUTHORIZE_CHECKED_CODEC, {
                    voteAuthorizationType: voteAuthorizationType.index,
                }),
            }),
        );
    ...
    static authorizeWithSeed(params: AuthorizeVoteWithSeedParams): Transaction {
        ...
        return new Transaction().add(
            new TransactionInstruction({
                keys,
                programId: this.programId,
                data: encodeVoteInstructionData(AUTHORIZE_WITH_SEED_INSTRUCTION_INDEX, AUTHORIZE_WITH_SEED_CODEC, {
                    voteAuthorizeWithSeedArgs: {
     ...
     */
    static authorizeCheckedWithSeed(params: AuthorizeCheckedVoteWithSeedParams): Transaction {
        const {
            currentAuthorityDerivedKeyBasePubkey,
            currentAuthorityDerivedKeyOwnerPubkey,
            currentAuthorityDerivedKeySeed,
    ...
    static withdraw(params: WithdrawFromVoteAccountParams): Transaction {
        ...
        return new Transaction().add(
            new TransactionInstruction({
                keys,
                programId: this.programId,
                data: encodeVoteInstructionData(WITHDRAW_INSTRUCTION_INDEX, WITHDRAW_CODEC, { lamports }),
            }),
        );
    }
    ...
    static safeWithdraw(
        ...
        const currentBalance = coerceU64ToBigInt(currentVoteAccountBalance, 'currentVoteAccountBalance');
        const rentMinimum = coerceU64ToBigInt(rentExemptMinimum, 'rentExemptMinimum');
    ...
    static updateCommission(params: UpdateCommissionVoteParams): Transaction {
        ...
        return new Transaction().add(
            new TransactionInstruction({
                ...
                programId: this.programId,
                data: encodeVoteInstructionData(UPDATE_COMMISSION_INSTRUCTION_INDEX, UPDATE_COMMISSION_CODEC, {
                    commission,
                }),
            }),
        );
