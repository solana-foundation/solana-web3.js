...
import { ConnectionSubscriptionRegistry, type ClientSubscriptionId, type SubscriptionConfigByKind } from './registry';
import {
...
export class ConnectionSubscriptionsController<TBlockDispatchConfig extends StoredBlockSubscriptionDispatchConfig> {
    constructor(
    ...
    handleNotification({ kind, notification }: RpcWebSocketNotificationEvent): void {
        switch (kind) {
            case 'account': {
                const { result, subscription } = notification;
                const accountInfo = normalizeWebSocketAccountInfo(result.value);
                this._subscriptionRegistry.dispatchNotification(subscription, [accountInfo, result.context]);
                return;
            }

            case 'block': {
                const { result, subscription } = notification;
                const dispatchConfig = this._subscriptionRegistry.getBlockDispatchConfigForServerId(subscription);
                const block = mapBlockNotificationBlock(result.value.block, dispatchConfig);
            ...
            case 'signature': {
                ...
                this._subscriptionRegistry.dispatchNotification(subscription, [signatureNotification, result.context]);
                return;
            }
    ...
    registerSubscription<TKind extends SubscriptionKind>(
        ...
        if (dispatchConfig?.defaultDispatchConfig !== undefined) {
            this._subscriptionRegistry.setDefaultBlockDispatchConfig(hash, dispatchConfig.defaultDispatchConfig);
        }
        if (dispatchConfig?.dispatchConfig !== undefined) {
            this._subscriptionRegistry.setBlockDispatchConfig(hash, dispatchConfig.dispatchConfig);
        }
        this._subscriptionRegistry.addSubscriptionCallback(hash, subscriptionConfig.callback, subscriptionConfig.spec);
        const clientSubscriptionId = this._subscriptionRegistry.createClientSubscription(
            hash,
    ...
    async removeClientSubscription(
        ...
        const clientSubscription = this._subscriptionRegistry.removeClientSubscription(clientSubscriptionId);
        if (
            clientSubscription != null &&
            this._subscriptionRegistry.removeSubscriptionCallback(clientSubscription.hash, clientSubscription.callback)
        ) {
    ...
    async updateSubscriptions(): Promise<void> {
        ...
        const activeSubscriptionChannelAbortController = subscriptionsRuntime.connectionGeneration;
        const isCurrentConnectionStillActive = () =>
            activeSubscriptionChannelAbortController !== null &&
            activeSubscriptionChannelAbortController === this._getSubscriptionsRuntime().connectionGeneration;
        ...
        await Promise.all(
            this._subscriptionRegistry.getSubscriptionHashes().map(async hash => {
                ...
                const getCurrentSubscriptionForUpdate = () => {
                    const currentSubscription = this._subscriptionRegistry.getSubscription(hash);
                    if (
                ...
                switch (subscription.state) {
                    case 'pending':
                    case 'unsubscribed': {
                        if (getPendingSubscriptionUpdateDisposition(subscription) === 'prune') {
                            this._subscriptionRegistry.pruneSubscription(hash);
                            await this.updateSubscriptions();
                            return;
                        }
                        ...
                        try {
                            const subscriptionHandle = await subscriptionsRuntime.openSubscription(subscription.spec);
                            const currentSubscription = getCurrentSubscriptionForUpdate();
                            const disposition = getPendingSubscriptionUpdateDisposition(currentSubscription);
                            if (disposition === 'stop') {
                            ...
                            if (
                                ...
                                try {
                                    ...
                                    if (error instanceof Error) {
                                        console.error(`${subscription.spec.kind} unsubscribe error:`, error.message);
                                    }
                                    await this.updateSubscriptions();
                                    return;
                                }
                            } else {
                                this._subscriptionRegistry.abortServerSubscription(subscription.serverSubscriptionId);
                            }
                            if (getCurrentSubscriptionForUpdate() === undefined) {
                                return;
                            }
