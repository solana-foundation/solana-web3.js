...
export class KitSubscriptionRuntime<TBlockDispatchConfig> implements ConnectionSubscriptionsRuntime {
    ...
    constructor(
        ...
        this._createSubscriptionChannel = createSubscriptionChannel(endpoint, subscriptionChannelConfig);
        this._subscriptions = createSubscriptionRpc({
            api: createSolanaRpcSubscriptionsApi<SolanaRpcSubscriptionsApi & SolanaRpcSubscriptionsApiUnstable>({
                ...DEFAULT_RPC_SUBSCRIPTIONS_CONFIG,
                defaultCommitment: defaultCommitment ?? DEFAULT_RPC_SUBSCRIPTIONS_CONFIG.defaultCommitment,
            }),
        ...
        try {
            switch (spec.kind) {
                case 'account': {
                    ...
                    const openAccountNotifications = this._subscriptions.accountNotifications as NotificationOpener<
                        Address,
                ...
                case 'block': {
                    ...
                    return this._createSubscriptionHandle(
                        this._subscriptions.blockNotifications(filter, spec.options).subscribe({ abortSignal }),
                        serverSubscriptionId,
                        abortController,
                        result => {
                            this._dispatchNotification({
                ...
                case 'logs': {
                    const openLogsNotifications = this._subscriptions.logsNotifications as NotificationOpener<
                ...
                case 'program': {
                    const typedAddress = spec.address;
                    assertIsAddress(typedAddress);
                    const openProgramNotifications = this._subscriptions.programNotifications as NotificationOpener<
                        Address,
                ...
                case 'slotsUpdates': {
                    return this._createSubscriptionHandle(
                        this._subscriptions.slotsUpdatesNotifications().subscribe({ abortSignal }),
                        serverSubscriptionId,
                        abortController,
                        result => {
                            this._dispatchNotification({
