...
import { ConnectionSubscriptionRegistry, type SubscriptionHandle, type ServerSubscriptionId } from './registry';
...
type UnstableSubscriptions = SubscriptionClient<SolanaRpcSubscriptionsApi & SolanaRpcSubscriptionsApiUnstable>;
...
export type RpcWebSocketAccountNotification = RpcWebSocketSubscriptionNotification<
    SolanaRpcResponse<RpcWebSocketAccountInfo>
>;

export type RpcWebSocketProgramNotification = RpcWebSocketSubscriptionNotification<
...
export type RpcWebSocketSignatureNotification = RpcWebSocketSubscriptionNotification<
...
export type RpcWebSocketSlotsUpdatesNotification = RpcWebSocketSubscriptionNotification<RpcWebSocketSlotUpdate>;

export type RpcWebSocketRootNotification = RpcWebSocketSubscriptionNotification<Slot>;

export type RpcWebSocketBlockNotification = RpcWebSocketSubscriptionNotification<
    SolanaRpcResponse<
        Readonly<{
            block: object | null;
            err: string | null;
...
export type AnyRpcWebSocketNotification = RpcWebSocketNotificationByKind[RpcWebSocketNotificationKind];

type BlockSubscriptionOptions = Parameters<UnstableSubscriptions['blockNotifications']>[1];

type LogsSubscriptionOptions = Parameters<StableSubscriptions['logsNotifications']>[1];

type SignatureSubscriptionOptions = Parameters<StableSubscriptions['signatureNotifications']>[1];
...
function resolveSubscriptionChannelConfig(config?: SubscriptionChannelConfig): ResolvedSubscriptionChannelConfig {
    return Object.freeze({
        intervalMs: config?.intervalMs ?? DEFAULT_SUBSCRIPTION_CHANNEL_CONFIG.intervalMs,
        maxSubscriptionsPerChannel:
            config?.maxSubscriptionsPerChannel ?? DEFAULT_SUBSCRIPTION_CHANNEL_CONFIG.maxSubscriptionsPerChannel,
        minChannels: config?.minChannels ?? DEFAULT_SUBSCRIPTION_CHANNEL_CONFIG.minChannels,
    });
}
...
type NotificationStream<TResult> = {
    subscribe(config: Readonly<{ abortSignal: AbortSignal }>): Promise<AsyncIterable<TResult>>;
};

type NotificationOpener<TTarget, TConfig, TResult> = (target: TTarget, config?: TConfig) => NotificationStream<TResult>;
...
export type RpcWebSocketNotificationEvent<TKind extends RpcWebSocketNotificationKind = RpcWebSocketNotificationKind> =
...
export type ConnectionSubscriptionsNotificationDispatcher = (notification: RpcWebSocketNotificationEvent) => void;
...
export class KitSubscriptionRuntime<TBlockDispatchConfig> implements ConnectionSubscriptionsRuntime {
    ...
    private readonly _createSubscriptionChannel: ReturnType<typeof createSubscriptionChannel>;
    ...
    constructor(
        ...
        const resolvedSubscriptionChannelConfig = resolveSubscriptionChannelConfig(subscriptionChannelConfig);
        this._createSubscriptionChannel = createSubscriptionChannel(endpoint, resolvedSubscriptionChannelConfig);
        this._stableSubscriptions = createSolanaRpcSubscriptions(endpoint, resolvedSubscriptionChannelConfig);
    ...
    ensureConnected(): void {
        ...
        void this._createSubscriptionChannel({
            ...
            .then(channel => {
                if (this._channelAbortController !== abortController || abortController.signal.aborted) {
                    return;
                }
    ...
    openSubscription(spec: SubscriptionSpec): Promise<SubscriptionHandle> {
        const { abortController, serverSubscriptionId } = this._subscriptionRegistry.createServerSubscription();
        ...
        try {
            switch (spec.kind) {
                case 'account': {
                    ...
                    >;
                    const notificationStream = openAccountNotifications(typedAddress, spec.options).subscribe({
                        abortSignal,
                    });
                ...
                case 'block': {
                    ...
                    return this._createSubscriptionHandle(
                        this._unstableSubscriptions.blockNotifications(filter, spec.options).subscribe({ abortSignal }),
                        serverSubscriptionId,
                        abortController,
                        result => {
                            this._dispatchNotification({
                ...
                case 'logs': {
                    const openLogsNotifications = this._stableSubscriptions.logsNotifications as NotificationOpener<
                ...
                case 'program': {
                    ...
                    >;
                    const notificationStream = openProgramNotifications(typedAddress, spec.options).subscribe({
                        abortSignal,
                    });
                    return this._createSubscriptionHandle(
                        notificationStream,
                ...
                case 'root': {
                    return this._createSubscriptionHandle(
                        this._stableSubscriptions.rootNotifications().subscribe({ abortSignal }),
                        serverSubscriptionId,
                        abortController,
                        result => {
                            this._dispatchNotification({
                ...
                case 'slot': {
                    return this._createSubscriptionHandle(
                        this._stableSubscriptions.slotNotifications().subscribe({ abortSignal }),
                        serverSubscriptionId,
                        abortController,
                        result => {
                            this._dispatchNotification({
                ...
                case 'slotsUpdates': {
                    return this._createSubscriptionHandle(
                        this._unstableSubscriptions.slotsUpdatesNotifications().subscribe({ abortSignal }),
                        serverSubscriptionId,
                        abortController,
                        result => {
                            this._dispatchNotification({
                ...
                case 'vote': {
                    return this._createSubscriptionHandle(
                        this._unstableSubscriptions.voteNotifications().subscribe({ abortSignal }),
                        serverSubscriptionId,
                        abortController,
                        result => {
                            this._dispatchNotification({
    ...
    private async _createSubscriptionHandle<TResult>(
            ...
            try {
                for await (const result of notificationStream) {
                    if (
                        ...
                        !this._subscriptionRegistry.hasServerSubscription(serverSubscriptionId)
                    ) {
                        return;
                    }
                ...
                if (this._channel !== null) {
                    this._disconnectChannel(SUBSCRIPTION_CHANNEL_CLOSE_CODE_UNEXPECTED, normalizedError);
                } else {
        ...
        return {
            serverSubscriptionId,
            unsubscribe: () =>
                Promise.resolve(this._subscriptionRegistry.abortServerSubscription(serverSubscriptionId)),
        };
    }
