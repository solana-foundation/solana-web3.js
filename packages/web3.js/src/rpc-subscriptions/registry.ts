...
export type ObservedSubscriptionState = StatefulSubscription['state'] | 'inactive';

export type SubscriptionStateChangeCallback = (nextState: ObservedSubscriptionState) => void;
...
export type SubscriptionConfigByKind<TKind extends SubscriptionKind = SubscriptionKind> = Readonly<{
    callback: SubscriptionCallback;
    spec: SubscriptionSpecByKind[TKind];
}>;
...
export type PendingSubscription<TKind extends SubscriptionKind = SubscriptionKind> = BaseSubscription<TKind> &
...
export class ConnectionSubscriptionRegistry<TBlockDispatchConfig> {
    private readonly _blockDispatchConfigByHash = new Map<SubscriptionConfigHash, TBlockDispatchConfig>();
    private readonly _blockDispatchConfigByServerId = new Map<ServerSubscriptionId, TBlockDispatchConfig>();
    private readonly _clientSubscriptionsById = new Map<ClientSubscriptionId, ClientSubscriptionRecord>();
    private _nextClientSubscriptionId = 0;
    private _nextServerSubscriptionId = 0;
    private readonly _abortControllersByServerId = new Map<ServerSubscriptionId, AbortController>();
    private readonly _callbacksByServerId = new Map<ServerSubscriptionId, Set<SubscriptionConfig['callback']>>();
    ...
    private readonly _stateChangeCallbacksByClientSubscriptionId = new Map<
        ...
        Set<SubscriptionStateChangeCallback>
    >();
    private readonly _autoDisposedServerSubscriptions = new Set<ServerSubscriptionId>();
    private readonly _subscriptionsByHash = new Map<SubscriptionConfigHash, Subscription>();
    ...
    addSubscriptionCallback<TKind extends SubscriptionKind>(
        ...
        (existingSubscription.callbacks as Set<SubscriptionConfig['callback']>).add(callback);
    ...
    consumeAutoDisposedSubscription(serverSubscriptionId: ServerSubscriptionId): boolean {
    ...
    createClientSubscription(
        ...
        const clientSubscriptionId = this._nextClientSubscriptionId++ as ClientSubscriptionId;
    ...
    createServerSubscription(): Readonly<{
        ...
        const serverSubscriptionId = ++this._nextServerSubscriptionId as ServerSubscriptionId;
    ...
    dispatchNotification<TActualCallback extends (...args: any[]) => void>(
        ...
        const callbacks = this._callbacksByServerId.get(serverSubscriptionId) as Set<TActualCallback> | undefined;
        if (callbacks == null) {
            return;
        }
    ...
    getBlockDispatchConfig(hash: SubscriptionConfigHash): TBlockDispatchConfig | undefined {
        return this._blockDispatchConfigByHash.get(hash);
    }

    getBlockDispatchConfigForServerId(serverSubscriptionId: ServerSubscriptionId): TBlockDispatchConfig | undefined {
    ...
    markAutoDisposedSubscription(serverSubscriptionId: ServerSubscriptionId): void {
    ...
    observeStateChanges(
        ...
        const stateChangeCallbacks = this._stateChangeCallbacksByHash.get(hash) ?? new Set();
        this._stateChangeCallbacksByHash.set(hash, stateChangeCallbacks);
        stateChangeCallbacks.add(callback);

        const clientStateChangeCallbacks =
            this._stateChangeCallbacksByClientSubscriptionId.get(clientSubscriptionId) ?? new Set();
        this._stateChangeCallbacksByClientSubscriptionId.set(clientSubscriptionId, clientStateChangeCallbacks);
        clientStateChangeCallbacks.add(callback);

        const currentState = this._subscriptionsByHash.get(hash)?.state ?? 'inactive';
        ...
        return {
            ...
            dispose: () => {
                ...
                if (clientStateChangeCallbacks.size === 0) {
                    this._stateChangeCallbacksByClientSubscriptionId.delete(clientSubscriptionId);
                }
            },
        };
    }
    ...
    abortServerSubscription(serverSubscriptionId: ServerSubscriptionId): boolean {
        const abortController = this._abortControllersByServerId.get(serverSubscriptionId);
    ...
    pruneSubscription(hash: SubscriptionConfigHash): void {
        ...
        if ('serverSubscriptionId' in subscription) {
            ...
            this._blockDispatchConfigByServerId.delete(subscription.serverSubscriptionId);
            this._autoDisposedServerSubscriptions.delete(subscription.serverSubscriptionId);
        }
    ...
    removeClientSubscription(clientSubscriptionId: ClientSubscriptionId): ClientSubscriptionRecord | undefined {
        ...
        if (clientSubscription != null) {
            ...
            const clientStateChangeCallbacks =
                this._stateChangeCallbacksByClientSubscriptionId.get(clientSubscriptionId);
            if (clientStateChangeCallbacks != null) {
                this._stateChangeCallbacksByClientSubscriptionId.delete(clientSubscriptionId);
                const stateChangeCallbacks = this._stateChangeCallbacksByHash.get(clientSubscription.hash);
    ...
    removeSubscriptionCallback(hash: SubscriptionConfigHash, callback: SubscriptionConfig['callback']): boolean {
        const subscription = this._subscriptionsByHash.get(hash);
        if (subscription == null) {
            return false;
        }
        (subscription.callbacks as Set<SubscriptionConfig['callback']>).delete(callback);
        return true;
    }

    setDefaultBlockDispatchConfig(hash: SubscriptionConfigHash, dispatchConfig: TBlockDispatchConfig): void {
        if (!this._blockDispatchConfigByHash.has(hash)) {
            this._blockDispatchConfigByHash.set(hash, dispatchConfig);
        }
    }

    setBlockDispatchConfig(hash: SubscriptionConfigHash, dispatchConfig: TBlockDispatchConfig): void {
        this._blockDispatchConfigByHash.set(hash, dispatchConfig);
    }

    setSubscription<TKind extends SubscriptionKind, TState extends StatefulSubscription>(
        hash: SubscriptionConfigHash,
        ...
        if (
            ...
            (nextSubscription.state === 'unsubscribed' ||
                ...
                nextSubscription.serverSubscriptionId !== prevSubscription.serverSubscriptionId)
        ) {
            this._callbacksByServerId.delete(prevSubscription.serverSubscriptionId);
            this._blockDispatchConfigByServerId.delete(prevSubscription.serverSubscriptionId);
            this._autoDisposedServerSubscriptions.delete(prevSubscription.serverSubscriptionId);
        }
        this._subscriptionsByHash.set(hash, nextSubscription as Subscription);
        if ('serverSubscriptionId' in nextSubscription) {
            this._callbacksByServerId.set(nextSubscription.serverSubscriptionId, nextSubscription.callbacks);
        }
