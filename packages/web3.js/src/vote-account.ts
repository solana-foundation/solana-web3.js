...
import { coerceNullableNumericToBigInt, coerceNumericToBigInt } from './utils/bigint';
...
export type VoteStateVersion = (typeof VoteStateVersion)[keyof typeof VoteStateVersion];
...
const VOTE_ACCOUNT_V1_14_11_CODEC = getStructCodec([
    ...
    ['authorizedVoters', getArrayCodec(AUTHORIZED_VOTER_CODEC, { size: U64_BIGINT_CODEC })],
    ['priorVoters', PRIOR_VOTERS_CODEC],
    ['epochCredits', getArrayCodec(EPOCH_CREDITS_CODEC, { size: U64_BIGINT_CODEC })],
...
function normalizePriorVoters({ buf, idx, isEmpty }: PriorVoters): PriorVoterRaw[] {
    if (idx < 0n || idx >= BigInt(PRIOR_VOTERS_COUNT)) {
        throw new Error('Invalid vote account data: prior voters index out of range');
    }
    ...
    return [...buf.slice(normalizedIndex + 1), ...buf.slice(0, normalizedIndex + 1)];
}
...
function decodeVoteAccountDataV1_14_11(decoded: VoteAccountV1_14_11CodecValue): DecodedVoteAccountData {
    return {
...
function decodeVoteAccountDataV3(decoded: VoteAccountV3CodecValue): DecodedVoteAccountData {
    return {
        nodePubkey: decoded.nodePubkey,
        authorizedWithdrawer: decoded.authorizedWithdrawer,
        commission: decoded.commission,
...
function decodeVoteAccountDataV4(decoded: VoteAccountV4CodecValue): DecodedVoteAccountData {
    return {
        nodePubkey: decoded.nodePubkey,
        authorizedWithdrawer: decoded.authorizedWithdrawer,
...
const decodeVoteAccountData = (bytes: Uint8Array): DecodedVoteAccountData => {
    ...
    switch (version) {
        case VoteStateVersion.V1_14_11:
            return decodeVoteAccountDataV1_14_11(VOTE_ACCOUNT_V1_14_11_CODEC.decode(bytes, U32_CODEC.fixedSize));
        case VoteStateVersion.V3:
            return decodeVoteAccountDataV3(VOTE_ACCOUNT_V3_CODEC.decode(bytes, U32_CODEC.fixedSize));
        case VoteStateVersion.V4:
            return decodeVoteAccountDataV4(VOTE_ACCOUNT_V4_CODEC.decode(bytes, U32_CODEC.fixedSize));
        case 0:
...
export class VoteAccount {
    ...
    static fromAccountData(bufferLike: Uint8Array | Array<number>): VoteAccount {
        ...
        return new VoteAccount({
            ...
            blockRevenueCollector: va.blockRevenueCollector ? new PublicKey(va.blockRevenueCollector) : null,
...
function parseAuthorizedVoter({ authorizedVoter, epoch }: AuthorizedVoterRaw): AuthorizedVoter {
    return {
        epoch,
...
function parsePriorVoters({ authorizedPubkey, epochOfLastAuthorizedSwitch, targetEpoch }: PriorVoterRaw): PriorVoter {
    return {
