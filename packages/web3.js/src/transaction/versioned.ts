...
import { SIGNATURE_LENGTH_IN_BYTES, V1_MESSAGE_PREFIX, VERSION_PREFIX_MASK } from './constants';
...
const VERSIONED_TRANSACTION_ENCODER = getStructEncoder([
    ['signatures', getArrayEncoder(SIGNATURE_ENCODER, { size: getShortU16Encoder() })],
    ['serializedMessage', getBytesEncoder()],
]);
const VERSIONED_TRANSACTION_DECODER = getStructDecoder([
    ['signatures', getArrayDecoder(SIGNATURE_DECODER, { size: getShortU16Decoder() })],
...
export class VersionedTransaction {
    ...
    serialize(): Uint8Array {
        ...
        if (this.message.version === 1) {
            const serializedTransaction = new Uint8Array(
                serializedMessage.length + this.signatures.length * SIGNATURE_LENGTH_IN_BYTES,
            );
            serializedTransaction.set(serializedMessage, 0);
            this.signatures.forEach((signature, index) => {
                serializedTransaction.set(signature, serializedMessage.length + index * SIGNATURE_LENGTH_IN_BYTES);
            });
    ...
    static deserialize(serializedTransaction: Uint8Array): VersionedTransaction {
        ...
        if ((prefix & ~VERSION_PREFIX_MASK) !== 0) {
            if (prefix !== V1_MESSAGE_PREFIX) {
                if ((prefix & VERSION_PREFIX_MASK) === 0) {
                    throw new Error('Version 0 transactions must be serialized signatures first');
                }
                throw new Error(`Invalid transaction discriminator ${prefix}`);
            }
            const numSignatures = serializedTransaction[1];
            const messageLength = serializedTransaction.length - numSignatures * SIGNATURE_LENGTH_IN_BYTES;
            assert(messageLength > 0, 'Transaction is too short for the number of signatures it requires');
            const message = VersionedMessage.deserialize(serializedTransaction.subarray(0, messageLength));
            const signatures = [];
            for (let i = 0; i < numSignatures; i++) {
                const offset = messageLength + i * SIGNATURE_LENGTH_IN_BYTES;
                signatures.push(
                    Uint8Array.from(serializedTransaction.subarray(offset, offset + SIGNATURE_LENGTH_IN_BYTES)),
                );
            }
            return new VersionedTransaction(message, signatures);
        }

        const { serializedMessage, signatures } = VERSIONED_TRANSACTION_DECODER.decode(serializedTransaction);
        const message = VersionedMessage.deserialize(Uint8Array.from(serializedMessage));
        assert(message.version !== 1, 'Invalid message version for a signatures-first transaction');
    ...
    async sign(signers: Array<MessagePartialSigner>) {
        ...
        for (const signer of signers) {
            ...
            const signerIndex = signerPubkeys.findIndex(pubkey => pubkey.equals(signerPublicKey));
            assert(signerIndex >= 0, `Cannot sign with non signer key ${signerPublicKey.toBase58()}`);
            ...
            const signature = await signTransactionMessageBytes(signer, messageData, signerPubkeys);
