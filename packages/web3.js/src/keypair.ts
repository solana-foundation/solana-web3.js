...
/**
 * A Kit signer that can sign both transactions and arbitrary messages.
 */
...
export class Keypair implements KeyPairSigner {
    ...
    private constructor(signer: KeyPairSigner<Address>, privateKeyBytes: Uint8Array, publicKeyBytes: Uint8Array) {
        this.#signer = signer;
     ...
     */
    signMessages: MessagePartialSigner<Address>['signMessages'] = (messages, config) =>
     ...
     */
    signTransactions: TransactionPartialSigner<Address>['signTransactions'] = (transactions, config) =>
    ...
    async signBytes(message: Uint8Array): Promise<Uint8Array> {
        return await signBytes(this.#signer.keyPair.privateKey, toPackedUint8Array(message));
    }
     ...
     * public key.
     */
    async verifySignature(signature: Uint8Array, message: Uint8Array): Promise<boolean> {
