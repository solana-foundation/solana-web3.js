...
type V1Compiled = V1CompiledTransactionMessage & CompiledTransactionMessageWithLifetime;

/** A single config value as encoded in a compiled v1 message. */
type CompiledTransactionConfigValue = V1CompiledTransactionMessage['configValues'][number];
 ...
 */
function getTransactionConfigValues(config: V1TransactionConfig): CompiledTransactionConfigValue[] {
...
function decompileTransactionConfig(
    ...
    const supportedConfigs: Array<[keyof V1TransactionConfig, 'u32' | 'u64', (mask: number) => boolean]> = [
    ...
    for (const [name, kind, predicate] of supportedConfigs) {
        ...
        (config[name] as CompiledTransactionConfigValue['value']) = configValue.value;
    }
    return config;
}
...
export class MessageV1 {
    ...
    isAccountWritable(index: number): boolean {
        ...
        const numWritableSignedAccounts = numSigners - this.header.numReadonlySignedAccounts;
        const numWritableUnsignedAccounts =
            this.staticAccountKeys.length - numSigners - this.header.numReadonlyUnsignedAccounts;
        return (
    ...
    static compile(args: CompileV1Args): MessageV1 {
        const instructions = expandInstructionPlans(args.instructions).map(instruction =>
            isKitInstruction(instruction) ? toLegacyInstructionFields(instruction) : instruction,
        );
    ...
    static fromCompiledMessage(decoded: V1Compiled): MessageV1 {
        const compiledInstructions = decoded.instructionHeaders.map((instructionHeader, i) => {
            ...
            return {
                ...
                data: payload.instructionData ? Uint8Array.from(payload.instructionData) : new Uint8Array(0),
            };
        });
        const decompiledConfig = decompileTransactionConfig(decoded.configMask, decoded.configValues);
        const transactionConfig = isV1ConfigEmpty(decompiledConfig) ? undefined : decompiledConfig;
        return new MessageV1({
            header: {
            ...
            staticAccountKeys: decoded.staticAccounts.map(addr => new PublicKey(addr)),
