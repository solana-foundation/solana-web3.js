...
type V0Compiled = Extract<CompiledTransactionMessage, { version: 0 }> & CompiledTransactionMessageWithLifetime;
...
export class MessageV0 {
    ...
    static deserialize(serializedMessage: Uint8Array): MessageV0 {
        ...
        if (maskedPrefix !== 0) {
            throw new Error(`Expected versioned message with version 0 but found version ${maskedPrefix}`);
        }
        const decoded = MESSAGE_DECODER.decode(toUint8ArrayView(serializedMessage));
        if (decoded.version !== 0) {
            throw new Error(`Expected versioned message with version 0 but found version ${decoded.version}`);
        }
