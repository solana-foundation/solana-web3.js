...
export const VALIDATOR_INFO_KEY = new PublicKey('Va1idator1nfo111111111111111111111111111111');
...
function parseOptionalStringField(info: Record<string, unknown>, key: keyof Info): string | undefined {
    const value = info[key];
    assert(value === undefined || typeof value === 'string', `Expected validator info field "${key}" to be a string`);
    return value;
}

function parseInfo(value: unknown): Info {
    assert(isRecord(value), 'Expected validator info to be an object');
    assert(typeof value.name === 'string', 'Expected validator info field "name" to be a string');
...
export class ValidatorInfo {
    ...
    static fromConfigData(buffer: Uint8Array | Array<number>): ValidatorInfo | null {
        const { configKeys: decodedConfigKeys, info: rawInfo } = VALIDATOR_INFO_CONFIG_DECODER.decode(
