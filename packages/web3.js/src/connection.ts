...
import { type CompiledInstruction, Message, type V1TransactionConfig, VersionedMessage } from './message';
...
import { Transaction, TransactionStatus, TransactionVersion, VersionedTransaction } from './transaction';
...
import { coerceNumericToBigInt, coerceOptionalNumericToBigInt } from './utils/bigint';
...
): asserts signatures is Parameters<GetSignatureStatusesApi['getSignatureStatuses']>[0] {
 ...
 */
export type SendOptions = Omit<NonNullable<Parameters<SendTransactionApi['sendTransaction']>[1]>, 'encoding'>;
...
type GetBlocksWithLimitResult = ReturnType<GetBlocksWithLimitApi['getBlocksWithLimit']>;
type GetLatestBlockhashKitResult = ReturnType<GetLatestBlockhashApi['getLatestBlockhash']>;
type GetSignaturesForAddressKitResult = ReturnType<GetSignaturesForAddressApi['getSignaturesForAddress']>;
 ...
 */
export type BlockheightBasedTransactionConfirmationStrategy = BaseTransactionConfirmationStrategy &
    Readonly<Overwrite<BlockhashWithExpiryBlockHeight, { lastValidBlockHeight: number | bigint }>>;

/**
 * A strategy for confirming durable nonce transactions.
 */
export type DurableNonceTransactionConfirmationStrategy = BaseTransactionConfirmationStrategy & {
    /**
 ...
 * @internal
 */
function coerceToBase64EncodedWireTransaction(transaction: string): Base64EncodedWireTransaction {
 ...
 */
type GetVoteAccountsKitResult = ReturnType<GetVoteAccountsApi['getVoteAccounts']>;
 ...
 */
export type InflationReward = NonNullable<ReturnType<GetInflationRewardApi['getInflationReward']>[number]>;
...
type RpcPartiallyDecodedInstruction = Exclude<RpcParsedMessageInstruction, Readonly<{ parsed: unknown }>>;

type RpcParsedInstruction = Extract<RpcParsedMessageInstruction, Readonly<{ parsed: unknown }>>;

type RpcParsedMessageAccount = TransactionForAccounts<0 | 1>['transaction']['accountKeys'][number];

type RpcParsedAddressTableLookup = NonNullable<
    NonNullable<TransactionForFullJson<0 | 1>['transaction']['message']['addressTableLookups']>
>[number];
...
type BlockResponseTransactionMeta = NonNullable<TransactionForFullJson<void>['meta']> & {
    costUnits?: bigint;
};
...
type AccountsModeBlockResponseTransactionMeta = NonNullable<TransactionForAccounts<0 | 1>['meta']> & {
    costUnits?: bigint;
};
...
type OptionalBlockSubscriptionLoadedAddresses = {
    loadedAddresses?: NonNullable<TransactionForFullJson<0 | 1>['meta']>['loadedAddresses'];
};

type BlockSubscriptionTransactionMeta = Overwrite<
    NonNullable<TransactionForFullJson<0 | 1>['meta']>,
    NormalizedBlockSubscriptionMetaFields & OptionalBlockSubscriptionLoadedAddresses
> &
 ...
 */
export type ParsedAccountsModeBlockResponse = Omit<ParsedBlockResponse, 'transactions'> & {
    transactions: Array<
        Overwrite<
 ...
 */
export type ParsedNoneModeBlockResponse = Omit<ParsedBlockResponse, 'transactions'>;

/**
 * A block with parsed transactions where the `transactionDetails` mode is `signatures`
 */
export type ParsedSignaturesModeBlockResponse = Omit<ParsedBlockResponse, 'transactions'> & {
    signatures: Array<string>;
};
 ...
 * A processed block fetched from the RPC API where the `transactionDetails` mode is `accounts`
 */
export type VersionedAccountsModeBlockResponse = Omit<VersionedBlockResponse, 'transactions'> & {
    transactions: Array<
        Overwrite<
            TransactionForAccounts<0 | 1>,
            {
                transaction: Pick<VersionedTransactionResponse['transaction'], 'signatures'> & {
                    accountKeys: ParsedMessageAccount[];
                };
                meta: AccountsModeBlockResponseTransactionMeta | null;
                version?: TransactionVersion;
 ...
 * A processed block fetched from the RPC API where the `transactionDetails` mode is `none`
 */
export type VersionedNoneModeBlockResponse = Omit<VersionedBlockResponse, 'transactions'>;

/**
 * A processed block fetched from the RPC API where the `transactionDetails` mode is `signatures`
 */
export type VersionedSignaturesModeBlockResponse = Omit<VersionedBlockResponse, 'transactions'> & {
    signatures: Array<string>;
};
 ...
 */
export type BlockProduction = ReturnType<GetBlockProductionApi['getBlockProduction']>['value'];
...
type ConnectionHttpTransportFetch = (url: string, requestInfo: ConnectionHttpRequestInit) => Promise<Response>;
...
function createConnectionHttpTransport(
    ...
    return async <TResponse>({
        ...
        return (rawResponse.length === 0 ? null : parseJsonWithBigInts(rawResponse)) as TResponse;
    };
}

function createRpcTransport(url: string, config: RpcTransportConfig = {}): RpcTransport {
    ...
    return async <TResponse>({
        ...
        for (;;) {
            ...
            console.error(`Server responded with 429 Too Many Requests.  Retrying after ${waitTime}ms delay...`);
...
function createFetchRpcTransport(
    ...
    const callFetch =
        fetchMiddleware == null
            ? async (requestUrl: string, options: ConnectionHttpRequestInit) => await fetch(requestUrl, options)
            : async (requestUrl: string, options: ConnectionHttpRequestInit) => {
                  const [modifiedUrl, modifiedOptions] = await new Promise<[string, ConnectionHttpRequestInit]>(
                      (resolve, reject) => {
                          try {
...
function extractSendTransactionErrorDetails(error: unknown): { logs?: string[]; transactionMessage: string } | null {
    if (isSolanaError(error, SOLANA_ERROR__JSON_RPC__SERVER_ERROR_SEND_TRANSACTION_PREFLIGHT_FAILURE)) {
        const logs =
            error.context.logs == null
                ? undefined
                : error.context.logs.filter((log): log is string => typeof log === 'string');
        const transactionMessage =
            logs?.at(-1) ?? (error.cause instanceof Error ? error.cause.message : undefined) ?? error.message;
    ...
    if ('data' in error && isRecord(error.data)) {
        ...
        if (Array.isArray(maybeLogs) && maybeLogs.every(log => typeof log === 'string')) {
            logs = maybeLogs;
        }
    }
...
export type Supply = Overwrite<GetSupplyKitResult['value'], { nonCirculatingAccounts: Array<PublicKey> }>;
...
type GetTokenLargestAccountsWithPublicKeys = Overwrite<
    ...
    {
        value: ReadonlyArray<Overwrite<GetTokenLargestAccountsKitResult['value'][number], { address: PublicKey }>>;
    }
>;
...
type GetLargestAccountsWithPublicKeys = Overwrite<
    ...
    {
        value: ReadonlyArray<Overwrite<GetLargestAccountsKitResult['value'][number], { address: PublicKey }>>;
    }
>;
...
type SimulatedAccountInfoLike = Readonly<KitRawBase64AccountInfo & { rentEpoch?: unknown }>;
...
type TypedAccountsModeBlockSource = RpcBlockLike & {
    transactions: readonly (TransactionForAccounts<void> | TransactionForAccounts<0 | 1>)[];
};

type TypedFullBlockSource = RpcBlockLike & {
    transactions: readonly (TransactionForFullJson<void> | TransactionForFullJson<0 | 1>)[];
};

type TypedParsedBlockSource = RpcBlockLike & {
    transactions: readonly (TransactionForFullJsonParsed<void> | TransactionForFullJsonParsed<0 | 1>)[];
};
...
type TypedTransactionSource = Readonly<{
    blockTime: number | bigint | null;
    meta: TransactionForFullJson<void>['meta'] | TransactionForFullJson<0 | 1>['meta'];
    slot: number | bigint;
    transaction: TransactionForFullJson<void>['transaction'] | TransactionForFullJson<0 | 1>['transaction'];
    version?: TransactionVersion;
}>;

type TypedParsedTransactionSource = Readonly<{
    blockTime: number | bigint | null;
    meta: TransactionForFullJsonParsed<void>['meta'] | TransactionForFullJsonParsed<0 | 1>['meta'];
    slot: number | bigint;
    transaction: TransactionForFullJsonParsed<void>['transaction'] | TransactionForFullJsonParsed<0 | 1>['transaction'];
    version?: TransactionVersion;
}>;
...
type TypedBlockMappers<
    ...
    TFullBlockResult,
> = Readonly<{
    mapAccountsBlock: (block: TypedAccountsModeBlockSource) => TAccountsBlockResult;
...
>(
    ...
    args: TypedBlockMappers<TAccountsBlockResult, TFullBlockSource, TFullBlockResult>,
): Promise<
...
> {
    switch (config?.transactionDetails) {
        ...
        case 'accounts': {
            ...
            const result = await sendTypedBlockRequest<TypedAccountsModeBlockSource>(typedRpc, slot, accountsConfig);
            return result ? args.mapAccountsBlock(result) : null;
        }
        default: {
            const result = await sendTypedBlockRequest<TFullBlockSource>(typedRpc, slot, args.fullConfig);
...
function sendTypedTransactionRequest<TResponse>(
    ...
    const getTransaction = typedRpc.getTransaction as TypedRpcRequestMethod<
        [signature: Signature, config?: TypedTransactionConfig | TypedParsedTransactionConfig],
        TResponse | null
    >;
...
async function fetchTransactionsBySignature<TResponse>(
    ...
    fetcher: (signature: TransactionSignature, config: GetVersionedTransactionConfig | undefined) => Promise<TResponse>,
): Promise<TResponse[]> {
    return await Promise.all(signatures.map(signature => fetcher(signature, config)));
}
...
type AnySignatureSubscriptionCallback = SignatureResultCallback | SignatureSubscriptionCallback;
type AnyAccountChangeCallback = AccountChangeCallback | Base64ZstdAccountChangeCallback | ParsedAccountChangeCallback;
...
type RpcTransportConfig = Readonly<
    Pick<ConnectionConfig, 'commitment' | 'disableRetryOnRateLimit' | 'fetch' | 'fetchMiddleware' | 'httpHeaders'>
>;
...
export class Connection {
    ...
    constructor(endpoint: string, commitmentOrConfig?: Commitment | ConnectionConfig) {
        ...
        if (commitmentOrConfig && typeof commitmentOrConfig === 'string') {
            ...
            subscriptionChannelConfig = commitmentOrConfig.subscriptions?.channelConfig;
        ...
        const dispatchSubscriptionNotification: ConnectionSubscriptionsNotificationDispatcher = notificationEvent => {
    ...
    async getBalanceAndContext(
        ...
        const minContextSlot = coerceOptionalNumericToBigInt(config?.minContextSlot, 'minContextSlot');
        ...
        try {
            ...
            throwSolanaRpcErrorIfNeeded(error, `failed to get balance for ${publicKey.toBase58()}`);
        }
    }
    ...
    async getBalance(
        ...
        return (await this.getBalanceAndContext(publicKey, commitmentOrConfig)).value;
    }

    /**
     * Fetch the estimated production time of a block
     */
    async getBlockTime(slot: number | bigint): Promise<ReturnType<GetBlockTimeApi['getBlockTime']>> {
        try {
     ...
     */
    async getMinimumLedgerSlot(): Promise<ReturnType<MinimumLedgerSlotApi['minimumLedgerSlot']>> {
        try {
     ...
     */
    async getFirstAvailableBlock(): Promise<ReturnType<GetFirstAvailableBlockApi['getFirstAvailableBlock']>> {
        try {
     ...
     */
    async getSupply(config?: GetSupplyConfig | Commitment): Promise<GetSupplyResult> {
    ...
    async getTokenAccountsByOwner(
        ...
        try {
            ...
            return {
                ...
                value: mapKeyedBase64AccountInfos(response.value, 'Expected token account rentEpoch'),
            };
        } catch (error) {
    ...
    async getTokenAccountsByDelegate(
        ...
        try {
            ...
            return {
                context: response.context,
                value: mapKeyedBase64AccountInfos(response.value, 'Expected delegated token account rentEpoch'),
            };
        } catch (error) {
            throwSolanaRpcErrorIfNeeded(
                error,
    ...
    > {
        ...
        try {
            ...
            return {
                context: response.context,
                value: mapKeyedParsedAccountInfos(response.value, 'Expected token account rentEpoch'),
            };
        } catch (error) {
            throwSolanaRpcErrorIfNeeded(
                error,
     ...
     */
    async getLargestAccounts(config?: GetLargestAccountsConfig): Promise<GetLargestAccountsWithPublicKeys> {
        ...
        try {
            ...
            const response = await this._typedRpc
                .getAccountInfo(typedPublicKey, {
                    ...
                    dataSlice: config?.dataSlice,
                    encoding: 'base64',
                    minContextSlot: coerceOptionalNumericToBigInt(minContextSlot, 'minContextSlot'),
                })
                .send();

            return {
                context: response.context,
                value: mapBase64AccountInfo(response.value, 'Expected raw account info rentEpoch'),
            };
        } catch (error) {
            throwSolanaRpcErrorIfNeeded(error, `failed to get info about account ${publicKey.toBase58()}`);
        }
    }
    ...
    async getParsedAccountInfo(
        publicKey: PublicKey,
        commitmentOrConfig?: Commitment | GetAccountInfoConfig,
    ): Promise<RpcResponseAndContext<AccountInfoWithSpace<Uint8Array | ParsedAccountData> | null>> {
        try {
            const { commitment, config } = extractCommitmentFromConfig(commitmentOrConfig);
            const typedPublicKey = publicKey.toBase58();
            const rpcCommitment = this._resolveCommitment(commitment);
            ...
            return {
                context: response.context,
                value: mapJsonParsedAccountInfo(response.value, 'Expected parsed account info rentEpoch'),
            };
        } catch (error) {
            throwSolanaRpcErrorIfNeeded(error, `failed to get info about account ${publicKey.toBase58()}`);
        }
    ...
    async getAccountInfo(
        ...
        try {
            ...
            return mapBase64AccountInfo(response.value, 'Expected raw account info rentEpoch');
        } catch (e) {
            throw new Error('failed to get info about account ' + publicKey.toBase58() + ': ' + String(e));
        }
    }
    ...
    async getProgramAccounts(
        ...
        const minContextSlot = coerceOptionalNumericToBigInt(configWithoutEncoding.minContextSlot, 'minContextSlot');
        ...
        try {
            if (configWithoutEncoding.withContext === true) {
                ...
                return {
                    context: response.context,
                    value: mapKeyedBase64AccountInfos(response.value, 'Expected program account rentEpoch'),
                };
            }

            const response = await this._typedRpc.getProgramAccounts(typedProgramId, rpcConfig).send();

            return mapKeyedBase64AccountInfos(response, 'Expected program account rentEpoch');
        } catch (error) {
            throwSolanaRpcErrorIfNeeded(error, `failed to get accounts owned by program ${programId.toBase58()}`);
        }
    }
    ...
    > {
        ...
        try {
            ...
            return mapKeyedJsonParsedAccountInfos(response, 'Expected program account rentEpoch');
        } catch (error) {
            throwSolanaRpcErrorIfNeeded(error, `failed to get accounts owned by program ${programId.toBase58()}`);
        }
    }
    ...
    private _resolveSupportedFinality(requestedCommitment?: Commitment): Finality {
    ...
    private _resolveSubscriptionCommitment(requestedCommitment: Commitment | undefined): Commitment {
    ...
    private _resolveSubscriptionConfig<TConfig extends { commitment?: Commitment }>(
        ...
        const { commitment, config } = extractCommitmentFromConfig<TConfig>(commitmentOrConfig);
        return {
            ...config,
    ...
    private getTransactionConfirmationPromise({
        ...
        let disposeSignatureSubscriptionStateChangeObserver: (() => void) | undefined;
        let done = false;
        const confirmationPromise = new Promise<{
            ...
            try {
                ...
                const subscriptionSetupPromise = new Promise<void>(resolveSubscriptionSetup => {
                    if (signatureSubscriptionId == null) {
                        resolveSubscriptionSetup();
                    } else {
                        const settleSubscriptionSetup = (nextState: ObservedSubscriptionState) => {
                            if (nextState !== 'failed' && nextState !== 'inactive' && nextState !== 'subscribed') {
                                return;
                            }
                        ...
                        const { currentState, dispose } = this._subscriptionRegistry.observeStateChanges(
                ...
                void (async () => {
                    try {
                        ...
                        if (value?.err) {
                            // oxlint-disable-next-line typescript/prefer-promise-reject-errors
                            reject(value.err);
                        } else if (confirmationStatusSatisfiesCommitment(commitment, value.confirmationStatus)) {
                            done = true;
                            resolve({
                                __type: TransactionStatus.PROCESSED,
                                response: {
    ...
    private async confirmTransactionUsingBlockHeightExceedanceStrategy({
        ...
        const lastValidBlockHeightBigInt = coerceNumericToBigInt(lastValidBlockHeight, 'lastValidBlockHeight');
        ...
        const expiryPromise = new Promise<{
            ...
            void (async () => {
                let currentBlockHeight = await Promise.race<bigint | typeof cancellationSentinel>([
                    checkBlockHeight(),
                ...
                while (currentBlockHeight <= lastValidBlockHeightBigInt) {
                    const sleepResult = await Promise.race<void | typeof cancellationSentinel>([
                        sleep(1000),
                        cancellationPromise.catch(() => cancellationSentinel),
                    ]);
                    if (done || sleepResult === cancellationSentinel) return;
                    currentBlockHeight = await Promise.race<bigint | typeof cancellationSentinel>([
                        checkBlockHeight(),
                        cancellationPromise.catch(() => cancellationSentinel),
                    ]);
                    if (done || currentBlockHeight === cancellationSentinel) return;
        ...
        try {
            ...
            if (outcome.__type === TransactionStatus.PROCESSED) {
                ...
                const signatureStatus = await Promise.race([this.getSignatureStatus(signature), cancellationPromise]);
                if (
                    signatureStatus?.value &&
                    confirmationStatusSatisfiesCommitment(commitment, signatureStatus.value.confirmationStatus, false)
                ) {
                    result = {
    ...
    private async confirmTransactionUsingDurableNonceStrategy({
        commitment,
        strategy: { abortSignal, minContextSlot, nonceAccountPubkey, nonceValue, signature },
    }: {
        commitment: Commitment;
        strategy: DurableNonceTransactionConfirmationStrategy;
    }) {
        let done: boolean = false;
        const nonceMinContextSlot = coerceOptionalNumericToBigInt(minContextSlot, 'minContextSlot');
        const cancellationPromise = this.getCancellationPromise(abortSignal);
        ...
        const expiryPromise = new Promise<{
            ...
            void (async () => {
                const initialNonceValue = await Promise.race<string | undefined | typeof cancellationSentinel>([
                ...
                while (true) {
                    ...
                    if (done || sleepResult === cancellationSentinel) return;
                    const nextNonceValue = await Promise.race<string | undefined | typeof cancellationSentinel>([
                        getCurrentNonceValue(),
                        cancellationPromise.catch(() => cancellationSentinel),
                    ]);
        ...
        try {
            ...
            if (outcome.__type === TransactionStatus.PROCESSED) {
                ...
                // Double check that the transaction is indeed unconfirmed.
                let signatureStatus: RpcResponseAndContext<SignatureStatus | null> | null | undefined;
                while (true) {
                    ...
                    if (status.context.slot < (outcome.slotInWhichNonceDidAdvance ?? nonceMinContextSlot ?? 0n)) {
                ...
                if (signatureStatus?.value) {
                    ...
                    if (!confirmationStatusSatisfiesCommitment(commitment, confirmationStatus, false)) {
     ...
     */
    async getHighestSnapshotSlot(): Promise<ReturnType<GetHighestSnapshotSlotApi['getHighestSnapshotSlot']>> {
        try {
     ...
     */
    async getMaxRetransmitSlot(): Promise<ReturnType<GetMaxRetransmitSlotApi['getMaxRetransmitSlot']>> {
        try {
     ...
     */
    async getMaxShredInsertSlot(): Promise<ReturnType<GetMaxShredInsertSlotApi['getMaxShredInsertSlot']>> {
        try {
     ...
     * Return the list of nodes that are currently participating in the cluster
     */
    async getVoteAccounts(commitmentOrConfig?: Commitment | GetVoteAccountsConfig): Promise<VoteAccountStatus> {
        const { commitment, config } = extractCommitmentFromConfig(commitmentOrConfig);
        const rpcCommitment = this._resolveCommitment(commitment);
        const typedVotePubkey = config?.votePubkey == null ? undefined : new PublicKey(config.votePubkey).toBase58();
        const delinquentSlotDistance =
            config?.delinquentSlotDistance == null
                ? undefined
                : coerceNumericToBigInt(config.delinquentSlotDistance, 'delinquentSlotDistance');
        const rpcConfig = {
            ...(rpcCommitment != null ? { commitment: rpcCommitment } : null),
     ...
     */
    async getSlot(commitmentOrConfig?: Commitment | GetSlotConfig): Promise<ReturnType<GetSlotApi['getSlot']>> {
        const { commitment, config } = extractCommitmentFromConfig(commitmentOrConfig);
        const rpcCommitment = this._resolveCommitment(commitment);
        const minContextSlot = config?.minContextSlot;
     ...
     */
    async getSlotLeader(commitmentOrConfig?: Commitment | GetSlotLeaderConfig): Promise<string> {
        const { commitment, config } = extractCommitmentFromConfig(commitmentOrConfig);
        const rpcCommitment = this._resolveCommitment(commitment);
        const minContextSlot = config?.minContextSlot;
     ...
     */
    async getSlotLeaders(startSlot: number | bigint, limit: number): Promise<Array<PublicKey>> {
        try {
            const response = await this._typedRpc
     ...
     */
    async getTransactionCount(commitmentOrConfig?: Commitment | GetTransactionCountConfig): Promise<bigint> {
        const { commitment, config } = extractCommitmentFromConfig(commitmentOrConfig);
        const rpcCommitment = this._resolveCommitment(commitment);
        const minContextSlot = config?.minContextSlot;
    ...
    async getInflationReward(
        ...
        const getInflationReward = this._typedRpc.getInflationReward as TypedRpcRequestMethod<
            [addresses: readonly Address[], config?: TypedInflationRewardRequestConfig],
     ...
     */
    async getEpochInfo(commitmentOrConfig?: Commitment | GetEpochInfoConfig): Promise<EpochInfo> {
        const { commitment, config } = extractCommitmentFromConfig(commitmentOrConfig);
        const rpcCommitment = this._resolveCommitment(commitment);
        const minContextSlot = config?.minContextSlot;
    ...
    async getLeaderSchedule(
        slotOrCommitmentOrConfig?: number | bigint | null | Commitment | GetLeaderScheduleConfig,
        ...
        const { commitment, config } = extractCommitmentFromConfig(rawCommitmentOrConfig);
        const rpcCommitment = this._resolveCommitment(commitment);
        ...
        const rpcConfig: TypedLeaderScheduleRequestConfig | undefined =
            rpcCommitment != null || rpcIdentity != null || rpcKeyByVoteAccount != null
                ? {
                      ...(rpcCommitment != null ? { commitment: rpcCommitment } : null),
                      ...(rpcIdentity != null ? { identity: rpcIdentity } : null),
                      ...(rpcKeyByVoteAccount != null ? { keyByVoteAccount: rpcKeyByVoteAccount } : null),
                  }
                : undefined;
        const getLeaderSchedule = this._typedRpc.getLeaderSchedule as TypedRpcRequestMethod<
            [slot?: Slot | null, config?: TypedLeaderScheduleRequestConfig],
            LeaderSchedule | null
        >;
        const rpcSlot =
            typeof slot === 'number' || typeof slot === 'bigint' ? coerceNumericToBigInt(slot, 'slot') : slot;
    ...
    ): Promise<ReturnType<GetMinimumBalanceForRentExemptionApi['getMinimumBalanceForRentExemption']>> {
        const { commitment } = extractCommitmentFromConfig(commitmentOrConfig);
        const rpcCommitment = this._resolveCommitment(commitment);
    ...
    async getRecentPerformanceSamples(
        ...
        try {
            return await (
                limit ? this._typedRpc.getRecentPerformanceSamples(limit) : this._typedRpc.getRecentPerformanceSamples()
            ).send();
        } catch (error) {
            ...
            const response = await (
                rpcCommitment == null && minContextSlot == null
                    ...
                    : this._typedRpc.getLatestBlockhash({
                          ...
                          ...(minContextSlot != null
                              ? {
                                    minContextSlot: coerceNumericToBigInt(minContextSlot, 'minContextSlot'),
                                }
                              : null),
                      })
            ).send();
     ...
     * have one.
     */
    async getAgGenesisCert(): Promise<ReturnType<GetAgGenesisCertApi['getAgGenesisCert']>> {
        try {
            return await this._typedRpc.getAgGenesisCert().send();
        } catch (error) {
            throwSolanaRpcErrorIfNeeded(error, 'failed to get Alpenglow genesis certificate');
        }
    }
     ...
     */
    async getGenesisHash(): Promise<ReturnType<GetGenesisHashApi['getGenesisHash']>> {
        try {
    ...
    async getBlock(slot: number | bigint, rawConfig?: GetBlockConfig): Promise<BlockResponse | null>;
    ...
    async getBlock(slot: number | bigint, rawConfig?: GetVersionedBlockConfig): Promise<VersionedBlockResponse | null>;
    ...
    > {
        ...
        const fullConfig = buildTypedFullBlockConfig(finality, config, rawConfig != null);
        ...
        try {
            ...
            >(this._typedRpc, rpcSlot, finality, config, {
                mapAccountsBlock: (result: TypedAccountsModeBlockSource) => ({
                    ...mapBlockBase(result),
                    transactions: mapTypedAccountsModeBlockTransactions(result.transactions),
                }),
                fullConfig,
            ...
            throw new SolanaJSONRPCError(e as JsonRpcErrorLike, 'failed to get confirmed block');
        }
    }
    ...
    > {
        ...
        try {
            ...
            throw new SolanaJSONRPCError(e as JsonRpcErrorLike, 'failed to get block');
        }
    }
    ...
    getBlockHeight = (() => {
        ...
        return (commitmentOrConfig?: Commitment | GetBlockHeightConfig): Promise<bigint> => {
            const { commitment, config } = extractCommitmentFromConfig(commitmentOrConfig);
            const rpcCommitment = this._resolveCommitment(commitment);
            const rpcMinContextSlot = coerceOptionalNumericToBigInt(config?.minContextSlot, 'minContextSlot');
            const rpcConfig =
                rpcCommitment == null && rpcMinContextSlot == null
                    ? undefined
                    : {
                          ...(rpcCommitment != null ? { commitment: rpcCommitment } : null),
                          ...(rpcMinContextSlot != null ? { minContextSlot: rpcMinContextSlot } : null),
                      };
            ...
            requestPromises[requestHash] =
                ...
                (async () => {
                    try {
                        ...
                        throwSolanaRpcErrorIfNeeded(error, 'failed to get block height information');
                    } finally {
    ...
    async getBlockProduction(
        ...
        const rpcConfig =
            rpcCommitment == null && rpcIdentity == null && rpcRange == null
                ...
                : {
                      ...(rpcCommitment != null ? { commitment: rpcCommitment } : null),
                      ...(rpcIdentity != null ? { identity: rpcIdentity as PublicKey } : null),
                      ...(rpcRange != null ? { range: rpcRange } : null),
                  };

        try {
            return await (
                rpcConfig == null ? this._typedRpc.getBlockProduction() : this._typedRpc.getBlockProduction(rpcConfig)
            ).send();
        } catch (error) {
     ...
     */
    async getTransaction(signature: string, rawConfig?: GetTransactionConfig): Promise<TransactionResponse | null>;
    ...
    async getTransaction(
        ...
        const { commitment, config } = extractCommitmentFromConfig(rawConfig);
        const typedConfig = buildTypedTransactionConfig(this._resolveSupportedFinality(commitment), config);
        try {
    ...
    async getParsedTransaction(
        ...
        const { commitment, config } = extractCommitmentFromConfig(commitmentOrConfig);
        const typedConfig = buildTypedParsedTransactionConfig(this._resolveSupportedFinality(commitment), config);
        try {
        ...
        const config =
            commitmentOrConfig == null
                ...
                : typeof commitmentOrConfig === 'string'
                  ? { commitment: commitmentOrConfig }
                  : commitmentOrConfig;
        return await fetchTransactionsBySignature(signatures, config, (signature, config) =>
            this.getParsedTransaction(signature, config),
        );
    }
        ...
        const config =
            commitmentOrConfig == null
                ...
                : typeof commitmentOrConfig === 'string'
                  ...
                  : commitmentOrConfig;
        return await fetchTransactionsBySignature(signatures, config, (signature, config) =>
            this.getTransaction(signature, config),
        );
    }
     ...
     */
    async getConfirmedBlock(slot: number | bigint, commitment?: Finality): Promise<ConfirmedBlock> {
    ...
    async getBlocks(startSlot: number | bigint, config?: GetBlocksConfig): Promise<GetBlocksResult>;
    ...
    async getBlocks(
        ...
        if (typeof endSlotOrCommitmentOrConfig === 'number' || typeof endSlotOrCommitmentOrConfig === 'bigint') {
            rpcEndSlot = coerceNumericToBigInt(endSlotOrCommitmentOrConfig, 'endSlot');
            rawCommitmentOrConfig = commitmentOrConfig;
        } else {
    ...
    async getBlocksWithLimit(
        ...
        const rpcConfig = { commitment: rpcFinality };
        const rpcStartSlot = coerceNumericToBigInt(startSlot, 'startSlot');
        const getBlocksWithLimit = this._typedRpc.getBlocksWithLimit as TypedRpcRequestMethod<
     ...
     */
    async getBlockCommitment(slot: number | bigint): Promise<ReturnType<GetBlockCommitmentApi['getBlockCommitment']>> {
        try {
     ...
     */
    getBlockSignatures(slot: number | bigint, commitment?: Finality): Promise<BlockSignatures> {
     ...
     */
    getConfirmedBlockSignatures(slot: number | bigint, commitment?: Finality): Promise<BlockSignatures> {
        return fetchBlockSignaturesFromRpc(
            this._typedRpc,
            slot,
            this._resolveSupportedFinality(commitment),
    ...
    async getConfirmedTransaction(
        ...
        return {
            ...result,
            transaction: Transaction.populate(result.transaction.message, result.transaction.signatures),
        };
    }
    ...
    async getSignaturesForAddress(
        ...
        const rpcConfig = {
            ...
            ...(options?.minContextSlot != null
                ? {
                      minContextSlot: coerceNumericToBigInt(options.minContextSlot, 'minContextSlot'),
                  }
                : null),
        ...
        try {
            const response = await this._typedRpc.getSignaturesForAddress(address.toBase58(), rpcConfig).send();
            return response.map(({ signature, ...rest }) => ({ signature, ...rest }));
        } catch (error) {
            throwSolanaRpcErrorIfNeeded(error, 'failed to get signatures for address');
        }
    }
    ...
    async getAddressLookupTable(
        ...
        if (accountInfo !== null) {
            if (!accountInfo.owner.equals(AddressLookupTableProgram.programId)) {
                throw new Error(`Account ${accountKey.toBase58()} is not owned by the Address Lookup Table program`);
            }
     ...
     * @internal
     */
    async _blockhashWithExpiryBlockHeight(disableCache: boolean): Promise<BlockhashWithExpiryBlockHeight> {
        if (!disableCache) {
    ...
    async _pollNewBlockhash(): Promise<BlockhashWithExpiryBlockHeight> {
        ...
        try {
            ...
            for (let i = 0; i < 50; i++) {
                const latestBlockhashResult = await this.getLatestBlockhash('finalized');
    ...
    ): Promise<ReturnType<GetStakeMinimumDelegationApi['getStakeMinimumDelegation']>> {
        try {
            return await this._typedRpc
    ...
    async simulateTransaction(
        ...
        configOrSigners?: SimulateTransactionConfig | Array<TransactionPartialSigner>,
        includeAccounts?: boolean | Array<PublicKey>,
    ): Promise<RpcResponseAndContext<SimulatedTransactionResponse>> {
        ...
        if ('message' in transactionOrMessage) {
            ...
            if (includeAccounts) {
                const addresses = (Array.isArray(includeAccounts) ? includeAccounts : message.nonProgramIds()).map(
                    key => key.toBase58(),
                );
        ...
        const rpcConfigBase = {
            ...
            ...(config.accounts != null
                ? {
                      accounts: {
                          encoding: 'base64' as const,
                          addresses: config.accounts.addresses.map(address => new PublicKey(address).toBase58()),
                      },
                  }
                : null),
            ...(config.commitment != null ? { commitment: config.commitment } : null),
            ...(config.innerInstructions !== undefined ? { innerInstructions: config.innerInstructions } : null),
            ...(minContextSlot != null ? { minContextSlot } : null),
        };

        const base64EncodedWireTransaction = coerceToBase64EncodedWireTransaction(encodedTransaction);
        const simulateTransaction = this._typedRpc.simulateTransaction as TypedRpcRequestMethod<
            [transaction: Base64EncodedWireTransaction, config: TypedSimulateTransactionRequestConfig],
        ...
        try {
            const response = await simulateTransaction(base64EncodedWireTransaction, rpcConfig).send();
            return {
                context: response.context,
    ...
    async sendEncodedTransaction(encodedTransaction: string, options?: SendOptions): Promise<TransactionSignature> {
        ...
        try {
            return await this._typedRpc
                .sendTransaction(coerceToBase64EncodedWireTransaction(encodedTransaction), config)
                .send();
        } catch (error) {
            const sendTransactionErrorDetails = extractSendTransactionErrorDetails(error);
    ...
    private _registerSubscription<TKind extends SubscriptionKind>(
        ...
        return this._subscriptionController.registerSubscription(subscriptionConfig, dispatchConfig);
    }
    ...
    async awaitSubscriptionReady(
        ...
        const readinessPromise = new Promise<void>((resolve, reject) => {
            const settleSubscriptionReadiness = (nextState: ObservedSubscriptionState) => {
                if (nextState === 'subscribed') {
                    resolve();
                    return;
                }
                if (nextState === 'failed') {
                    reject(new Error(`Subscription with id \`${clientSubscriptionId}\` failed to establish.`));
                    return;
                }
                if (nextState === 'inactive') {
                    reject(new Error(`Subscription with id \`${clientSubscriptionId}\` is no longer active.`));
                }
            };
        ...
        try {
            await Promise.race([readinessPromise, this.getCancellationPromise(config?.abortSignal)]);
        } finally {
    ...
    onAccountChange(
        ...
        return this._registerSubscription({
            callback,
            spec: buildAccountSubscriptionSpec(publicKey, this._resolveSubscriptionConfig(commitmentOrConfig)),
        });
    }
     ...
     * @param clientSubscriptionId client subscription id to deregister
     */
    async removeProgramAccountChangeListener(clientSubscriptionId: ClientSubscriptionId): Promise<void> {
    ...
    onLogs(filter: LogsFilter, callback: LogsCallback, commitment?: Commitment): ClientSubscriptionId {
        return this._registerSubscription({
            callback,
            spec: buildLogsSubscriptionSpec(filter, this._resolveSubscriptionCommitment(commitment)),
        });
    }
     ...
     * @param clientSubscriptionId client subscription id to deregister
     */
    async removeSlotChangeListener(clientSubscriptionId: ClientSubscriptionId): Promise<void> {
     ...
     * @param clientSubscriptionId client subscription id to deregister
     */
    async removeSlotUpdateListener(clientSubscriptionId: ClientSubscriptionId): Promise<void> {
    ...
    private async _unsubscribeClientSubscription(clientSubscriptionId: ClientSubscriptionId, subscriptionName: string) {
        await this._subscriptionController.removeClientSubscription(clientSubscriptionId, subscriptionName);
    }
        ...
        const clientSubscriptionId = this._registerSubscription({
            callback: (notification, context) => {
                ...
                try {
                    ...
                    void this.removeSignatureListener(clientSubscriptionId).catch(() => {
                        // Already removed.
                    });
                }
            },
    ...
    onSignatureWithOptions(
        ...
        const clientSubscriptionId = this._registerSubscription({
            ...
            spec: buildSignatureSubscriptionSpec(signature, this._resolveSubscriptionConfig(options)),
        });
        return clientSubscriptionId;
    }
     ...
     * @param clientSubscriptionId client subscription id to deregister
     */
    async removeSignatureListener(clientSubscriptionId: ClientSubscriptionId): Promise<void> {
     ...
     * @param clientSubscriptionId client subscription id to deregister
     */
    async removeRootChangeListener(clientSubscriptionId: ClientSubscriptionId): Promise<void> {
    ...
    onBlock(
        ...
        return this._registerSubscription(
            ...
            config == null ? { defaultDispatchConfig: 'default' } : { dispatchConfig: config },
        );
    }
     ...
     * @param clientSubscriptionId client subscription id to deregister
     */
    async removeBlockListener(clientSubscriptionId: ClientSubscriptionId): Promise<void> {
     ...
     * @param clientSubscriptionId client subscription id to deregister
     */
    async removeVoteListener(clientSubscriptionId: ClientSubscriptionId): Promise<void> {
