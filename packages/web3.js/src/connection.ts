...
type GetTokenLargestAccountsWithPublicKeys = RpcResponseAndContext<ReadonlyArray<TokenAccountBalancePair>>;

type GetLargestAccountsWithPublicKeys = RpcResponseAndContext<ReadonlyArray<AccountBalancePair>>;
...
type RpcTransportConfig = Readonly<
    Pick<ConnectionConfig, 'disableRetryOnRateLimit' | 'fetch' | 'fetchMiddleware' | 'httpHeaders'>
>;
...
export class Connection {
     ...
     */
    async getBalance(publicKey: PublicKey, commitmentOrConfig?: Commitment | GetBalanceConfig): Promise<bigint> {
    ...
    async getTokenAccountsByOwner(
        ...
        const { commitment, config } = extractCommitmentFromConfig(commitmentOrConfig);
        const typedFilter =
            'mint' in filter ? { mint: filter.mint.toAddress() } : { programId: filter.programId.toAddress() };
        const rpcCommitment = this._resolveCommitment(commitment);
        const minContextSlot = coerceOptionalNumericToBigInt(config?.minContextSlot, 'minContextSlot');
    ...
    private async confirmTransactionUsingDurableNonceStrategy({
        ...
        let done: boolean = false;
        const nonceMinContextSlot = coerceNumericToBigInt(minContextSlot, 'minContextSlot');
        const cancellationPromise = this.getCancellationPromise(abortSignal);
        ...
        try {
            ...
            if (outcome.__type === TransactionStatus.PROCESSED) {
                ...
                while (true) {
                    ...
                    if (status.context.slot < (outcome.slotInWhichNonceDidAdvance ?? nonceMinContextSlot)) {
                        await sleep(400);
                        continue;
                    }
     ...
     */
    async getHighestSnapshotSlot(): Promise<Readonly<{ full: bigint; incremental: bigint | null }>> {
        try {
    ...
    async getVoteAccounts(commitmentOrConfig?: Commitment | GetVoteAccountsConfig): Promise<VoteAccountStatus> {
        const { commitment, config } = extractCommitmentFromConfig(commitmentOrConfig);
        const rpcCommitment = this._resolveCommitment(commitment);
        const typedVotePubkey = config?.votePubkey == null ? undefined : new PublicKey(config.votePubkey).toAddress();
     ...
     */
    async getSlot(commitmentOrConfig?: Commitment | GetSlotConfig): Promise<bigint> {
        const { commitment, config } = extractCommitmentFromConfig(commitmentOrConfig);
        const rpcCommitment = this._resolveCommitment(commitment);
        const minContextSlot = config?.minContextSlot;
     ...
     */
    async getRecentPerformanceSamples(limit?: number): Promise<readonly PerfSample[]> {
        try {
            return await (
    ...
    async getRecentPrioritizationFees(
        ...
        const accounts = config?.lockedWritableAccounts?.map(key => key.toAddress());
        try {
            return await (
                accounts == null
    ...
    async getSignaturesForAddress(
        ...
        try {
            const response = await this._typedRpc.getSignaturesForAddress(address.toAddress(), rpcConfig).send();
     ...
     */
    async getStakeMinimumDelegation(config?: GetStakeMinimumDelegationConfig): Promise<RpcResponseAndContext<bigint>> {
        try {
            return await this._typedRpc
    ...
    async simulateTransaction(
        ...
        const rpcConfigBase = {
            ...
            ...(config.accounts != null
                ? {
                      accounts: {
                          encoding: 'base64' as const,
                          addresses: config.accounts.addresses.map(address => new PublicKey(address).toAddress()),
                      },
                  }
                : null),
        ...
        try {
            ...
            if (Array.isArray(rawLogs) && rawLogs.every((log): log is string => typeof log === 'string')) {
                logs = rawLogs;
            }
            if (logs) {
                const traceIndent = '\n    ';
                const logTrace = traceIndent + logs.join(traceIndent);
                console.error(isJsonRpcErrorLike(error) ? error.message : String(error), logTrace);
            }
