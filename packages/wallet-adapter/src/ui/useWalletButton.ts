...
export function useWalletConnectButton() {
    ...
    const buttonState = connecting
        ? 'connecting'
        : connected && (!selectedWallet || selectedWallet.adapter.name === wallet?.adapter.name)
          ? 'connected'
          : target
            ? 'has-wallet'
            : 'no-wallet';
...
export function useWalletDisconnectButton() {
    ...
    const buttonState = disconnecting ? 'disconnecting' : connected ? 'has-wallet' : 'no-wallet';
    return {
        buttonState,
        buttonDisabled: buttonState !== 'has-wallet',
...
export function useWalletMultiButton({
    ...
    onSelectWallet: (config: { wallets: Wallet[]; onSelectWallet: (name: string) => void }) => void;
}) {
    ...
    return {
        ...
        onDisconnect: buttonState !== 'disconnecting' && buttonState !== 'no-wallet' ? disconnect : undefined,
        onSelectWallet: () => onSelectWallet({ wallets: [...wallets], onSelectWallet: select }),
