...
beforeAll(async () => {
    [signer, other] = await Promise.all([generateKeyPairSigner(), generateKeyPairSigner()]);
});
...
describe('verifySignIn', () => {
    ...
    it('accepts an offchain sign-in for the requested address', async () => {
        ...
        expect(await verifySignIn({ ...input, address: signer.address }, output)).toBe(true);
    });

    it('rejects an offchain sign-in from an account other than the requested one', async () => {
        const output = await offchainOutput();
        expect(await verifySignIn({ ...input, address: other.address }, output)).toBe(false);
    });
    ...
    it('verifies a plain sign-in through wallet-standard-util', async () => {
        const signedMessage = new TextEncoder().encode(createSignInMessageText({ ...input, address: signer.address }));
        const output = {
        ...
        expect(await verifySignIn({ ...input, statement: 'Nope.' }, output)).toBe(false);
    });
});
