...
it('persists useLocalStorage updates and removes null without writing on mount', () => {
    ...
    const { result, unmount } = renderHook(() => useLocalStorage<number | null>('hook-state', 0), {
        wrapper: StrictMode,
    });
...
it('keeps useLocalStorage state usable when storage is unavailable', () => {
    const unavailable = vi.spyOn(globalThis, 'localStorage', 'get').mockImplementation(() => {
...
it('adopts the stored value of a new key instead of copying the previous key into it', () => {
    ...
    const { result, rerender } = renderHook(({ key }) => useLocalStorage<string | null>(key, 'fallback'), {
