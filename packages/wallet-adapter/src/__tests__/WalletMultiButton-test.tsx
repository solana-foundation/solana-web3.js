...
import { useWalletConnectButton, useWalletDisconnectButton, useWalletMultiButton } from '../ui/useWalletButton.js';
import { BaseWalletConnectButton, WalletDisconnectButton } from '../ui/WalletConnectionButton.js';
...
describe('WalletMultiButton', () => {
    ...
    it('shows the truncated address and disconnects from the dropdown when connected', async () => {
        ...
        expect(wallet.features['standard:disconnect'].disconnect).toHaveBeenCalledTimes(2);
...
it('keeps custom labels, cancellable clicks and headless operation failures', async () => {
    ...
    await act(async () => {
        await expect(result.current.connect.onButtonClick!()).rejects.toMatchObject({ cause: rejection });
    });
    ...
    expect(result.current.multi.publicKey).toEqual(result.current.wallet.publicKey);
    await act(async () => {
...
it('shows the pending wallet separately and preserves the active account after rejection', async () => {
    ...
    await act(async () => {
        fireEvent.click(within(screen.getByRole('dialog')).getByRole('button', { name: /Pending/ }));
    });
...
it('ignores a late rejection from a superseded picker request', async () => {
    ...
    fireEvent.click(within(dialog).getByRole('button', { name: 'Close wallet selection' }));
