...
import { WalletConfigError, WalletNotReadyError, type WalletControllerOptions } from '../core.js';
...
describe('provider', () => {
    ...
    it('observes browser registration and recreates Kit configuration without leaking StrictMode resources', async () => {
        ...
        let filter: WalletControllerOptions['filter'] = candidate => candidate.name !== excluded.wallet.name;
        ...
        renderHook(() => null, {
            wrapper: () => (
                <Suspense fallback={null}>
                    <WalletProvider chain="solana:devnet" storage={null} children={null} />
                    <Suspend />
                </Suspense>
            ),
        });
...
it('publishes the same authorized capabilities through WalletContext and the React hooks', async () => {
    const { wallet, listeners } = standardWallet();
    wallet.accounts[0]!.features = ['solana:signMessage', 'solana:signTransaction'];
    const output = {
    ...
    const signMessage = vi.fn(async (_input: { account: unknown; message: Uint8Array }) => [output]);
    const signing = {
        ...wallet,
        features: {
            ...wallet.features,
    ...
    const { result } = renderHook(
        () => {
            ...
            expect(anchor?.publicKey).toBe(wallet.signTransaction ? wallet.publicKey : undefined);
    ...
    expect(await result.current.signMessage!(new Uint8Array([1]))).toEqual(output.signature);
...
it('signs in over an offchain message only when the wallet advertises solana:signIn 1.1.0', async () => {
    ...
    expect(result.current.supportsSignInWithOffchainMessage).toBe(false);
    await act(async () => {
        await expect(result.current.signIn!(input)).rejects.toThrow(WalletNotReadyError);
    });
    ...
    expect(result.current.publicKey?.toBase58()).toBe(upgraded.accounts[0]!.address);
});
