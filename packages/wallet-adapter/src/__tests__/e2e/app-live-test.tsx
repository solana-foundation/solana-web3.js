...
import { WalletNotConnectedError, WalletSendTransactionError } from '../../errors.js';
...
describe.skipIf(!process.env.TEST_LIVE)('example app on a live surfnet', () => {
    ...
    beforeAll(async () => {
        ...
        const messageOnlyWallet = await createTestWallet({
            features: ['standard:connect', 'standard:disconnect', 'standard:events', 'solana:signMessage'],
        ...
        for (const { wallet } of [fullWallet, legacyOnlyWallet, messageOnlyWallet, mainnetWallet]) {
    ...
    it('filters the wrong-chain wallet and lists the rest in the modal', () => {
        ...
        expect(result.current.wallets.map(({ adapter }) => adapter.name).sort()).toEqual([
            'Full wallet',
            'Legacy-only wallet',
            'Message-only wallet',
        ]);
        expect(result.current.wallets.find(({ adapter }) => adapter.name === 'Mainnet wallet')).toBeUndefined();
        ...
        for (const name of ['Full wallet', 'Legacy-only wallet', 'Message-only wallet']) {
            expect(dialog.getByRole('button', { name: new RegExp(name, 'i') })).toBeDefined();
        }
    ...
    it("signs a message that verifies against the connected account's public key", async () => {
        ...
        const publicKey = await getPublicKeyFromAddress(address(fullSigner.address));
    ...
    it('rejects an unsupported transaction version, then rejects sending while disconnected', async () => {
        ...
        await expect(result.current.sendTransaction(versioned, connection)).rejects.toBeInstanceOf(
            WalletSendTransactionError,
        );
        await act(() => result.current.disconnect());
        await expect(result.current.sendTransaction(new Transaction(), connection)).rejects.toBeInstanceOf(
    ...
    it('connects, sends and switches wallets through the UI, then disconnects', async () => {
        ...
        await waitFor(() => expect(screen.getByTestId('address').textContent).toBe(fullSigner.address));
