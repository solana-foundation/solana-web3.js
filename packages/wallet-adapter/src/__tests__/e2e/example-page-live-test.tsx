...
const MEMO_PROGRAM_ID = 'Memo4c2pN8afCj432Lb7RMVKi9PbQnnW7ewFFaV3oAH';
...
describe.skipIf(!process.env.TEST_LIVE)(
  'example page on a live surfnet',
  () => {
    let surfnet: Surfnet;
    ...
    beforeAll(async () => {
      ...
      surfnet.deploy({
        ...
        soPath: join(__dirname, 'fixtures/memo4c.so'),
      });
      process.env.NEXT_PUBLIC_RPC_URL = surfnet.rpcUrl;
      ...
      surfnet.fundSol(fullWallet.signers[0]!.address, FUNDING_LAMPORTS);
    ...
    async function connectTo(name: string) {
      ...
      await waitFor(
        () =>
          expect(['Connect', 'Connected']).toContain(
