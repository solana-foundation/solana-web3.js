...
async function signWireTransaction(signer: KeyPairSigner, wire: Uint8Array): Promise<Uint8Array> {
    const codec = getTransactionCodec();
    const transaction = codec.decode(wire) as Parameters<KeyPairSigner['signTransactions']>[0][number];
...
export async function createTestWallet(options: TestWalletOptions = {}) {
    ...
    const signers =
        options.signers ?? (await Promise.all(Array.from({ length: options.accounts ?? 1 }, generateKeyPairSigner)));
    const base58 = getBase58Encoder();
    const accountFeatures = features.filter(feature => feature.startsWith('solana:'));
    ...
    async function signBytes(signer: KeyPairSigner, bytes: Uint8Array): Promise<Uint8Array> {
    ...
    async function sendSignedTransaction(signed: Uint8Array): Promise<string> {
        if (!rpcUrl) {
            throw new Error('Pass `rpcUrl` to createTestWallet to use solana:signAndSendTransaction.');
        }
        const response = await fetch(rpcUrl, {
            body: JSON.stringify({
                ...
                params: [getBase64Decoder().decode(signed), { encoding: 'base64', preflightCommitment: 'confirmed' }],
            }),
    ...
    const allFeatures = {
        ...
        'solana:signIn': {
            ...
            signIn: async (
                ...
                await Promise.all(
                    (inputs.length ? inputs : [{}]).map(async ({ useOffchainMessage, ...input }) => {
                        ...
                        return {
                            account,
                            signature: await signBytes(signerFor(account.address), signedMessage),
                            signedMessage,
                            ...(useOffchainMessage && {
                                signedMessageFormat: {
            ...
            signOffchainMessage: async (
                ...
                await Promise.all(
                    inputs.map(async ({ account, message }) => {
                        ...
                        return {
                            signature: await signBytes(signerFor(account.address), signedOffchainMessage),
            ...
            signTransaction: async (
                ...
                await Promise.all(
                    inputs.map(async ({ account, transaction }) => ({
                        signedTransaction: await signWireTransaction(signerFor(account.address), transaction),
                    })),
                ),
        },
        'solana:signAndSendTransaction': {
            ...
            signAndSendTransaction: async (
                ...
                await Promise.all(
                    inputs.map(async ({ account, transaction }) => {
                        const signed = await signWireTransaction(signerFor(account.address), transaction);
    ...
    const wallet = {
        ...
        features: Object.fromEntries(features.map(feature => [feature, allFeatures[feature]])),
    };
