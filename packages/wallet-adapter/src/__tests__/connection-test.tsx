...
it('preserves the active account through a rejected switch and reports each failure once, to the adapter', async () => {
    ...
    b.wallet.features['standard:connect'].connect.mockRejectedValueOnce(rejection);
    ...
    expect(onError).toHaveBeenCalledExactlyOnceWith(failure, expect.objectContaining({ name: 'B' }));
    expect(owner.getSnapshot().account).toBe(account);
    ...
    b.wallet.features['standard:disconnect'].disconnect.mockRejectedValueOnce(remoteFailure);
    ...
    expect(owner.getSnapshot().selectedWallet?.adapter.name).toBe('A');
    expect(onError).toHaveBeenLastCalledWith(disconnectError, expect.objectContaining({ name: 'B' }));
...
it('accepts any selection, as in v1, rejects connecting to one that does not resolve, and lists a duplicated name once', async () => {
    ...
    try {
        owner.select('A');
        expect(owner.getSnapshot().wallets.map(wallet => wallet.adapter.name)).toEqual(['A', 'B']);
        await owner.connect();
        expect(owner.getSnapshot().publicKey?.toBase58()).toBe(a.wallet.accounts[0]!.address);
    } finally {
        remove();
    }
    expect(owner.getSnapshot().selectedWallet?.adapter.name).toBe('A');
