...
import { SystemInstruction, SystemProgram, Transaction, VersionedTransaction } from '@solana/web3.js';

const transactionCodec = getTransactionCodec();
type SignKitTransactions = (transactions: readonly KitTransaction[]) => Promise<readonly KitTransaction[]>;
...
export function serializeTransaction(transaction: Transaction | VersionedTransaction): Promise<Uint8Array> {
...
function advancesNonce(instruction: Transaction['instructions'][number] | undefined): boolean {
    return (
        instruction !== undefined &&
        instruction.programId.equals(SystemProgram.programId) &&
        SystemInstruction.decodeInstructionType(instruction) === 'AdvanceNonceAccount'
    );
}

/** Sign `@solana/web3.js` transactions, returning new transactions of the same class as the inputs. */
export async function signTransactionsWithKit<T extends Transaction | VersionedTransaction>(
    sign: SignKitTransactions,
    transactions: readonly T[],
): Promise<T[]> {
    const wireBytes = await Promise.all(transactions.map(serializeTransaction));
    const signed = await sign(wireBytes.map(bytes => transactionCodec.decode(bytes)));
    // Kit's signer maps outputs by position and never checks their count.
    if (signed.length !== transactions.length) throw new Error('Expected one signed transaction per input.');
    return transactions.map((transaction, i) => {
        const bytes = transactionCodec.encode(signed[i]!) as Uint8Array;
        if (isVersionedTransaction(transaction)) return VersionedTransaction.deserialize(bytes) as T;
        ...
        } else if (result.recentBlockhash === transaction.recentBlockhash && !advancesNonce(result.instructions[0])) {
...
function messageBytes(transaction: Transaction | VersionedTransaction): Uint8Array {
    return isVersionedTransaction(transaction) ? transaction.message.serialize() : transaction.serializeMessage();
}
...
function presentSignatures(transaction: Transaction | VersionedTransaction): Map<string, Uint8Array> {
    const entries: [string, Uint8Array | null][] = isVersionedTransaction(transaction)
        ? transaction.message.staticAccountKeys
              .slice(0, transaction.signatures.length)
              .map((key, index) => [key.toBase58(), transaction.signatures[index]!])
        : transaction.signatures.map(({ publicKey, signature }) => [publicKey.toBase58(), signature]);
    return new Map(entries.filter((entry): entry is [string, Uint8Array] => isPresent(entry[1])));
}
...
export function assertCallerSignaturesPreserved(
    ...
    if (!bytesEqual(messageBytes(original), messageBytes(signed))) {
        throw new Error('The wallet modified a transaction that carries additional signatures.');
    }
