 ...
 */
export type SupportedTransactionVersions = NonNullable<WalletState['connected']>['supportedTransactionVersions'];
...
export type SignOffchainMessageOutput = Omit<SolanaSignOffchainMessageOutput, 'signature'> & {
...
export interface WalletOperations {
    ...
    signOffchainMessage?: (message: string, options?: SignOffchainMessageOptions) => Promise<SignOffchainMessageOutput>;
...
export interface WalletContextState extends WalletOperations {
    ...
    readonly supportsSignInWithOffchainMessage: boolean;
