...
import { createWalletController, type WalletController, type WalletControllerOptions } from './wallet-controller.js';
...
type Operations = Pick<WalletController, 'select' | 'connect' | 'disconnect' | 'sendTransaction'>;
 ...
 */
function forwardedOperations(owner: { current: WalletController | null }): Operations {
    const mounted = () => {
        if (!owner.current) throw new WalletNotReadyError('WalletProvider has not mounted.');
...
function pendingSnapshot(operations: Operations, autoConnect: boolean): WalletContextState {
...
export function WalletProvider({
    ...
    const [pending] = useState(() => pendingSnapshot(operations, autoConnect ?? true));
    ...
    return <WalletContext.Provider value={snapshot}>{children}</WalletContext.Provider>;
}
